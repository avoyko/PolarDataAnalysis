...
std::string FieldBase::GetName(const FIT_UINT16 subFieldIndex) const {
  if (subFieldIndex >= GetNumSubFields()) return GetName();
...
FIT_UINT8 FieldBase::GetType(const FIT_UINT16 subFieldIndex) const {
  if (subFieldIndex >= GetNumSubFields()) return GetType();
...
std::string FieldBase::GetUnits(const FIT_UINT16 subFieldIndex) const {
  if (subFieldIndex >= GetNumSubFields()) return GetUnits();
...
FIT_FLOAT64 FieldBase::GetScale(const FIT_UINT16 subFieldIndex) const {
  if (subFieldIndex >= GetNumSubFields()) return GetScale();
...
FIT_FLOAT64 FieldBase::GetOffset(const FIT_UINT16 subFieldIndex) const {
  if (subFieldIndex >= GetNumSubFields()) return GetOffset();
...
FIT_UINT8 FieldBase::GetNumValues(void) const {
  ...
  if (GetType() != FIT_BASE_TYPE_STRING)
    return (FIT_UINT8)(values.size() /
                       baseTypeSizes[GetType() & FIT_BASE_TYPE_NUM_MASK]);

  return (FIT_UINT8)stringIndexes.size();
}

FIT_UINT32 FieldBase::GetBitsValue(const FIT_UINT16 offset,
                                   const FIT_UINT8 bits) const {
  FIT_UINT32 value = 0;
  ...
  if (values.size() == 0) return FIT_UINT32_INVALID;

  while (numValueBits < bits) {
    if (index >= GetSize()) return FIT_UINT32_INVALID;

    if (newOffset >= 8) {
      newOffset -= 8;
    } else {
      // Get Nth byte from the jagged array considering elements may be
      // multibyte
      ...
      // If the number of data bits will overflow the number of bits we want in
      // the final value, only use as many as will fit
      if (numDataBitsUsed > (bits - numValueBits))
        numDataBitsUsed = bits - numValueBits;

      mask = (1 << numDataBitsUsed) - 1;
      // OR the data from this byte into our final value, then update number of
      // bits in our final value
...
FIT_SINT32 FieldBase::GetBitsSignedValue(const FIT_UINT16 offset,
                                         const FIT_UINT8 bits) const {
  FIT_UINT32 value;
  ...
  if (value == FIT_UINT32_INVALID) return FIT_SINT32_INVALID;
...
FIT_FLOAT32 FieldBase::GetFLOAT32Value(const FIT_UINT8 fieldArrayIndex,
                                       const FIT_UINT16 subFieldIndex) const {
  FIT_FLOAT32 float32Value;

  if (!IsValid()) return FIT_FLOAT32_INVALID;

  switch (GetType()) {  // Note: This checks the type of the MAIN field since
                        // data is aligned according to that type
    case FIT_BASE_TYPE_BYTE: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_BYTE>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }

    case FIT_BASE_TYPE_ENUM: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_ENUM>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }

    case FIT_BASE_TYPE_SINT8: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_SINT8>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }
    ...
    case FIT_BASE_TYPE_UINT8Z: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_UINT8>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }

    case FIT_BASE_TYPE_SINT16: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_SINT16>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }
    ...
    case FIT_BASE_TYPE_UINT16Z: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_UINT16>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }

    case FIT_BASE_TYPE_SINT32: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_SINT32>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }
    ...
    case FIT_BASE_TYPE_UINT32Z: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_UINT32>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }

    case FIT_BASE_TYPE_SINT64: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_SINT64>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }
    ...
    case FIT_BASE_TYPE_UINT64Z: {
      float32Value = ConvertBaseType<FIT_FLOAT32, FIT_UINT64>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT32);
      break;
    }
    ...
    case FIT_BASE_TYPE_FLOAT32: {
      ...
      if (sint32Value == FIT_SINT32_INVALID) return FIT_FLOAT32_INVALID;
  ...
  if (memcmp(&float32Value,
             baseTypeInvalids[FIT_BASE_TYPE_FLOAT32 & FIT_BASE_TYPE_NUM_MASK],
             sizeof(FIT_FLOAT32)) == 0) {
    return float32Value;
  }

  return static_cast<FIT_FLOAT32>(static_cast<FIT_FLOAT64>(float32Value) /
                                      GetScale(subFieldIndex) -
                                  GetOffset(subFieldIndex));
}

FIT_FLOAT64 FieldBase::GetFLOAT64Value(const FIT_UINT8 fieldArrayIndex,
                                       const FIT_UINT16 subFieldIndex) const {
  FIT_FLOAT64 float64Value;

  if (!IsValid()) return FIT_FLOAT64_INVALID;

  switch (GetType()) {  // Note: This checks the type of the MAIN field since
                        // data is aligned according to that type
    case FIT_BASE_TYPE_BYTE: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_BYTE>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }

    case FIT_BASE_TYPE_ENUM: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_ENUM>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }

    case FIT_BASE_TYPE_SINT8: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_SINT8>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }
    ...
    case FIT_BASE_TYPE_UINT8:
    case FIT_BASE_TYPE_UINT8Z: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_UINT8>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }

    case FIT_BASE_TYPE_SINT16: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_SINT16>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }
    ...
    case FIT_BASE_TYPE_UINT16Z:
    case FIT_BASE_TYPE_UINT16: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_UINT16>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }

    case FIT_BASE_TYPE_SINT32: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_SINT32>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }
    ...
    case FIT_BASE_TYPE_UINT32:
    case FIT_BASE_TYPE_UINT32Z: {
      float64Value = ConvertBaseType<FIT_FLOAT64, FIT_UINT32>(
          fieldArrayIndex, FIT_BASE_TYPE_FLOAT64);
      break;
    }
  ...
  if (memcmp(&float64Value,
             baseTypeInvalids[FIT_BASE_TYPE_FLOAT64 & FIT_BASE_TYPE_NUM_MASK],
             sizeof(FIT_FLOAT64)) == 0) {
    return float64Value;
  }
...
FIT_WSTRING FieldBase::GetSTRINGValue(const FIT_UINT8 fieldArrayIndex,
                                      const FIT_UINT16 subFieldIndex) const {
  Unicode::UTF8_STRING value;

  if (!IsValid()) return FIT_WSTRING_INVALID;

  if (GetType() ==
      FIT_BASE_TYPE_STRING)  // Note: This checks the type of the MAIN field
                             // since data is aligned according to that type
  {
    FIT_UINT8 numStrings = (FIT_UINT8)stringIndexes.size();

    if (numStrings == 0) return FIT_WSTRING_INVALID;
    ...
    // If this is the last string, its length is the size of the array
    // subtracted by its start position.
    ...
    if (floatValue != FIT_FLOAT64_INVALID) {
      ...
      if ((value.find('.') != Unicode::UTF8_STRING::npos) &&
          (value[value.length() - 1] == '0')) {
        Unicode::UTF8_STRING::size_type lastZeroIndex = value.length() - 1;

        while (value[lastZeroIndex - 1] == '0') lastZeroIndex--;
...
FIT_UINT16 FieldBase::GetSubField(const std::string &subFieldName) const {
  for (FIT_UINT16 i = 0; i < this->GetNumSubFields(); i++) {
    if (this->GetSubField(i)->name.compare(subFieldName) == 0) return i;
  }

  return FIT_SUBFIELD_INDEX_MAIN_FIELD;
}

FIT_FLOAT64 FieldBase::GetRawValue() const { return GetRawValueInternal(0); }

FIT_FLOAT64 FieldBase::GetRawValue(const FIT_UINT8 fieldArrayIndex) const {
  return GetRawValueInternal(fieldArrayIndex);
}

FIT_BOOL FieldBase::GetMemoryValue(const FIT_UINT8 fieldArrayIndex,
                                   FIT_UINT8 *buffer,
...
FIT_FLOAT64 FieldBase::GetRawValueInternal(
    const FIT_UINT8 fieldArrayIndex) const {
  FIT_FLOAT64 float64Value;
  ...
  switch (GetType()) {  // Note: This checks the type of the MAIN field since
    ...
    case FIT_BASE_TYPE_BYTE: {
      ...
      if (byteValue == FIT_BYTE_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_ENUM: {
      ...
      if (enumValue == FIT_ENUM_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_SINT8: {
      ...
      if (sint8Value == FIT_SINT8_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_UINT8: {
      ...
      if (uint8Value == FIT_UINT8_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_UINT8Z: {
      ...
      if (uint8zValue == FIT_UINT8Z_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_SINT16: {
      ...
      if (sint16Value == FIT_SINT16_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_UINT16: {
      ...
      if (uint16Value == FIT_UINT16_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_UINT16Z: {
      ...
      if (uint16zValue == FIT_UINT16Z_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_SINT32: {
      ...
      if (sint32Value == FIT_SINT32_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_UINT32: {
      ...
      if (uint32Value == FIT_UINT32_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_UINT32Z: {
      ...
      if (uint32zValue == FIT_UINT32Z_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_FLOAT32: {
      ...
      if (float32Value == FIT_FLOAT32_INVALID) return FIT_FLOAT64_INVALID;
    ...
    case FIT_BASE_TYPE_FLOAT64: {
      ...
      for (size_t i = 0; i < sizeof(FIT_UINT64); i++) {
        uint64Value |=
            (values[(fieldArrayIndex * sizeof(FIT_UINT64)) + i] << (i * 8));
      }
...
void FieldBase::AddValue(const FIT_FLOAT64 value,
                         const FIT_UINT8 fieldArrayIndex,
                         const FIT_UINT16 subFieldIndex) {
  SetFLOAT64Value(value, fieldArrayIndex, subFieldIndex);
}

// rawValue is already the correct quantity (scale/offsets applied) but possibly
// not the correct underlying type
void FieldBase::AddRawValue(const FIT_FLOAT64 rawValue,
                            const FIT_UINT8 fieldArrayIndex) {
  ...
  switch (GetType()) {
    ...
    case FIT_BASE_TYPE_FLOAT64:
      if ((FIT_UINT8)values.size() <
          ((fieldArrayIndex + 1) * sizeof(FIT_FLOAT64))) {
        values.resize((fieldArrayIndex + 1) * 8);
      }

      for (size_t i = 0; i < sizeof(FIT_FLOAT64); i++) {
        values[(fieldArrayIndex * sizeof(FIT_FLOAT64) + i)] =
            (*(((FIT_BYTE *)&rawValue) + i));
      }
      break;
...
void FieldBase::SetENUMValue(const FIT_ENUM value,
                             const FIT_UINT8 fieldArrayIndex) {
  SetUINT8Value(value, fieldArrayIndex);
}

void FieldBase::SetBYTEValue(const FIT_BYTE value,
                             const FIT_UINT8 fieldArrayIndex) {
  SetUINT8Value(value, fieldArrayIndex);
}

void FieldBase::SetSINT8Value(const FIT_SINT8 value,
                              const FIT_UINT8 fieldArrayIndex) {
  SetUINT8Value(value, fieldArrayIndex);
}

void FieldBase::SetUINT8Value(const FIT_UINT8 value,
                              const FIT_UINT8 fieldArrayIndex) {
...
void FieldBase::SetUINT8ZValue(const FIT_UINT8 value,
                               const FIT_UINT8 fieldArrayIndex) {
  SetUINT8Value(value, fieldArrayIndex);
}

void FieldBase::SetSINT16Value(const FIT_SINT16 value,
                               const FIT_UINT8 fieldArrayIndex) {
  SetUINT16Value(value, fieldArrayIndex);
}

void FieldBase::SetUINT16Value(const FIT_UINT16 value,
                               const FIT_UINT8 fieldArrayIndex) {
  ...
  for (size_t i = 0; i < sizeof(FIT_UINT16); i++) {
    values[(fieldArrayIndex * sizeof(FIT_UINT16)) + i] =
        ((FIT_BYTE)(value >> (8 * i)));
  }
}

void FieldBase::SetUINT16ZValue(const FIT_UINT16Z value,
                                const FIT_UINT8 fieldArrayIndex) {
  SetUINT16Value(value, fieldArrayIndex);
}

void FieldBase::SetSINT32Value(const FIT_SINT32 value,
                               const FIT_UINT8 fieldArrayIndex) {
  SetUINT32Value(value, fieldArrayIndex);
}

void FieldBase::SetUINT32Value(const FIT_UINT32 value,
                               const FIT_UINT8 fieldArrayIndex) {
  ...
  for (size_t i = 0; i < sizeof(FIT_UINT32); i++) {
    values[(fieldArrayIndex * sizeof(FIT_UINT32)) + i] =
        ((FIT_BYTE)(value >> (8 * i)));
  }
}

void FieldBase::SetUINT32ZValue(const FIT_UINT32Z value,
                                const FIT_UINT8 fieldArrayIndex) {
  SetUINT32Value(value, fieldArrayIndex);
}

void FieldBase::SetSINT64Value(const FIT_SINT64 value,
                               const FIT_UINT8 fieldArrayIndex) {
...
void FieldBase::SetUINT64Value(const FIT_UINT64 value,
  ...
  for (size_t i = 0; i < sizeof(FIT_UINT64); i++) {
    values[(fieldArrayIndex * sizeof(FIT_UINT64)) + i] =
        ((FIT_BYTE)(value >> (8 * i)));
  }
}

void FieldBase::SetUINT64ZValue(const FIT_UINT64Z value,
                                const FIT_UINT8 fieldArrayIndex) {
  SetUINT64Value(value, fieldArrayIndex);
}

void FieldBase::SetFLOAT32Value(const FIT_FLOAT32 value,
                                const FIT_UINT8 fieldArrayIndex,
                                const FIT_UINT16 subFieldIndex) {
  SetFLOAT64Value(value, fieldArrayIndex, subFieldIndex);
}

void FieldBase::SetFLOAT64Value(const FIT_FLOAT64 value,
                                const FIT_UINT8 fieldArrayIndex,
                                const FIT_UINT16 subFieldIndex) {
  if (!IsValid()) return;

  FIT_FLOAT64 recalculatedValue =
      (value + GetOffset(subFieldIndex)) * GetScale(subFieldIndex);
  // Make sure floating point representations trunc to the expected integer
  FIT_FLOAT64 roundedValue = (recalculatedValue >= 0.0)
                                 ? (recalculatedValue + 0.5)
                                 : (recalculatedValue - 0.5);
...
void FieldBase::SetSTRINGValue(const FIT_WSTRING &value,
                               const FIT_UINT8 fieldArrayIndex) {
  if (!IsValid()) return;
...
FIT_BOOL FieldBase::Read(const void *data, const FIT_UINT8 size) {
  ...
  switch (GetType()) {
    ...
    default: {
      ...
      while (bytes > 0) {
        if (memcmp(point, baseTypeInvalids[GetType() & FIT_BASE_TYPE_NUM_MASK],
                   typeSize) != 0) {
...
FIT_BOOL FieldBase::IsValueValid(const FIT_UINT8 fieldArrayIndex,
                                 const FIT_UINT16 subfieldIndex) const {
...
T FieldBase::GetValue(const FIT_UINT8 fieldArrayIndex) const {
  ...
  if (fieldArrayIndex >= (FIT_UINT8)values.size()) {
    return *reinterpret_cast<const T *>(
        baseTypeInvalids[GetType() & FIT_BASE_TYPE_NUM_MASK]);
  }
...
TTo FieldBase::ConvertBaseType(const FIT_UINT8 fieldArrayIndex,
                               const FIT_UINT8 toBaseType) const {
  FIT_CONST_UINT8_PTR invalid =
      baseTypeInvalids[GetType() & FIT_BASE_TYPE_NUM_MASK];
  TFrom value = GetValue<TFrom>(fieldArrayIndex);

  if (memcmp(&value, invalid, sizeof(TFrom)) == 0)
    return *reinterpret_cast<const TTo *>(
        baseTypeInvalids[toBaseType & FIT_BASE_TYPE_NUM_MASK]);
