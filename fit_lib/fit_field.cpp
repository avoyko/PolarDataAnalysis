...
void Field::SetBaseType(FIT_UINT8 type) { this->type = type; }
...
FIT_UINT16 Field::GetIndex(void) const { return profileIndex; }
...
FIT_UINT8 Field::GetType() const {
  if (type != FIT_UINT8_INVALID) return type;
...
std::string Field::GetUnits() const {
  if ((profile == NULL) || (profileIndex >= profile->numFields)) return "";
  return profile->fields[profileIndex].units;
}

FIT_FLOAT64 Field::GetScale() const {
  if ((profile == NULL) || (profileIndex >= profile->numFields)) return 1;
  return profile->fields[profileIndex].scale;
}

FIT_FLOAT64 Field::GetOffset() const {
  if ((profile == NULL) || (profileIndex >= profile->numFields)) return 0;
...
FIT_BOOL Field::GetIsExpanded(void) const { return isFieldExpanded; }

void Field::SetIsExpanded(FIT_BOOL newValue) { isFieldExpanded = newValue; }

const Profile::FIELD_COMPONENT* Field::GetComponent(
    const FIT_UINT16 component) const {
  if (component >= GetNumComponents()) return NULL;
  return &profile->fields[profileIndex].components[component];
}

const Profile::SUBFIELD* Field::GetSubField(
    const FIT_UINT16 subFieldIndex) const {
  if (subFieldIndex >= GetNumSubFields()) return NULL;
