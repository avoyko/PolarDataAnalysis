...
#if !defined( \
    FIT_CPP_INCLUDE_C)  // Types defined in fit_product.h if including C code.
...
#define FIT_FILE_DEVICE \
  ((FIT_FILE)1)  // Read only, single file. Must be in root directory.
#define FIT_FILE_SETTINGS \
  ((FIT_FILE)2)  // Read/write, single file. Directory=Settings
#define FIT_FILE_SPORT \
  ((FIT_FILE)3)  // Read/write, multiple files, file number = sport type.
                 // Directory=Sports
#define FIT_FILE_ACTIVITY \
  ((FIT_FILE)4)  // Read/erase, multiple files. Directory=Activities
#define FIT_FILE_WORKOUT \
  ((FIT_FILE)5)  // Read/write/erase, multiple files. Directory=Workouts
#define FIT_FILE_COURSE \
  ((FIT_FILE)6)  // Read/write/erase, multiple files. Directory=Courses
#define FIT_FILE_SCHEDULES \
  ((FIT_FILE)7)  // Read/write, single file. Directory=Schedules
#define FIT_FILE_WEIGHT \
  ((FIT_FILE)9)  // Read only, single file. Circular buffer. All message
                 // definitions at start of file. Directory=Weight
#define FIT_FILE_TOTALS \
  ((FIT_FILE)10)  // Read only, single file. Directory=Totals
#define FIT_FILE_GOALS \
  ((FIT_FILE)11)  // Read/write, single file. Directory=Goals
#define FIT_FILE_BLOOD_PRESSURE \
  ((FIT_FILE)14)  // Read only. Directory=Blood Pressure
#define FIT_FILE_MONITORING_A \
  ((FIT_FILE)15)  // Read only. Directory=Monitoring. File number=sub type.
#define FIT_FILE_ACTIVITY_SUMMARY \
  ((FIT_FILE)20)  // Read/erase, multiple files. Directory=Activities
#define FIT_FILE_MONITORING_DAILY ((FIT_FILE)28)
#define FIT_FILE_MONITORING_B \
  ((FIT_FILE)32)  // Read only. Directory=Monitoring. File number=identifier
#define FIT_FILE_SEGMENT \
  ((FIT_FILE)34)  // Read/write/erase. Multiple Files. Directory=Segments
#define FIT_FILE_SEGMENT_LIST \
  ((FIT_FILE)35)  // Read/write/erase. Single File. Directory=Segments
#define FIT_FILE_EXD_CONFIGURATION \
  ((FIT_FILE)40)  // Read/write/erase. Single File. Directory=Settings
#define FIT_FILE_MFG_RANGE_MIN \
  ((FIT_FILE)0xF7)  // 0xF7 - 0xFE reserved for manufacturer specific file types
...
#define FIT_MESG_NUM_MFG_RANGE_MIN \
  ((FIT_MESG_NUM)0xFF00)  // 0xFF00 - 0xFFFE reserved for manufacturer specific
                          // messages
#define FIT_MESG_NUM_MFG_RANGE_MAX \
  ((FIT_MESG_NUM)0xFFFE)  // 0xFF00 - 0xFFFE reserved for manufacturer specific
                          // messages
...
#define FIT_CHECKSUM_CLEAR \
  ((FIT_CHECKSUM)0)  // Allows clear of checksum for flash memory where can only
                     // write 1 to 0 without erasing sector.
#define FIT_CHECKSUM_OK \
  ((FIT_CHECKSUM)1)  // Set to mark checksum as valid if computes to invalid
                     // values 0 or 0xFF. Checksum can also be set to ok to save
...
#define FIT_DATE_TIME_MIN \
  ((FIT_DATE_TIME)0x10000000)  // if date_time is < 0x10000000 then it is system
                               // time (seconds from device power on)
...
#define FIT_LOCAL_DATE_TIME_MIN \
  ((FIT_LOCAL_DATE_TIME)0x10000000)  // if date_time is < 0x10000000 then it is
                                     // system time (seconds from device power
                                     // on)
...
#define FIT_MESSAGE_INDEX_SELECTED \
  ((FIT_MESSAGE_INDEX)0x8000)  // message is selected if set
#define FIT_MESSAGE_INDEX_RESERVED \
  ((FIT_MESSAGE_INDEX)0x7000)  // reserved (default 0)
...
#define FIT_DEVICE_INDEX_CREATOR \
  ((FIT_DEVICE_INDEX)0)  // Creator of the file is always device index 0.
...
typedef FIT_UINT8Z FIT_LANGUAGE_BITS_0;  // Bit field corresponding to language
                                         // enum type (1 << language).
...
#define FIT_SPORT_ALL \
  ((FIT_SPORT)254)  // All is for goals only to include all sports.
#define FIT_SPORT_COUNT 58

typedef FIT_UINT8Z FIT_SPORT_BITS_0;  // Bit field corresponding to sport enum
                                      // type (1 << sport).
...
#define FIT_SPORT_BITS_0_TRANSITION \
  ((FIT_SPORT_BITS_0)0x08)  // Mulitsport transition
...
typedef FIT_UINT8Z FIT_SPORT_BITS_1;  // Bit field corresponding to sport enum
                                      // type (1 << (sport-8)).
...
typedef FIT_UINT8Z FIT_SPORT_BITS_2;  // Bit field corresponding to sport enum
                                      // type (1 << (sport-16)).
...
typedef FIT_UINT8Z FIT_SPORT_BITS_3;  // Bit field corresponding to sport enum
                                      // type (1 << (sport-24)).
...
typedef FIT_UINT8Z FIT_SPORT_BITS_4;  // Bit field corresponding to sport enum
                                      // type (1 << (sport-32)).
...
typedef FIT_UINT8Z FIT_SPORT_BITS_5;  // Bit field corresponding to sport enum
                                      // type (1 << (sport-40)).
...
typedef FIT_UINT8Z FIT_SPORT_BITS_6;  // Bit field corresponding to sport enum
                                      // type (1 << (sport-48)).
...
#define FIT_SUB_SPORT_INDOOR_CYCLING \
  ((FIT_SUB_SPORT)6)                               // Cycling/Fitness Equipment
...
#define FIT_SUB_SPORT_INDOOR_WALKING \
  ((FIT_SUB_SPORT)27)  // Walking/Fitness Equipment
...
#define FIT_SUB_SPORT_BACKCOUNTRY \
  ((FIT_SUB_SPORT)37)                             // Alpine Skiing/Snowboarding
...
#define FIT_SUB_SPORT_OBSTACLE \
  ((FIT_SUB_SPORT)59)  // Used for events where participants run, crawl through
                       // mud, climb over walls, etc.
...
#define FIT_SUB_SPORT_HIIT \
  ((FIT_SUB_SPORT)70)  // High Intensity Interval Training
...
#define FIT_SESSION_TRIGGER_MANUAL \
  ((FIT_SESSION_TRIGGER)1)  // User changed sport.
#define FIT_SESSION_TRIGGER_AUTO_MULTI_SPORT \
  ((FIT_SESSION_TRIGGER)2)  // Auto multi-sport feature is enabled and user
                            // pressed lap button to advance session.
#define FIT_SESSION_TRIGGER_FITNESS_EQUIPMENT \
  ((FIT_SESSION_TRIGGER)3)  // Auto sport change caused by user linking to
                            // fitness equipment.
...
#define FIT_TIME_MODE_HOUR24 \
  ((FIT_TIME_MODE)1)  // Does not use a leading zero and has a colon
#define FIT_TIME_MODE_MILITARY \
  ((FIT_TIME_MODE)2)  // Uses a leading zero and does not have a colon
...
#define FIT_BACKLIGHT_MODE_KEY_AND_MESSAGES_AND_SMART_NOTIFICATIONS \
  ((FIT_BACKLIGHT_MODE)6)
...
#define FIT_BACKLIGHT_TIMEOUT_INFINITE \
  ((FIT_BACKLIGHT_TIMEOUT)0)  // Backlight stays on forever.
...
#define FIT_EVENT_WORKOUT_STEP \
  ((FIT_EVENT)4)  // Start at beginning of workout. Stop at end of each step.
...
#define FIT_EVENT_VIRTUAL_PARTNER_PACE \
  ((FIT_EVENT)12)  // Group 1. Start at beginning of activity if VP enabled,
                   // when VP pace is changed during activity or VP enabled mid
                   // activity. stop_disable when VP disabled.
#define FIT_EVENT_HR_HIGH_ALERT \
  ((FIT_EVENT)13)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_HR_LOW_ALERT \
  ((FIT_EVENT)14)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_SPEED_HIGH_ALERT \
  ((FIT_EVENT)15)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_SPEED_LOW_ALERT \
  ((FIT_EVENT)16)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_CAD_HIGH_ALERT \
  ((FIT_EVENT)17)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_CAD_LOW_ALERT \
  ((FIT_EVENT)18)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_POWER_HIGH_ALERT \
  ((FIT_EVENT)19)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_POWER_LOW_ALERT \
  ((FIT_EVENT)20)  // Group 0. Start / stop when in alert condition.
...
#define FIT_EVENT_TIME_DURATION_ALERT \
  ((FIT_EVENT)23)  // Group 1. Start if enabled mid activity (not required at
                   // start of activity). Stop when duration is reached.
                   // stop_disable if disabled.
#define FIT_EVENT_DISTANCE_DURATION_ALERT \
  ((FIT_EVENT)24)  // Group 1. Start if enabled mid activity (not required at
                   // start of activity). Stop when duration is reached.
                   // stop_disable if disabled.
#define FIT_EVENT_CALORIE_DURATION_ALERT \
  ((FIT_EVENT)25)  // Group 1. Start if enabled mid activity (not required at
                   // start of activity). Stop when duration is reached.
                   // stop_disable if disabled.
#define FIT_EVENT_ACTIVITY \
  ((FIT_EVENT)26)  // Group 1.. Stop at end of activity.
...
#define FIT_EVENT_ELEV_HIGH_ALERT \
  ((FIT_EVENT)45)  // Group 0. Start / stop when in alert condition.
#define FIT_EVENT_ELEV_LOW_ALERT \
  ((FIT_EVENT)46)  // Group 0. Start / stop when in alert condition.
...
#define FIT_EVENT_TANK_POD_CONNECTED \
  ((FIT_EVENT)81)  // marker - tank pod has connected
#define FIT_EVENT_TANK_POD_DISCONNECTED \
  ((FIT_EVENT)82)  // marker - tank pod has lost connection
...
#define FIT_FITNESS_EQUIPMENT_STATE_UNKNOWN \
  ((FIT_FITNESS_EQUIPMENT_STATE)3)  // lost connection to fitness equipment
...
#define FIT_WKT_STEP_DURATION_REPEAT_UNTIL_POWER_LAST_LAP_LESS_THAN \
  ((FIT_WKT_STEP_DURATION)17)
#define FIT_WKT_STEP_DURATION_REPEAT_UNTIL_MAX_POWER_LAST_LAP_LESS_THAN \
  ((FIT_WKT_STEP_DURATION)18)
...
#define FIT_WKT_STEP_DURATION_REPEAT_UNTIL_TRAINING_PEAKS_TSS \
  ((FIT_WKT_STEP_DURATION)27)
...
#define FIT_GOAL_SOURCE_COMMUNITY \
  ((FIT_GOAL_SOURCE)1)  // Social network sourced goal
...
#define FIT_COURSE_POINT_GENERAL_DISTANCE \
  ((FIT_COURSE_POINT)30)  // Used with UpAhead
...
#define FIT_MANUFACTURER_GARMIN_FR405_ANTFS \
  ((FIT_MANUFACTURER)2)  // Do not use. Used by FR405 for ANTFS man id.
...
#define FIT_MANUFACTURER_LIMITS_TECHNOLOGY \
  ((FIT_MANUFACTURER)103)  // Limits Technology Ltd.
#define FIT_MANUFACTURER_TOPACTION_TECHNOLOGY \
  ((FIT_MANUFACTURER)104)  // TopAction Technology Inc.
...
#define FIT_MANUFACTURER_NAVMAN \
  ((FIT_MANUFACTURER)269)  // MiTAC Global Corporation (Mio Technology)
...
#define FIT_MANUFACTURER_FALCO_E_MOTORS \
  ((FIT_MANUFACTURER)277)  // Falco eMotors Inc.
...
#define FIT_MANUFACTURER_LSEC \
  ((FIT_MANUFACTURER)320)  // Lishun Electric & Communication
...
#define FIT_GARMIN_PRODUCT_BSM \
  ((FIT_GARMIN_PRODUCT)9)  // BSM model for ANT+ messaging
#define FIT_GARMIN_PRODUCT_BCM \
  ((FIT_GARMIN_PRODUCT)10)  // BCM model for ANT+ messaging
#define FIT_GARMIN_PRODUCT_AXS01 \
  ((FIT_GARMIN_PRODUCT)11)  // AXS01 HRM Bike Chipset model for ANT+ messaging
...
#define FIT_GARMIN_PRODUCT_FR225_SINGLE_BYTE_PRODUCT_ID \
  ((FIT_GARMIN_PRODUCT)14)  // fr225 model for HRM ANT+ messaging
...
#define FIT_GARMIN_PRODUCT_OHR \
  ((FIT_GARMIN_PRODUCT)255)  // Garmin Wearable Optical Heart Rate Sensor for
                             // ANT+ HR Profile Broadcasting
...
#define FIT_GARMIN_PRODUCT_EDGE_TOURING \
  ((FIT_GARMIN_PRODUCT)1736)  // Also Edge Touring Plus
...
#define FIT_GARMIN_PRODUCT_VARIA_UT800 \
  ((FIT_GARMIN_PRODUCT)2567)  // Varia UT 800 SW
...
#define FIT_GARMIN_PRODUCT_GEN3_BSM \
  ((FIT_GARMIN_PRODUCT)3192)  // gen3 bike speed sensor
#define FIT_GARMIN_PRODUCT_GEN3_BCM \
  ((FIT_GARMIN_PRODUCT)3193)  // gen3 bike cadence sensor
...
#define FIT_GARMIN_PRODUCT_RALLY_200 \
  ((FIT_GARMIN_PRODUCT)3578)  // Rally 100/200 Power Meter Series
...
#define FIT_GARMIN_PRODUCT_DESCENT_MK2_ASIA \
...
#define FIT_GARMIN_PRODUCT_MARQ_GEN2 \
  ((FIT_GARMIN_PRODUCT)4105)  // Adventurer, Athlete, Captain, Golfer
...
#define FIT_GARMIN_PRODUCT_TACX_NEO_SMART \
  ((FIT_GARMIN_PRODUCT)4265)  // Neo Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_NEO2_SMART \
  ((FIT_GARMIN_PRODUCT)4266)  // Neo 2 Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_NEO2_T_SMART \
  ((FIT_GARMIN_PRODUCT)4267)  // Neo 2T Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_NEO_SMART_BIKE \
  ((FIT_GARMIN_PRODUCT)4268)  // Neo Smart Bike, Tacx
#define FIT_GARMIN_PRODUCT_TACX_SATORI_SMART \
  ((FIT_GARMIN_PRODUCT)4269)  // Satori Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_FLOW_SMART \
  ((FIT_GARMIN_PRODUCT)4270)  // Flow Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_VORTEX_SMART \
  ((FIT_GARMIN_PRODUCT)4271)  // Vortex Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_BUSHIDO_SMART \
  ((FIT_GARMIN_PRODUCT)4272)  // Bushido Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_GENIUS_SMART \
  ((FIT_GARMIN_PRODUCT)4273)  // Genius Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_FLUX_FLUX_S_SMART \
  ((FIT_GARMIN_PRODUCT)4274)  // Flux/Flux S Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_FLUX2_SMART \
  ((FIT_GARMIN_PRODUCT)4275)  // Flux 2 Smart, Tacx
#define FIT_GARMIN_PRODUCT_TACX_MAGNUM \
  ((FIT_GARMIN_PRODUCT)4276)  // Magnum, Tacx
...
#define FIT_GARMIN_PRODUCT_TACX_TRAINING_APP_MAC_CATALYST \
  ((FIT_GARMIN_PRODUCT)20565)
...
#define FIT_GARMIN_PRODUCT_CONNECT \
  ((FIT_GARMIN_PRODUCT)65534)  // Garmin Connect website
...
#define FIT_ANTPLUS_DEVICE_TYPE_ENVIRONMENT_SENSOR_LEGACY \
  ((FIT_ANTPLUS_DEVICE_TYPE)12)
#define FIT_ANTPLUS_DEVICE_TYPE_MULTI_SPORT_SPEED_DISTANCE \
  ((FIT_ANTPLUS_DEVICE_TYPE)15)
...
#define FIT_ANTPLUS_DEVICE_TYPE_LIGHT_ELECTRIC_VEHICLE \
  ((FIT_ANTPLUS_DEVICE_TYPE)20)
...
#define FIT_ANTPLUS_DEVICE_TYPE_BIKE_SPEED_CADENCE \
  ((FIT_ANTPLUS_DEVICE_TYPE)121)
...
#define FIT_ANTPLUS_DEVICE_TYPE_STRIDE_SPEED_DISTANCE \
  ((FIT_ANTPLUS_DEVICE_TYPE)124)
...
#define FIT_WORKOUT_CAPABILITIES_FITNESS_EQUIPMENT \
  ((FIT_WORKOUT_CAPABILITIES)0x00000004)
#define FIT_WORKOUT_CAPABILITIES_FIRSTBEAT \
  ((FIT_WORKOUT_CAPABILITIES)0x00000008)
#define FIT_WORKOUT_CAPABILITIES_NEW_LEAF ((FIT_WORKOUT_CAPABILITIES)0x00000010)
#define FIT_WORKOUT_CAPABILITIES_TCX \
  ((FIT_WORKOUT_CAPABILITIES)0x00000020)  // For backwards compatibility. Watch
                                          // should add missing id fields then
                                          // clear flag.
#define FIT_WORKOUT_CAPABILITIES_SPEED \
  ((FIT_WORKOUT_CAPABILITIES)0x00000080)  // Speed source required for workout
                                          // step.
#define FIT_WORKOUT_CAPABILITIES_HEART_RATE \
  ((FIT_WORKOUT_CAPABILITIES)0x00000100)  // Heart rate source required for
                                          // workout step.
#define FIT_WORKOUT_CAPABILITIES_DISTANCE \
  ((FIT_WORKOUT_CAPABILITIES)0x00000200)  // Distance source required for
                                          // workout step.
#define FIT_WORKOUT_CAPABILITIES_CADENCE \
  ((FIT_WORKOUT_CAPABILITIES)0x00000400)  // Cadence source required for workout
                                          // step.
#define FIT_WORKOUT_CAPABILITIES_POWER \
  ((FIT_WORKOUT_CAPABILITIES)0x00000800)  // Power source required for workout
                                          // step.
#define FIT_WORKOUT_CAPABILITIES_GRADE \
  ((FIT_WORKOUT_CAPABILITIES)0x00001000)  // Grade source required for workout
                                          // step.
#define FIT_WORKOUT_CAPABILITIES_RESISTANCE \
  ((FIT_WORKOUT_CAPABILITIES)0x00002000)  // Resistance source required for
                                          // workout step.
#define FIT_WORKOUT_CAPABILITIES_PROTECTED \
  ((FIT_WORKOUT_CAPABILITIES)0x00004000)
...
#define FIT_COURSE_CAPABILITIES_AVIATION \
  ((FIT_COURSE_CAPABILITIES)0x00001000)  // Denote course files to be used as
                                         // flight plans
...
typedef FIT_UINT32 FIT_WORKOUT_HR;  // 0 - 100 indicates% of max hr; >100
                                    // indicates bpm (255 max) plus 100
...
typedef FIT_UINT32
    FIT_WORKOUT_POWER;  // 0 - 1000 indicates % of functional threshold power;
                        // >1000 indicates watts plus 1000.
...
#define FIT_SWIM_STROKE_IM \
  ((FIT_SWIM_STROKE)6)  // IM is a mixed interval containing the same number of
                        // lengths for each of: Butterfly, Backstroke,
                        // Breaststroke, Freestyle, swam in that order.
...
#define FIT_ACTIVITY_TYPE_TRANSITION \
  ((FIT_ACTIVITY_TYPE)3)  // Mulitsport transition
...
#define FIT_ACTIVITY_TYPE_ALL \
  ((FIT_ACTIVITY_TYPE)254)  // All is for goals only to include all sports.
...
#define FIT_ACTIVITY_SUBTYPE_INDOOR_CYCLING \
  ((FIT_ACTIVITY_SUBTYPE)6)                                           // Cycling
...
#define FIT_ACTIVITY_SUBTYPE_TRACK_CYCLING \
  ((FIT_ACTIVITY_SUBTYPE)13)  // Cycling
#define FIT_ACTIVITY_SUBTYPE_INDOOR_ROWING \
  ((FIT_ACTIVITY_SUBTYPE)14)  // Fitness Equipment
#define FIT_ACTIVITY_SUBTYPE_ELLIPTICAL \
  ((FIT_ACTIVITY_SUBTYPE)15)  // Fitness Equipment
#define FIT_ACTIVITY_SUBTYPE_STAIR_CLIMBING \
  ((FIT_ACTIVITY_SUBTYPE)16)  // Fitness Equipment
#define FIT_ACTIVITY_SUBTYPE_LAP_SWIMMING \
  ((FIT_ACTIVITY_SUBTYPE)17)                                        // Swimming
...
#define FIT_LEFT_RIGHT_BALANCE_MASK \
  ((FIT_LEFT_RIGHT_BALANCE)0x7F)  // % contribution
#define FIT_LEFT_RIGHT_BALANCE_RIGHT \
  ((FIT_LEFT_RIGHT_BALANCE)0x80)  // data corresponds to right if set, otherwise
                                  // unknown
...
#define FIT_LEFT_RIGHT_BALANCE_100_MASK \
  ((FIT_LEFT_RIGHT_BALANCE_100)0x3FFF)  // % contribution scaled by 100
#define FIT_LEFT_RIGHT_BALANCE_100_RIGHT \
  ((FIT_LEFT_RIGHT_BALANCE_100)0x8000)  // data corresponds to right if set,
                                        // otherwise unknown
...
#define FIT_LENGTH_TYPE_IDLE \
  ((FIT_LENGTH_TYPE)0)  // Rest period. Length with no strokes
...
#define FIT_CONNECTIVITY_CAPABILITIES_BLUETOOTH \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000001)
#define FIT_CONNECTIVITY_CAPABILITIES_BLUETOOTH_LE \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000002)
#define FIT_CONNECTIVITY_CAPABILITIES_ANT \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000004)
#define FIT_CONNECTIVITY_CAPABILITIES_ACTIVITY_UPLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000008)
#define FIT_CONNECTIVITY_CAPABILITIES_COURSE_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000010)
#define FIT_CONNECTIVITY_CAPABILITIES_WORKOUT_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000020)
#define FIT_CONNECTIVITY_CAPABILITIES_LIVE_TRACK \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000040)
#define FIT_CONNECTIVITY_CAPABILITIES_WEATHER_CONDITIONS \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000080)
#define FIT_CONNECTIVITY_CAPABILITIES_WEATHER_ALERTS \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000100)
#define FIT_CONNECTIVITY_CAPABILITIES_GPS_EPHEMERIS_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000200)
#define FIT_CONNECTIVITY_CAPABILITIES_EXPLICIT_ARCHIVE \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000400)
#define FIT_CONNECTIVITY_CAPABILITIES_SETUP_INCOMPLETE \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00000800)
#define FIT_CONNECTIVITY_CAPABILITIES_CONTINUE_SYNC_AFTER_SOFTWARE_UPDATE \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00001000)
#define FIT_CONNECTIVITY_CAPABILITIES_CONNECT_IQ_APP_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00002000)
#define FIT_CONNECTIVITY_CAPABILITIES_GOLF_COURSE_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00004000)
#define FIT_CONNECTIVITY_CAPABILITIES_DEVICE_INITIATES_SYNC \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00008000)  // Indicates device is in control
                                               // of initiating all syncs
#define FIT_CONNECTIVITY_CAPABILITIES_CONNECT_IQ_WATCH_APP_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00010000)
#define FIT_CONNECTIVITY_CAPABILITIES_CONNECT_IQ_WIDGET_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00020000)
#define FIT_CONNECTIVITY_CAPABILITIES_CONNECT_IQ_WATCH_FACE_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00040000)
#define FIT_CONNECTIVITY_CAPABILITIES_CONNECT_IQ_DATA_FIELD_DOWNLOAD \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00080000)
#define FIT_CONNECTIVITY_CAPABILITIES_CONNECT_IQ_APP_MANAGMENT \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00100000)  // Device supports delete and
                                               // reorder of apps via GCM
#define FIT_CONNECTIVITY_CAPABILITIES_SWING_SENSOR \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00200000)
#define FIT_CONNECTIVITY_CAPABILITIES_SWING_SENSOR_REMOTE \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00400000)
#define FIT_CONNECTIVITY_CAPABILITIES_INCIDENT_DETECTION \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x00800000)  // Device supports incident
                                               // detection
#define FIT_CONNECTIVITY_CAPABILITIES_AUDIO_PROMPTS \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x01000000)
#define FIT_CONNECTIVITY_CAPABILITIES_WIFI_VERIFICATION \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x02000000)  // Device supports reporting wifi
                                               // verification via GCM
#define FIT_CONNECTIVITY_CAPABILITIES_TRUE_UP \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x04000000)  // Device supports True Up
#define FIT_CONNECTIVITY_CAPABILITIES_FIND_MY_WATCH \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x08000000)  // Device supports Find My Watch
#define FIT_CONNECTIVITY_CAPABILITIES_REMOTE_MANUAL_SYNC \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x10000000)
#define FIT_CONNECTIVITY_CAPABILITIES_LIVE_TRACK_AUTO_START \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x20000000)  // Device supports LiveTrack auto
                                               // start
#define FIT_CONNECTIVITY_CAPABILITIES_LIVE_TRACK_MESSAGING \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x40000000)  // Device supports LiveTrack
                                               // Messaging
#define FIT_CONNECTIVITY_CAPABILITIES_INSTANT_INPUT \
  ((FIT_CONNECTIVITY_CAPABILITIES)0x80000000)  // Device supports instant input
                                               // feature
...
#define FIT_WEATHER_REPORT_FORECAST \
  ((FIT_WEATHER_REPORT)1)  // Deprecated use hourly_forecast instead
...
#define FIT_WEATHER_SEVERE_TYPE_HURRICANE_FORCE_WIND \
  ((FIT_WEATHER_SEVERE_TYPE)7)
...
#define FIT_WEATHER_SEVERE_TYPE_INLAND_TROPICAL_STORM \
  ((FIT_WEATHER_SEVERE_TYPE)15)
...
#define FIT_WEATHER_SEVERE_TYPE_HEAVY_FREEZING_SPRAY \
  ((FIT_WEATHER_SEVERE_TYPE)24)
...
#define FIT_WEATHER_SEVERE_TYPE_LAKE_EFFECT_BLOWING_SNOW \
  ((FIT_WEATHER_SEVERE_TYPE)29)
...
#define FIT_WEATHER_SEVERE_TYPE_SNOW_AND_BLOWING_SNOW \
  ((FIT_WEATHER_SEVERE_TYPE)35)
...
#define FIT_WEATHER_SEVERE_TYPE_HIGH_HEAT_AND_HUMIDITY \
  ((FIT_WEATHER_SEVERE_TYPE)49)
...
#define FIT_WEATHER_SEVERE_TYPE_SMALL_CRAFT_HAZARDOUS_SEAS \
  ((FIT_WEATHER_SEVERE_TYPE)60)
...
#define FIT_WEATHER_SEVERE_TYPE_SMALL_CRAFT_ROUGH_BAR \
  ((FIT_WEATHER_SEVERE_TYPE)64)
...
typedef FIT_UINT32
    FIT_TIME_INTO_DAY;  // number of seconds into the day since 00:00:00 UTC
#define FIT_TIME_INTO_DAY_INVALID FIT_UINT32_INVALID
#define FIT_TIME_INTO_DAY_COUNT 0

typedef FIT_UINT32 FIT_LOCALTIME_INTO_DAY;  // number of seconds into the day
                                            // since local 00:00:00
...
#define FIT_STROKE_TYPE_OTHER \
  ((FIT_STROKE_TYPE)1)  // stroke was detected but cannot be identified
...
#define FIT_BODY_LOCATION_LEFT_BRACHIORADIALIS \
  ((FIT_BODY_LOCATION)26)  // Left anterior forearm
#define FIT_BODY_LOCATION_LEFT_FOREARM_EXTENSORS \
  ((FIT_BODY_LOCATION)27)  // Left posterior forearm
...
#define FIT_BODY_LOCATION_RIGHT_BRACHIORADIALIS \
  ((FIT_BODY_LOCATION)32)  // Right anterior forearm
#define FIT_BODY_LOCATION_RIGHT_FOREARM_EXTENSORS \
  ((FIT_BODY_LOCATION)33)  // Right posterior forearm
...
#define FIT_SEGMENT_LEADERBOARD_TYPE_PERSONAL_BEST \
  ((FIT_SEGMENT_LEADERBOARD_TYPE)1)
#define FIT_SEGMENT_LEADERBOARD_TYPE_CONNECTIONS \
  ((FIT_SEGMENT_LEADERBOARD_TYPE)2)
#define FIT_SEGMENT_LEADERBOARD_TYPE_GROUP ((FIT_SEGMENT_LEADERBOARD_TYPE)3)
#define FIT_SEGMENT_LEADERBOARD_TYPE_CHALLENGER \
  ((FIT_SEGMENT_LEADERBOARD_TYPE)4)
...
#define FIT_SEGMENT_LEADERBOARD_TYPE_CLUB_LEADER \
  ((FIT_SEGMENT_LEADERBOARD_TYPE)10)
...
#define FIT_SOURCE_TYPE_ANT \
  ((FIT_SOURCE_TYPE)0)  // External device connected with ANT
#define FIT_SOURCE_TYPE_ANTPLUS \
  ((FIT_SOURCE_TYPE)1)  // External device connected with ANT+
#define FIT_SOURCE_TYPE_BLUETOOTH \
  ((FIT_SOURCE_TYPE)2)  // External device connected with BT
#define FIT_SOURCE_TYPE_BLUETOOTH_LOW_ENERGY \
  ((FIT_SOURCE_TYPE)3)  // External device connected with BLE
#define FIT_SOURCE_TYPE_WIFI \
  ((FIT_SOURCE_TYPE)4)  // External device connected with Wifi
...
#define FIT_LOCAL_DEVICE_TYPE_GPS \
  ((FIT_LOCAL_DEVICE_TYPE)0)  // Onboard gps receiver
#define FIT_LOCAL_DEVICE_TYPE_GLONASS \
  ((FIT_LOCAL_DEVICE_TYPE)1)  // Onboard glonass receiver
#define FIT_LOCAL_DEVICE_TYPE_GPS_GLONASS \
  ((FIT_LOCAL_DEVICE_TYPE)2)  // Onboard gps glonass receiver
#define FIT_LOCAL_DEVICE_TYPE_ACCELEROMETER \
  ((FIT_LOCAL_DEVICE_TYPE)3)  // Onboard sensor
#define FIT_LOCAL_DEVICE_TYPE_BAROMETER \
  ((FIT_LOCAL_DEVICE_TYPE)4)  // Onboard sensor
#define FIT_LOCAL_DEVICE_TYPE_TEMPERATURE \
  ((FIT_LOCAL_DEVICE_TYPE)5)  // Onboard sensor
#define FIT_LOCAL_DEVICE_TYPE_WHR \
  ((FIT_LOCAL_DEVICE_TYPE)10)  // Onboard wrist HR sensor
#define FIT_LOCAL_DEVICE_TYPE_SENSOR_HUB \
  ((FIT_LOCAL_DEVICE_TYPE)12)  // Onboard software package
...
#define FIT_BLE_DEVICE_TYPE_CONNECTED_GPS \
  ((FIT_BLE_DEVICE_TYPE)0)  // GPS that is provided over a proprietary bluetooth
                            // service
...
#define FIT_BLE_DEVICE_TYPE_BIKE_TRAINER \
  ((FIT_BLE_DEVICE_TYPE)7)  // Indoor-Bike FTMS protocol
...
#define FIT_ANT_CHANNEL_ID_ANT_EXTENDED_DEVICE_NUMBER_UPPER_NIBBLE \
  ((FIT_ANT_CHANNEL_ID)0xF0000000)
#define FIT_ANT_CHANNEL_ID_ANT_TRANSMISSION_TYPE_LOWER_NIBBLE \
  ((FIT_ANT_CHANNEL_ID)0x0F000000)
...
#define FIT_DISPLAY_ORIENTATION_AUTO \
  ((FIT_DISPLAY_ORIENTATION)0)  // automatic if the device supports it
...
#define FIT_DISPLAY_ORIENTATION_PORTRAIT_FLIPPED \
  ((FIT_DISPLAY_ORIENTATION)3)  // portrait mode but rotated 180 degrees
#define FIT_DISPLAY_ORIENTATION_LANDSCAPE_FLIPPED \
  ((FIT_DISPLAY_ORIENTATION)4)  // landscape mode but rotated 180 degrees
...
#define FIT_DIGITAL_WATCHFACE_LAYOUT_TRADITIONAL \
  ((FIT_DIGITAL_WATCHFACE_LAYOUT)0)
...
#define FIT_RIDER_POSITION_TYPE_TRANSITION_TO_SEATED \
  ((FIT_RIDER_POSITION_TYPE)2)
#define FIT_RIDER_POSITION_TYPE_TRANSITION_TO_STANDING \
  ((FIT_RIDER_POSITION_TYPE)3)
...
#define FIT_CAMERA_EVENT_TYPE_VIDEO_START \
  ((FIT_CAMERA_EVENT_TYPE)0)  // Start of video recording
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SPLIT \
  ((FIT_CAMERA_EVENT_TYPE)1)  // Mark of video file split (end of one file,
                              // beginning of the other)
#define FIT_CAMERA_EVENT_TYPE_VIDEO_END \
  ((FIT_CAMERA_EVENT_TYPE)2)  // End of video recording
#define FIT_CAMERA_EVENT_TYPE_PHOTO_TAKEN \
  ((FIT_CAMERA_EVENT_TYPE)3)  // Still photo taken
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SECOND_STREAM_START \
  ((FIT_CAMERA_EVENT_TYPE)4)
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SECOND_STREAM_SPLIT \
  ((FIT_CAMERA_EVENT_TYPE)5)
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SECOND_STREAM_END ((FIT_CAMERA_EVENT_TYPE)6)
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SPLIT_START \
  ((FIT_CAMERA_EVENT_TYPE)7)  // Mark of video file split start
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SECOND_STREAM_SPLIT_START \
  ((FIT_CAMERA_EVENT_TYPE)8)
#define FIT_CAMERA_EVENT_TYPE_VIDEO_PAUSE \
  ((FIT_CAMERA_EVENT_TYPE)11)  // Mark when a video recording has been paused
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SECOND_STREAM_PAUSE \
  ((FIT_CAMERA_EVENT_TYPE)12)
#define FIT_CAMERA_EVENT_TYPE_VIDEO_RESUME \
  ((FIT_CAMERA_EVENT_TYPE)13)  // Mark when a video recording has been resumed
#define FIT_CAMERA_EVENT_TYPE_VIDEO_SECOND_STREAM_RESUME \
  ((FIT_CAMERA_EVENT_TYPE)14)
...
#define FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE_AUTO \
  ((FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE)0)
#define FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE_INDIVIDUAL \
  ((FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE)4)
#define FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE_HIGH_VISIBILITY \
  ((FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE)5)
#define FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE_TRAIL \
  ((FIT_BIKE_LIGHT_NETWORK_CONFIG_TYPE)6)
...
#define FIT_COMM_TIMEOUT_TYPE_WILDCARD_PAIRING_TIMEOUT \
  ((FIT_COMM_TIMEOUT_TYPE)0)  // Timeout pairing to any device
#define FIT_COMM_TIMEOUT_TYPE_PAIRING_TIMEOUT \
  ((FIT_COMM_TIMEOUT_TYPE)1)  // Timeout pairing to previously paired device
#define FIT_COMM_TIMEOUT_TYPE_CONNECTION_LOST \
  ((FIT_COMM_TIMEOUT_TYPE)2)  // Temporary loss of communications
#define FIT_COMM_TIMEOUT_TYPE_CONNECTION_TIMEOUT \
  ((FIT_COMM_TIMEOUT_TYPE)3)  // Connection closed due to extended bad
                              // communications
...
#define FIT_CAMERA_ORIENTATION_TYPE_CAMERA_ORIENTATION_0 \
  ((FIT_CAMERA_ORIENTATION_TYPE)0)
#define FIT_CAMERA_ORIENTATION_TYPE_CAMERA_ORIENTATION_90 \
  ((FIT_CAMERA_ORIENTATION_TYPE)1)
#define FIT_CAMERA_ORIENTATION_TYPE_CAMERA_ORIENTATION_180 \
  ((FIT_CAMERA_ORIENTATION_TYPE)2)
#define FIT_CAMERA_ORIENTATION_TYPE_CAMERA_ORIENTATION_270 \
  ((FIT_CAMERA_ORIENTATION_TYPE)3)
...
#define FIT_ATTITUDE_VALIDITY_TRACK_ANGLE_HEADING_VALID \
  ((FIT_ATTITUDE_VALIDITY)0x0001)
...
#define FIT_ATTITUDE_VALIDITY_LATERAL_BODY_ACCEL_VALID \
  ((FIT_ATTITUDE_VALIDITY)0x0008)
#define FIT_ATTITUDE_VALIDITY_NORMAL_BODY_ACCEL_VALID \
  ((FIT_ATTITUDE_VALIDITY)0x0010)
...
#define FIT_EXD_LAYOUT_DYNAMIC \
  ((FIT_EXD_LAYOUT)8)  // The EXD may display the configured concepts in any
                       // layout it sees fit.
...
#define FIT_EXD_DATA_UNITS_ENUM_BIKE_LIGHT_BEAM_ANGLE_MODE \
  ((FIT_EXD_DATA_UNITS)26)
#define FIT_EXD_DATA_UNITS_ENUM_BIKE_LIGHT_BATTERY_STATUS \
  ((FIT_EXD_DATA_UNITS)27)
#define FIT_EXD_DATA_UNITS_ENUM_BIKE_LIGHT_NETWORK_CONFIG_TYPE \
  ((FIT_EXD_DATA_UNITS)28)
...
#define FIT_EXD_DESCRIPTORS_COURSE_ESTIMATED_TIME_OF_ARRIVAL \
  ((FIT_EXD_DESCRIPTORS)49)
#define FIT_EXD_DESCRIPTORS_NAVIGATION_ESTIMATED_TIME_OF_ARRIVAL \
  ((FIT_EXD_DESCRIPTORS)50)
...
#define FIT_EXD_DESCRIPTORS_LEFT_PLATFORM_CENTER_OFFSET \
  ((FIT_EXD_DESCRIPTORS)59)
#define FIT_EXD_DESCRIPTORS_RIGHT_PLATFORM_CENTER_OFFSET \
  ((FIT_EXD_DESCRIPTORS)60)
#define FIT_EXD_DESCRIPTORS_LEFT_POWER_PHASE_START_ANGLE \
  ((FIT_EXD_DESCRIPTORS)61)
#define FIT_EXD_DESCRIPTORS_RIGHT_POWER_PHASE_START_ANGLE \
  ((FIT_EXD_DESCRIPTORS)62)
#define FIT_EXD_DESCRIPTORS_LEFT_POWER_PHASE_FINISH_ANGLE \
  ((FIT_EXD_DESCRIPTORS)63)
#define FIT_EXD_DESCRIPTORS_RIGHT_POWER_PHASE_FINISH_ANGLE \
  ((FIT_EXD_DESCRIPTORS)64)
#define FIT_EXD_DESCRIPTORS_GEARS \
  ((FIT_EXD_DESCRIPTORS)65)  // Combined gear information
...
#define FIT_EXD_DESCRIPTORS_LEFT_GROUND_CONTACT_TIME_BALANCE \
  ((FIT_EXD_DESCRIPTORS)71)
#define FIT_EXD_DESCRIPTORS_RIGHT_GROUND_CONTACT_TIME_BALANCE \
  ((FIT_EXD_DESCRIPTORS)72)
...
#define FIT_AUTO_ACTIVITY_DETECT_ELLIPTICAL \
  ((FIT_AUTO_ACTIVITY_DETECT)0x00000020)
#define FIT_AUTO_ACTIVITY_DETECT_SEDENTARY \
  ((FIT_AUTO_ACTIVITY_DETECT)0x00000400)
...
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_FULL_SCREEN \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000001)
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_HALF_VERTICAL \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000002)
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_HALF_HORIZONTAL \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000004)
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_HALF_VERTICAL_RIGHT_SPLIT \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000008)
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_HALF_HORIZONTAL_BOTTOM_SPLIT \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000010)
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_FULL_QUARTER_SPLIT \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000020)
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_HALF_VERTICAL_LEFT_SPLIT \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000040)
#define FIT_SUPPORTED_EXD_SCREEN_LAYOUTS_HALF_HORIZONTAL_TOP_SPLIT \
  ((FIT_SUPPORTED_EXD_SCREEN_LAYOUTS)0x00000080)
...
#define FIT_BIKE_LIGHT_BEAM_ANGLE_MODE_MANUAL \
  ((FIT_BIKE_LIGHT_BEAM_ANGLE_MODE)0)
...
#define FIT_BENCH_PRESS_EXERCISE_NAME_ALTERNATING_DUMBBELL_CHEST_PRESS_ON_SWISS_BALL \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)0)
#define FIT_BENCH_PRESS_EXERCISE_NAME_BARBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)1)
#define FIT_BENCH_PRESS_EXERCISE_NAME_BARBELL_BOARD_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)2)
#define FIT_BENCH_PRESS_EXERCISE_NAME_BARBELL_FLOOR_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)3)
#define FIT_BENCH_PRESS_EXERCISE_NAME_CLOSE_GRIP_BARBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)4)
#define FIT_BENCH_PRESS_EXERCISE_NAME_DECLINE_DUMBBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)5)
#define FIT_BENCH_PRESS_EXERCISE_NAME_DUMBBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)6)
#define FIT_BENCH_PRESS_EXERCISE_NAME_DUMBBELL_FLOOR_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)7)
#define FIT_BENCH_PRESS_EXERCISE_NAME_INCLINE_BARBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)8)
#define FIT_BENCH_PRESS_EXERCISE_NAME_INCLINE_DUMBBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)9)
#define FIT_BENCH_PRESS_EXERCISE_NAME_INCLINE_SMITH_MACHINE_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)10)
#define FIT_BENCH_PRESS_EXERCISE_NAME_ISOMETRIC_BARBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)11)
#define FIT_BENCH_PRESS_EXERCISE_NAME_KETTLEBELL_CHEST_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)12)
#define FIT_BENCH_PRESS_EXERCISE_NAME_NEUTRAL_GRIP_DUMBBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)13)
#define FIT_BENCH_PRESS_EXERCISE_NAME_NEUTRAL_GRIP_DUMBBELL_INCLINE_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)14)
#define FIT_BENCH_PRESS_EXERCISE_NAME_ONE_ARM_FLOOR_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)15)
#define FIT_BENCH_PRESS_EXERCISE_NAME_WEIGHTED_ONE_ARM_FLOOR_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)16)
#define FIT_BENCH_PRESS_EXERCISE_NAME_PARTIAL_LOCKOUT \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)17)
#define FIT_BENCH_PRESS_EXERCISE_NAME_REVERSE_GRIP_BARBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)18)
#define FIT_BENCH_PRESS_EXERCISE_NAME_REVERSE_GRIP_INCLINE_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)19)
#define FIT_BENCH_PRESS_EXERCISE_NAME_SINGLE_ARM_CABLE_CHEST_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)20)
#define FIT_BENCH_PRESS_EXERCISE_NAME_SINGLE_ARM_DUMBBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)21)
#define FIT_BENCH_PRESS_EXERCISE_NAME_SMITH_MACHINE_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)22)
#define FIT_BENCH_PRESS_EXERCISE_NAME_SWISS_BALL_DUMBBELL_CHEST_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)23)
#define FIT_BENCH_PRESS_EXERCISE_NAME_TRIPLE_STOP_BARBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)24)
#define FIT_BENCH_PRESS_EXERCISE_NAME_WIDE_GRIP_BARBELL_BENCH_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)25)
#define FIT_BENCH_PRESS_EXERCISE_NAME_ALTERNATING_DUMBBELL_CHEST_PRESS \
  ((FIT_BENCH_PRESS_EXERCISE_NAME)26)
...
#define FIT_CALF_RAISE_EXERCISE_NAME_3_WAY_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)0)
#define FIT_CALF_RAISE_EXERCISE_NAME_3_WAY_WEIGHTED_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)1)
#define FIT_CALF_RAISE_EXERCISE_NAME_3_WAY_SINGLE_LEG_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)2)
#define FIT_CALF_RAISE_EXERCISE_NAME_3_WAY_WEIGHTED_SINGLE_LEG_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)3)
#define FIT_CALF_RAISE_EXERCISE_NAME_DONKEY_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)4)
#define FIT_CALF_RAISE_EXERCISE_NAME_WEIGHTED_DONKEY_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)5)
#define FIT_CALF_RAISE_EXERCISE_NAME_SEATED_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)6)
#define FIT_CALF_RAISE_EXERCISE_NAME_WEIGHTED_SEATED_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)7)
#define FIT_CALF_RAISE_EXERCISE_NAME_SEATED_DUMBBELL_TOE_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)8)
#define FIT_CALF_RAISE_EXERCISE_NAME_SINGLE_LEG_BENT_KNEE_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)9)
#define FIT_CALF_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_BENT_KNEE_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)10)
#define FIT_CALF_RAISE_EXERCISE_NAME_SINGLE_LEG_DECLINE_PUSH_UP \
  ((FIT_CALF_RAISE_EXERCISE_NAME)11)
#define FIT_CALF_RAISE_EXERCISE_NAME_SINGLE_LEG_DONKEY_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)12)
#define FIT_CALF_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_DONKEY_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)13)
#define FIT_CALF_RAISE_EXERCISE_NAME_SINGLE_LEG_HIP_RAISE_WITH_KNEE_HOLD \
  ((FIT_CALF_RAISE_EXERCISE_NAME)14)
#define FIT_CALF_RAISE_EXERCISE_NAME_SINGLE_LEG_STANDING_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)15)
#define FIT_CALF_RAISE_EXERCISE_NAME_SINGLE_LEG_STANDING_DUMBBELL_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)16)
#define FIT_CALF_RAISE_EXERCISE_NAME_STANDING_BARBELL_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)17)
#define FIT_CALF_RAISE_EXERCISE_NAME_STANDING_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)18)
#define FIT_CALF_RAISE_EXERCISE_NAME_WEIGHTED_STANDING_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)19)
#define FIT_CALF_RAISE_EXERCISE_NAME_STANDING_DUMBBELL_CALF_RAISE \
  ((FIT_CALF_RAISE_EXERCISE_NAME)20)
...
#define FIT_CARDIO_EXERCISE_NAME_BOB_AND_WEAVE_CIRCLE \
  ((FIT_CARDIO_EXERCISE_NAME)0)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_BOB_AND_WEAVE_CIRCLE \
  ((FIT_CARDIO_EXERCISE_NAME)1)
#define FIT_CARDIO_EXERCISE_NAME_CARDIO_CORE_CRAWL ((FIT_CARDIO_EXERCISE_NAME)2)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_CARDIO_CORE_CRAWL \
  ((FIT_CARDIO_EXERCISE_NAME)3)
#define FIT_CARDIO_EXERCISE_NAME_DOUBLE_UNDER ((FIT_CARDIO_EXERCISE_NAME)4)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_DOUBLE_UNDER \
  ((FIT_CARDIO_EXERCISE_NAME)5)
#define FIT_CARDIO_EXERCISE_NAME_JUMP_ROPE ((FIT_CARDIO_EXERCISE_NAME)6)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_JUMP_ROPE \
  ((FIT_CARDIO_EXERCISE_NAME)7)
#define FIT_CARDIO_EXERCISE_NAME_JUMP_ROPE_CROSSOVER \
  ((FIT_CARDIO_EXERCISE_NAME)8)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_JUMP_ROPE_CROSSOVER \
  ((FIT_CARDIO_EXERCISE_NAME)9)
#define FIT_CARDIO_EXERCISE_NAME_JUMP_ROPE_JOG ((FIT_CARDIO_EXERCISE_NAME)10)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_JUMP_ROPE_JOG \
  ((FIT_CARDIO_EXERCISE_NAME)11)
#define FIT_CARDIO_EXERCISE_NAME_JUMPING_JACKS ((FIT_CARDIO_EXERCISE_NAME)12)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_JUMPING_JACKS \
  ((FIT_CARDIO_EXERCISE_NAME)13)
#define FIT_CARDIO_EXERCISE_NAME_SKI_MOGULS ((FIT_CARDIO_EXERCISE_NAME)14)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_SKI_MOGULS \
  ((FIT_CARDIO_EXERCISE_NAME)15)
#define FIT_CARDIO_EXERCISE_NAME_SPLIT_JACKS ((FIT_CARDIO_EXERCISE_NAME)16)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_SPLIT_JACKS \
  ((FIT_CARDIO_EXERCISE_NAME)17)
#define FIT_CARDIO_EXERCISE_NAME_SQUAT_JACKS ((FIT_CARDIO_EXERCISE_NAME)18)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_SQUAT_JACKS \
  ((FIT_CARDIO_EXERCISE_NAME)19)
#define FIT_CARDIO_EXERCISE_NAME_TRIPLE_UNDER ((FIT_CARDIO_EXERCISE_NAME)20)
#define FIT_CARDIO_EXERCISE_NAME_WEIGHTED_TRIPLE_UNDER \
  ((FIT_CARDIO_EXERCISE_NAME)21)
...
#define FIT_CARRY_EXERCISE_NAME_FARMERS_WALK_ON_TOES \
  ((FIT_CARRY_EXERCISE_NAME)2)
...
#define FIT_CHOP_EXERCISE_NAME_WEIGHTED_CROSS_CHOP_TO_KNEE \
  ((FIT_CHOP_EXERCISE_NAME)4)
#define FIT_CHOP_EXERCISE_NAME_DUMBBELL_CHOP ((FIT_CHOP_EXERCISE_NAME)5)
#define FIT_CHOP_EXERCISE_NAME_HALF_KNEELING_ROTATION \
  ((FIT_CHOP_EXERCISE_NAME)6)
#define FIT_CHOP_EXERCISE_NAME_WEIGHTED_HALF_KNEELING_ROTATION \
  ((FIT_CHOP_EXERCISE_NAME)7)
#define FIT_CHOP_EXERCISE_NAME_HALF_KNEELING_ROTATIONAL_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)8)
#define FIT_CHOP_EXERCISE_NAME_HALF_KNEELING_ROTATIONAL_REVERSE_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)9)
#define FIT_CHOP_EXERCISE_NAME_HALF_KNEELING_STABILITY_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)10)
#define FIT_CHOP_EXERCISE_NAME_HALF_KNEELING_STABILITY_REVERSE_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)11)
#define FIT_CHOP_EXERCISE_NAME_KNEELING_ROTATIONAL_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)12)
#define FIT_CHOP_EXERCISE_NAME_KNEELING_ROTATIONAL_REVERSE_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)13)
#define FIT_CHOP_EXERCISE_NAME_KNEELING_STABILITY_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)14)
#define FIT_CHOP_EXERCISE_NAME_KNEELING_WOODCHOPPER ((FIT_CHOP_EXERCISE_NAME)15)
#define FIT_CHOP_EXERCISE_NAME_MEDICINE_BALL_WOOD_CHOPS \
  ((FIT_CHOP_EXERCISE_NAME)16)
#define FIT_CHOP_EXERCISE_NAME_POWER_SQUAT_CHOPS ((FIT_CHOP_EXERCISE_NAME)17)
#define FIT_CHOP_EXERCISE_NAME_WEIGHTED_POWER_SQUAT_CHOPS \
  ((FIT_CHOP_EXERCISE_NAME)18)
#define FIT_CHOP_EXERCISE_NAME_STANDING_ROTATIONAL_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)19)
#define FIT_CHOP_EXERCISE_NAME_STANDING_SPLIT_ROTATIONAL_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)20)
#define FIT_CHOP_EXERCISE_NAME_STANDING_SPLIT_ROTATIONAL_REVERSE_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)21)
#define FIT_CHOP_EXERCISE_NAME_STANDING_STABILITY_REVERSE_CHOP \
  ((FIT_CHOP_EXERCISE_NAME)22)
...
#define FIT_CORE_EXERCISE_NAME_ALTERNATING_PLATE_REACH \
  ((FIT_CORE_EXERCISE_NAME)2)
#define FIT_CORE_EXERCISE_NAME_BARBELL_ROLLOUT ((FIT_CORE_EXERCISE_NAME)3)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_BARBELL_ROLLOUT \
  ((FIT_CORE_EXERCISE_NAME)4)
#define FIT_CORE_EXERCISE_NAME_BODY_BAR_OBLIQUE_TWIST \
  ((FIT_CORE_EXERCISE_NAME)5)
...
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_CRESCENT_CIRCLE \
  ((FIT_CORE_EXERCISE_NAME)11)
#define FIT_CORE_EXERCISE_NAME_CYCLING_RUSSIAN_TWIST \
  ((FIT_CORE_EXERCISE_NAME)12)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_CYCLING_RUSSIAN_TWIST \
  ((FIT_CORE_EXERCISE_NAME)13)
#define FIT_CORE_EXERCISE_NAME_ELEVATED_FEET_RUSSIAN_TWIST \
  ((FIT_CORE_EXERCISE_NAME)14)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_ELEVATED_FEET_RUSSIAN_TWIST \
  ((FIT_CORE_EXERCISE_NAME)15)
...
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_KNEELING_AB_WHEEL \
  ((FIT_CORE_EXERCISE_NAME)19)
...
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_OPEN_KNEE_TUCKS \
  ((FIT_CORE_EXERCISE_NAME)22)
#define FIT_CORE_EXERCISE_NAME_SIDE_ABS_LEG_LIFT ((FIT_CORE_EXERCISE_NAME)23)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_SIDE_ABS_LEG_LIFT \
  ((FIT_CORE_EXERCISE_NAME)24)
#define FIT_CORE_EXERCISE_NAME_SWISS_BALL_JACKKNIFE ((FIT_CORE_EXERCISE_NAME)25)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_SWISS_BALL_JACKKNIFE \
  ((FIT_CORE_EXERCISE_NAME)26)
#define FIT_CORE_EXERCISE_NAME_SWISS_BALL_PIKE ((FIT_CORE_EXERCISE_NAME)27)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PIKE \
  ((FIT_CORE_EXERCISE_NAME)28)
#define FIT_CORE_EXERCISE_NAME_SWISS_BALL_ROLLOUT ((FIT_CORE_EXERCISE_NAME)29)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_SWISS_BALL_ROLLOUT \
  ((FIT_CORE_EXERCISE_NAME)30)
#define FIT_CORE_EXERCISE_NAME_TRIANGLE_HIP_PRESS ((FIT_CORE_EXERCISE_NAME)31)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_TRIANGLE_HIP_PRESS \
  ((FIT_CORE_EXERCISE_NAME)32)
#define FIT_CORE_EXERCISE_NAME_TRX_SUSPENDED_JACKKNIFE \
  ((FIT_CORE_EXERCISE_NAME)33)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_TRX_SUSPENDED_JACKKNIFE \
  ((FIT_CORE_EXERCISE_NAME)34)
...
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_WINDMILL_SWITCHES \
  ((FIT_CORE_EXERCISE_NAME)38)
#define FIT_CORE_EXERCISE_NAME_ALTERNATING_SLIDE_OUT \
  ((FIT_CORE_EXERCISE_NAME)39)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_ALTERNATING_SLIDE_OUT \
  ((FIT_CORE_EXERCISE_NAME)40)
#define FIT_CORE_EXERCISE_NAME_GHD_BACK_EXTENSIONS ((FIT_CORE_EXERCISE_NAME)41)
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_GHD_BACK_EXTENSIONS \
  ((FIT_CORE_EXERCISE_NAME)42)
...
#define FIT_CORE_EXERCISE_NAME_WEIGHTED_MODIFIED_FRONT_LEVER \
  ((FIT_CORE_EXERCISE_NAME)45)
#define FIT_CORE_EXERCISE_NAME_RUSSIAN_TWIST ((FIT_CORE_EXERCISE_NAME)46)
#define FIT_CORE_EXERCISE_NAME_ABDOMINAL_LEG_ROTATIONS \
  ((FIT_CORE_EXERCISE_NAME)47)  // Deprecated do not use
#define FIT_CORE_EXERCISE_NAME_ARM_AND_LEG_EXTENSION_ON_KNEES \
  ((FIT_CORE_EXERCISE_NAME)48)
#define FIT_CORE_EXERCISE_NAME_BICYCLE ((FIT_CORE_EXERCISE_NAME)49)
#define FIT_CORE_EXERCISE_NAME_BICEP_CURL_WITH_LEG_EXTENSION \
  ((FIT_CORE_EXERCISE_NAME)50)
...
#define FIT_CORE_EXERCISE_NAME_CRISS_CROSS_WITH_BALL \
  ((FIT_CORE_EXERCISE_NAME)54)  // Deprecated do not use
...
#define FIT_CORE_EXERCISE_NAME_SNAKE_TWIST_1_AND_2 \
  ((FIT_CORE_EXERCISE_NAME)68)  // Deprecated do not use
...
#define FIT_CRUNCH_EXERCISE_NAME_CIRCULAR_ARM_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)2)
#define FIT_CRUNCH_EXERCISE_NAME_CROSSED_ARMS_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)3)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_CROSSED_ARMS_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)4)
#define FIT_CRUNCH_EXERCISE_NAME_CROSS_LEG_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)5)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_CROSS_LEG_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)6)
#define FIT_CRUNCH_EXERCISE_NAME_CRUNCH_CHOP ((FIT_CRUNCH_EXERCISE_NAME)7)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_CRUNCH_CHOP \
  ((FIT_CRUNCH_EXERCISE_NAME)8)
#define FIT_CRUNCH_EXERCISE_NAME_DOUBLE_CRUNCH ((FIT_CRUNCH_EXERCISE_NAME)9)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_DOUBLE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)10)
#define FIT_CRUNCH_EXERCISE_NAME_ELBOW_TO_KNEE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)11)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_ELBOW_TO_KNEE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)12)
#define FIT_CRUNCH_EXERCISE_NAME_FLUTTER_KICKS ((FIT_CRUNCH_EXERCISE_NAME)13)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_FLUTTER_KICKS \
  ((FIT_CRUNCH_EXERCISE_NAME)14)
#define FIT_CRUNCH_EXERCISE_NAME_FOAM_ROLLER_REVERSE_CRUNCH_ON_BENCH \
  ((FIT_CRUNCH_EXERCISE_NAME)15)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_FOAM_ROLLER_REVERSE_CRUNCH_ON_BENCH \
  ((FIT_CRUNCH_EXERCISE_NAME)16)
#define FIT_CRUNCH_EXERCISE_NAME_FOAM_ROLLER_REVERSE_CRUNCH_WITH_DUMBBELL \
  ((FIT_CRUNCH_EXERCISE_NAME)17)
#define FIT_CRUNCH_EXERCISE_NAME_FOAM_ROLLER_REVERSE_CRUNCH_WITH_MEDICINE_BALL \
  ((FIT_CRUNCH_EXERCISE_NAME)18)
#define FIT_CRUNCH_EXERCISE_NAME_FROG_PRESS ((FIT_CRUNCH_EXERCISE_NAME)19)
#define FIT_CRUNCH_EXERCISE_NAME_HANGING_KNEE_RAISE_OBLIQUE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)20)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_HANGING_KNEE_RAISE_OBLIQUE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)21)
#define FIT_CRUNCH_EXERCISE_NAME_HIP_CROSSOVER ((FIT_CRUNCH_EXERCISE_NAME)22)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_HIP_CROSSOVER \
  ((FIT_CRUNCH_EXERCISE_NAME)23)
#define FIT_CRUNCH_EXERCISE_NAME_HOLLOW_ROCK ((FIT_CRUNCH_EXERCISE_NAME)24)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_HOLLOW_ROCK \
  ((FIT_CRUNCH_EXERCISE_NAME)25)
#define FIT_CRUNCH_EXERCISE_NAME_INCLINE_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)26)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_INCLINE_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)27)
#define FIT_CRUNCH_EXERCISE_NAME_KNEELING_CABLE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)28)
#define FIT_CRUNCH_EXERCISE_NAME_KNEELING_CROSS_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)29)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_KNEELING_CROSS_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)30)
#define FIT_CRUNCH_EXERCISE_NAME_KNEELING_OBLIQUE_CABLE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)31)
...
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_LEG_EXTENSIONS \
  ((FIT_CRUNCH_EXERCISE_NAME)34)
...
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_MCGILL_CURL_UP \
  ((FIT_CRUNCH_EXERCISE_NAME)37)
#define FIT_CRUNCH_EXERCISE_NAME_MODIFIED_PILATES_ROLL_UP_WITH_BALL \
  ((FIT_CRUNCH_EXERCISE_NAME)38)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_MODIFIED_PILATES_ROLL_UP_WITH_BALL \
  ((FIT_CRUNCH_EXERCISE_NAME)39)
#define FIT_CRUNCH_EXERCISE_NAME_PILATES_CRUNCH ((FIT_CRUNCH_EXERCISE_NAME)40)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_PILATES_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)41)
#define FIT_CRUNCH_EXERCISE_NAME_PILATES_ROLL_UP_WITH_BALL \
  ((FIT_CRUNCH_EXERCISE_NAME)42)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_PILATES_ROLL_UP_WITH_BALL \
  ((FIT_CRUNCH_EXERCISE_NAME)43)
#define FIT_CRUNCH_EXERCISE_NAME_RAISED_LEGS_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)44)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_RAISED_LEGS_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)45)
#define FIT_CRUNCH_EXERCISE_NAME_REVERSE_CRUNCH ((FIT_CRUNCH_EXERCISE_NAME)46)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)47)
#define FIT_CRUNCH_EXERCISE_NAME_REVERSE_CRUNCH_ON_A_BENCH \
  ((FIT_CRUNCH_EXERCISE_NAME)48)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_REVERSE_CRUNCH_ON_A_BENCH \
  ((FIT_CRUNCH_EXERCISE_NAME)49)
#define FIT_CRUNCH_EXERCISE_NAME_REVERSE_CURL_AND_LIFT \
  ((FIT_CRUNCH_EXERCISE_NAME)50)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_REVERSE_CURL_AND_LIFT \
  ((FIT_CRUNCH_EXERCISE_NAME)51)
#define FIT_CRUNCH_EXERCISE_NAME_ROTATIONAL_LIFT ((FIT_CRUNCH_EXERCISE_NAME)52)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_ROTATIONAL_LIFT \
  ((FIT_CRUNCH_EXERCISE_NAME)53)
#define FIT_CRUNCH_EXERCISE_NAME_SEATED_ALTERNATING_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)54)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SEATED_ALTERNATING_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)55)
#define FIT_CRUNCH_EXERCISE_NAME_SEATED_LEG_U ((FIT_CRUNCH_EXERCISE_NAME)56)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SEATED_LEG_U \
  ((FIT_CRUNCH_EXERCISE_NAME)57)
#define FIT_CRUNCH_EXERCISE_NAME_SIDE_TO_SIDE_CRUNCH_AND_WEAVE \
  ((FIT_CRUNCH_EXERCISE_NAME)58)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SIDE_TO_SIDE_CRUNCH_AND_WEAVE \
  ((FIT_CRUNCH_EXERCISE_NAME)59)
#define FIT_CRUNCH_EXERCISE_NAME_SINGLE_LEG_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)60)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)61)
#define FIT_CRUNCH_EXERCISE_NAME_SKATER_CRUNCH_CROSS \
  ((FIT_CRUNCH_EXERCISE_NAME)62)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SKATER_CRUNCH_CROSS \
  ((FIT_CRUNCH_EXERCISE_NAME)63)
#define FIT_CRUNCH_EXERCISE_NAME_STANDING_CABLE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)64)
#define FIT_CRUNCH_EXERCISE_NAME_STANDING_SIDE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)65)
#define FIT_CRUNCH_EXERCISE_NAME_STEP_CLIMB ((FIT_CRUNCH_EXERCISE_NAME)66)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_STEP_CLIMB \
  ((FIT_CRUNCH_EXERCISE_NAME)67)
#define FIT_CRUNCH_EXERCISE_NAME_SWISS_BALL_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)68)
#define FIT_CRUNCH_EXERCISE_NAME_SWISS_BALL_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)69)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SWISS_BALL_REVERSE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)70)
#define FIT_CRUNCH_EXERCISE_NAME_SWISS_BALL_RUSSIAN_TWIST \
  ((FIT_CRUNCH_EXERCISE_NAME)71)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SWISS_BALL_RUSSIAN_TWIST \
  ((FIT_CRUNCH_EXERCISE_NAME)72)
#define FIT_CRUNCH_EXERCISE_NAME_SWISS_BALL_SIDE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)73)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SWISS_BALL_SIDE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)74)
#define FIT_CRUNCH_EXERCISE_NAME_THORACIC_CRUNCHES_ON_FOAM_ROLLER \
  ((FIT_CRUNCH_EXERCISE_NAME)75)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_THORACIC_CRUNCHES_ON_FOAM_ROLLER \
  ((FIT_CRUNCH_EXERCISE_NAME)76)
#define FIT_CRUNCH_EXERCISE_NAME_TRICEPS_CRUNCH ((FIT_CRUNCH_EXERCISE_NAME)77)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_BICYCLE_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)78)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_CRUNCH ((FIT_CRUNCH_EXERCISE_NAME)79)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_SWISS_BALL_CRUNCH \
  ((FIT_CRUNCH_EXERCISE_NAME)80)
#define FIT_CRUNCH_EXERCISE_NAME_TOES_TO_BAR ((FIT_CRUNCH_EXERCISE_NAME)81)
#define FIT_CRUNCH_EXERCISE_NAME_WEIGHTED_TOES_TO_BAR \
  ((FIT_CRUNCH_EXERCISE_NAME)82)
#define FIT_CRUNCH_EXERCISE_NAME_CRUNCH ((FIT_CRUNCH_EXERCISE_NAME)83)
#define FIT_CRUNCH_EXERCISE_NAME_STRAIGHT_LEG_CRUNCH_WITH_BALL \
  ((FIT_CRUNCH_EXERCISE_NAME)84)
...
#define FIT_CURL_EXERCISE_NAME_ALTERNATING_DUMBBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)0)
#define FIT_CURL_EXERCISE_NAME_ALTERNATING_DUMBBELL_BICEPS_CURL_ON_SWISS_BALL \
  ((FIT_CURL_EXERCISE_NAME)1)
#define FIT_CURL_EXERCISE_NAME_ALTERNATING_INCLINE_DUMBBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)2)
#define FIT_CURL_EXERCISE_NAME_BARBELL_BICEPS_CURL ((FIT_CURL_EXERCISE_NAME)3)
#define FIT_CURL_EXERCISE_NAME_BARBELL_REVERSE_WRIST_CURL \
  ((FIT_CURL_EXERCISE_NAME)4)
#define FIT_CURL_EXERCISE_NAME_BARBELL_WRIST_CURL ((FIT_CURL_EXERCISE_NAME)5)
#define FIT_CURL_EXERCISE_NAME_BEHIND_THE_BACK_BARBELL_REVERSE_WRIST_CURL \
  ((FIT_CURL_EXERCISE_NAME)6)
#define FIT_CURL_EXERCISE_NAME_BEHIND_THE_BACK_ONE_ARM_CABLE_CURL \
  ((FIT_CURL_EXERCISE_NAME)7)
...
#define FIT_CURL_EXERCISE_NAME_CHEATING_BARBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)10)
#define FIT_CURL_EXERCISE_NAME_CLOSE_GRIP_EZ_BAR_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)11)
#define FIT_CURL_EXERCISE_NAME_CROSS_BODY_DUMBBELL_HAMMER_CURL \
  ((FIT_CURL_EXERCISE_NAME)12)
#define FIT_CURL_EXERCISE_NAME_DEAD_HANG_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)13)
#define FIT_CURL_EXERCISE_NAME_DECLINE_HAMMER_CURL ((FIT_CURL_EXERCISE_NAME)14)
#define FIT_CURL_EXERCISE_NAME_DUMBBELL_BICEPS_CURL_WITH_STATIC_HOLD \
  ((FIT_CURL_EXERCISE_NAME)15)
#define FIT_CURL_EXERCISE_NAME_DUMBBELL_HAMMER_CURL ((FIT_CURL_EXERCISE_NAME)16)
#define FIT_CURL_EXERCISE_NAME_DUMBBELL_REVERSE_WRIST_CURL \
  ((FIT_CURL_EXERCISE_NAME)17)
...
#define FIT_CURL_EXERCISE_NAME_FORWARD_BEND_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)20)
#define FIT_CURL_EXERCISE_NAME_HAMMER_CURL_TO_PRESS ((FIT_CURL_EXERCISE_NAME)21)
#define FIT_CURL_EXERCISE_NAME_INCLINE_DUMBBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)22)
#define FIT_CURL_EXERCISE_NAME_INCLINE_OFFSET_THUMB_DUMBBELL_CURL \
  ((FIT_CURL_EXERCISE_NAME)23)
#define FIT_CURL_EXERCISE_NAME_KETTLEBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)24)
#define FIT_CURL_EXERCISE_NAME_LYING_CONCENTRATION_CABLE_CURL \
  ((FIT_CURL_EXERCISE_NAME)25)
#define FIT_CURL_EXERCISE_NAME_ONE_ARM_PREACHER_CURL \
  ((FIT_CURL_EXERCISE_NAME)26)
#define FIT_CURL_EXERCISE_NAME_PLATE_PINCH_CURL ((FIT_CURL_EXERCISE_NAME)27)
#define FIT_CURL_EXERCISE_NAME_PREACHER_CURL_WITH_CABLE \
  ((FIT_CURL_EXERCISE_NAME)28)
#define FIT_CURL_EXERCISE_NAME_REVERSE_EZ_BAR_CURL ((FIT_CURL_EXERCISE_NAME)29)
#define FIT_CURL_EXERCISE_NAME_REVERSE_GRIP_WRIST_CURL \
  ((FIT_CURL_EXERCISE_NAME)30)
#define FIT_CURL_EXERCISE_NAME_REVERSE_GRIP_BARBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)31)
#define FIT_CURL_EXERCISE_NAME_SEATED_ALTERNATING_DUMBBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)32)
#define FIT_CURL_EXERCISE_NAME_SEATED_DUMBBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)33)
#define FIT_CURL_EXERCISE_NAME_SEATED_REVERSE_DUMBBELL_CURL \
  ((FIT_CURL_EXERCISE_NAME)34)
#define FIT_CURL_EXERCISE_NAME_SPLIT_STANCE_OFFSET_PINKY_DUMBBELL_CURL \
  ((FIT_CURL_EXERCISE_NAME)35)
#define FIT_CURL_EXERCISE_NAME_STANDING_ALTERNATING_DUMBBELL_CURLS \
  ((FIT_CURL_EXERCISE_NAME)36)
#define FIT_CURL_EXERCISE_NAME_STANDING_DUMBBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)37)
#define FIT_CURL_EXERCISE_NAME_STANDING_EZ_BAR_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)38)
#define FIT_CURL_EXERCISE_NAME_STATIC_CURL ((FIT_CURL_EXERCISE_NAME)39)
#define FIT_CURL_EXERCISE_NAME_SWISS_BALL_DUMBBELL_OVERHEAD_TRICEPS_EXTENSION \
  ((FIT_CURL_EXERCISE_NAME)40)
#define FIT_CURL_EXERCISE_NAME_SWISS_BALL_EZ_BAR_PREACHER_CURL \
  ((FIT_CURL_EXERCISE_NAME)41)
#define FIT_CURL_EXERCISE_NAME_TWISTING_STANDING_DUMBBELL_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)42)
#define FIT_CURL_EXERCISE_NAME_WIDE_GRIP_EZ_BAR_BICEPS_CURL \
  ((FIT_CURL_EXERCISE_NAME)43)
...
#define FIT_DEADLIFT_EXERCISE_NAME_BARBELL_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)0)
#define FIT_DEADLIFT_EXERCISE_NAME_BARBELL_STRAIGHT_LEG_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)1)
#define FIT_DEADLIFT_EXERCISE_NAME_DUMBBELL_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)2)
#define FIT_DEADLIFT_EXERCISE_NAME_DUMBBELL_SINGLE_LEG_DEADLIFT_TO_ROW \
  ((FIT_DEADLIFT_EXERCISE_NAME)3)
#define FIT_DEADLIFT_EXERCISE_NAME_DUMBBELL_STRAIGHT_LEG_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)4)
#define FIT_DEADLIFT_EXERCISE_NAME_KETTLEBELL_FLOOR_TO_SHELF \
  ((FIT_DEADLIFT_EXERCISE_NAME)5)
#define FIT_DEADLIFT_EXERCISE_NAME_ONE_ARM_ONE_LEG_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)6)
#define FIT_DEADLIFT_EXERCISE_NAME_RACK_PULL ((FIT_DEADLIFT_EXERCISE_NAME)7)
#define FIT_DEADLIFT_EXERCISE_NAME_ROTATIONAL_DUMBBELL_STRAIGHT_LEG_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)8)
#define FIT_DEADLIFT_EXERCISE_NAME_SINGLE_ARM_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)9)
#define FIT_DEADLIFT_EXERCISE_NAME_SINGLE_LEG_BARBELL_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)10)
#define FIT_DEADLIFT_EXERCISE_NAME_SINGLE_LEG_BARBELL_STRAIGHT_LEG_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)11)
#define FIT_DEADLIFT_EXERCISE_NAME_SINGLE_LEG_DEADLIFT_WITH_BARBELL \
  ((FIT_DEADLIFT_EXERCISE_NAME)12)
#define FIT_DEADLIFT_EXERCISE_NAME_SINGLE_LEG_RDL_CIRCUIT \
  ((FIT_DEADLIFT_EXERCISE_NAME)13)
#define FIT_DEADLIFT_EXERCISE_NAME_SINGLE_LEG_ROMANIAN_DEADLIFT_WITH_DUMBBELL \
  ((FIT_DEADLIFT_EXERCISE_NAME)14)
#define FIT_DEADLIFT_EXERCISE_NAME_SUMO_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)15)
#define FIT_DEADLIFT_EXERCISE_NAME_SUMO_DEADLIFT_HIGH_PULL \
  ((FIT_DEADLIFT_EXERCISE_NAME)16)
#define FIT_DEADLIFT_EXERCISE_NAME_TRAP_BAR_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)17)
#define FIT_DEADLIFT_EXERCISE_NAME_WIDE_GRIP_BARBELL_DEADLIFT \
  ((FIT_DEADLIFT_EXERCISE_NAME)18)
...
#define FIT_FLYE_EXERCISE_NAME_SINGLE_ARM_STANDING_CABLE_REVERSE_FLYE \
  ((FIT_FLYE_EXERCISE_NAME)6)
#define FIT_FLYE_EXERCISE_NAME_SWISS_BALL_DUMBBELL_FLYE \
  ((FIT_FLYE_EXERCISE_NAME)7)
...
#define FIT_HIP_RAISE_EXERCISE_NAME_BARBELL_HIP_THRUST_ON_FLOOR \
  ((FIT_HIP_RAISE_EXERCISE_NAME)0)
#define FIT_HIP_RAISE_EXERCISE_NAME_BARBELL_HIP_THRUST_WITH_BENCH \
  ((FIT_HIP_RAISE_EXERCISE_NAME)1)
#define FIT_HIP_RAISE_EXERCISE_NAME_BENT_KNEE_SWISS_BALL_REVERSE_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)2)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_BENT_KNEE_SWISS_BALL_REVERSE_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)3)
#define FIT_HIP_RAISE_EXERCISE_NAME_BRIDGE_WITH_LEG_EXTENSION \
  ((FIT_HIP_RAISE_EXERCISE_NAME)4)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_BRIDGE_WITH_LEG_EXTENSION \
  ((FIT_HIP_RAISE_EXERCISE_NAME)5)
#define FIT_HIP_RAISE_EXERCISE_NAME_CLAM_BRIDGE ((FIT_HIP_RAISE_EXERCISE_NAME)6)
#define FIT_HIP_RAISE_EXERCISE_NAME_FRONT_KICK_TABLETOP \
  ((FIT_HIP_RAISE_EXERCISE_NAME)7)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_FRONT_KICK_TABLETOP \
  ((FIT_HIP_RAISE_EXERCISE_NAME)8)
#define FIT_HIP_RAISE_EXERCISE_NAME_HIP_EXTENSION_AND_CROSS \
  ((FIT_HIP_RAISE_EXERCISE_NAME)9)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_HIP_EXTENSION_AND_CROSS \
  ((FIT_HIP_RAISE_EXERCISE_NAME)10)
#define FIT_HIP_RAISE_EXERCISE_NAME_HIP_RAISE ((FIT_HIP_RAISE_EXERCISE_NAME)11)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)12)
#define FIT_HIP_RAISE_EXERCISE_NAME_HIP_RAISE_WITH_FEET_ON_SWISS_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)13)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_HIP_RAISE_WITH_FEET_ON_SWISS_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)14)
#define FIT_HIP_RAISE_EXERCISE_NAME_HIP_RAISE_WITH_HEAD_ON_BOSU_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)15)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_HIP_RAISE_WITH_HEAD_ON_BOSU_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)16)
#define FIT_HIP_RAISE_EXERCISE_NAME_HIP_RAISE_WITH_HEAD_ON_SWISS_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)17)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_HIP_RAISE_WITH_HEAD_ON_SWISS_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)18)
#define FIT_HIP_RAISE_EXERCISE_NAME_HIP_RAISE_WITH_KNEE_SQUEEZE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)19)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_HIP_RAISE_WITH_KNEE_SQUEEZE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)20)
#define FIT_HIP_RAISE_EXERCISE_NAME_INCLINE_REAR_LEG_EXTENSION \
  ((FIT_HIP_RAISE_EXERCISE_NAME)21)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_INCLINE_REAR_LEG_EXTENSION \
  ((FIT_HIP_RAISE_EXERCISE_NAME)22)
#define FIT_HIP_RAISE_EXERCISE_NAME_KETTLEBELL_SWING \
  ((FIT_HIP_RAISE_EXERCISE_NAME)23)
#define FIT_HIP_RAISE_EXERCISE_NAME_MARCHING_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)24)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_MARCHING_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)25)
#define FIT_HIP_RAISE_EXERCISE_NAME_MARCHING_HIP_RAISE_WITH_FEET_ON_A_SWISS_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)26)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_MARCHING_HIP_RAISE_WITH_FEET_ON_A_SWISS_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)27)
#define FIT_HIP_RAISE_EXERCISE_NAME_REVERSE_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)28)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_REVERSE_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)29)
#define FIT_HIP_RAISE_EXERCISE_NAME_SINGLE_LEG_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)30)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_HIP_RAISE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)31)
#define FIT_HIP_RAISE_EXERCISE_NAME_SINGLE_LEG_HIP_RAISE_WITH_FOOT_ON_BENCH \
  ((FIT_HIP_RAISE_EXERCISE_NAME)32)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_HIP_RAISE_WITH_FOOT_ON_BENCH \
  ((FIT_HIP_RAISE_EXERCISE_NAME)33)
#define FIT_HIP_RAISE_EXERCISE_NAME_SINGLE_LEG_HIP_RAISE_WITH_FOOT_ON_BOSU_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)34)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_HIP_RAISE_WITH_FOOT_ON_BOSU_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)35)
#define FIT_HIP_RAISE_EXERCISE_NAME_SINGLE_LEG_HIP_RAISE_WITH_FOOT_ON_FOAM_ROLLER \
  ((FIT_HIP_RAISE_EXERCISE_NAME)36)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_HIP_RAISE_WITH_FOOT_ON_FOAM_ROLLER \
  ((FIT_HIP_RAISE_EXERCISE_NAME)37)
#define FIT_HIP_RAISE_EXERCISE_NAME_SINGLE_LEG_HIP_RAISE_WITH_FOOT_ON_MEDICINE_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)38)
...
#define FIT_HIP_RAISE_EXERCISE_NAME_SINGLE_LEG_HIP_RAISE_WITH_HEAD_ON_BOSU_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)40)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_HIP_RAISE_WITH_HEAD_ON_BOSU_BALL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)41)
#define FIT_HIP_RAISE_EXERCISE_NAME_WEIGHTED_CLAM_BRIDGE \
  ((FIT_HIP_RAISE_EXERCISE_NAME)42)
#define FIT_HIP_RAISE_EXERCISE_NAME_SINGLE_LEG_SWISS_BALL_HIP_RAISE_AND_LEG_CURL \
  ((FIT_HIP_RAISE_EXERCISE_NAME)43)
#define FIT_HIP_RAISE_EXERCISE_NAME_CLAMS ((FIT_HIP_RAISE_EXERCISE_NAME)44)
#define FIT_HIP_RAISE_EXERCISE_NAME_INNER_THIGH_CIRCLES \
  ((FIT_HIP_RAISE_EXERCISE_NAME)45)  // Deprecated do not use
#define FIT_HIP_RAISE_EXERCISE_NAME_INNER_THIGH_SIDE_LIFT \
  ((FIT_HIP_RAISE_EXERCISE_NAME)46)  // Deprecated do not use
#define FIT_HIP_RAISE_EXERCISE_NAME_LEG_CIRCLES \
  ((FIT_HIP_RAISE_EXERCISE_NAME)47)
#define FIT_HIP_RAISE_EXERCISE_NAME_LEG_LIFT ((FIT_HIP_RAISE_EXERCISE_NAME)48)
#define FIT_HIP_RAISE_EXERCISE_NAME_LEG_LIFT_IN_EXTERNAL_ROTATION \
  ((FIT_HIP_RAISE_EXERCISE_NAME)49)
...
#define FIT_HIP_STABILITY_EXERCISE_NAME_BAND_SIDE_LYING_LEG_RAISE \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)0)
#define FIT_HIP_STABILITY_EXERCISE_NAME_DEAD_BUG \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)1)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_DEAD_BUG \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)2)
#define FIT_HIP_STABILITY_EXERCISE_NAME_EXTERNAL_HIP_RAISE \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)3)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_EXTERNAL_HIP_RAISE \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)4)
#define FIT_HIP_STABILITY_EXERCISE_NAME_FIRE_HYDRANT_KICKS \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)5)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_FIRE_HYDRANT_KICKS \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)6)
#define FIT_HIP_STABILITY_EXERCISE_NAME_HIP_CIRCLES \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)7)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_HIP_CIRCLES \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)8)
#define FIT_HIP_STABILITY_EXERCISE_NAME_INNER_THIGH_LIFT \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)9)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_INNER_THIGH_LIFT \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)10)
#define FIT_HIP_STABILITY_EXERCISE_NAME_LATERAL_WALKS_WITH_BAND_AT_ANKLES \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)11)
#define FIT_HIP_STABILITY_EXERCISE_NAME_PRETZEL_SIDE_KICK \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)12)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_PRETZEL_SIDE_KICK \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)13)
#define FIT_HIP_STABILITY_EXERCISE_NAME_PRONE_HIP_INTERNAL_ROTATION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)14)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_PRONE_HIP_INTERNAL_ROTATION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)15)
#define FIT_HIP_STABILITY_EXERCISE_NAME_QUADRUPED \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)16)
#define FIT_HIP_STABILITY_EXERCISE_NAME_QUADRUPED_HIP_EXTENSION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)17)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_QUADRUPED_HIP_EXTENSION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)18)
#define FIT_HIP_STABILITY_EXERCISE_NAME_QUADRUPED_WITH_LEG_LIFT \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)19)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_QUADRUPED_WITH_LEG_LIFT \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)20)
#define FIT_HIP_STABILITY_EXERCISE_NAME_SIDE_LYING_LEG_RAISE \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)21)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_SIDE_LYING_LEG_RAISE \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)22)
#define FIT_HIP_STABILITY_EXERCISE_NAME_SLIDING_HIP_ADDUCTION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)23)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_SLIDING_HIP_ADDUCTION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)24)
#define FIT_HIP_STABILITY_EXERCISE_NAME_STANDING_ADDUCTION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)25)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_STANDING_ADDUCTION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)26)
#define FIT_HIP_STABILITY_EXERCISE_NAME_STANDING_CABLE_HIP_ABDUCTION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)27)
#define FIT_HIP_STABILITY_EXERCISE_NAME_STANDING_HIP_ABDUCTION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)28)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_STANDING_HIP_ABDUCTION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)29)
#define FIT_HIP_STABILITY_EXERCISE_NAME_STANDING_REAR_LEG_RAISE \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)30)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_STANDING_REAR_LEG_RAISE \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)31)
#define FIT_HIP_STABILITY_EXERCISE_NAME_SUPINE_HIP_INTERNAL_ROTATION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)32)
#define FIT_HIP_STABILITY_EXERCISE_NAME_WEIGHTED_SUPINE_HIP_INTERNAL_ROTATION \
  ((FIT_HIP_STABILITY_EXERCISE_NAME)33)
...
#define FIT_HIP_SWING_EXERCISE_NAME_SINGLE_ARM_KETTLEBELL_SWING \
  ((FIT_HIP_SWING_EXERCISE_NAME)0)
#define FIT_HIP_SWING_EXERCISE_NAME_SINGLE_ARM_DUMBBELL_SWING \
  ((FIT_HIP_SWING_EXERCISE_NAME)1)
#define FIT_HIP_SWING_EXERCISE_NAME_STEP_OUT_SWING \
  ((FIT_HIP_SWING_EXERCISE_NAME)2)
...
#define FIT_HYPEREXTENSION_EXERCISE_NAME_BASE_ROTATIONS \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)2)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_BASE_ROTATIONS \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)3)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_BENT_KNEE_REVERSE_HYPEREXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)4)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_BENT_KNEE_REVERSE_HYPEREXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)5)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_HOLLOW_HOLD_AND_ROLL \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)6)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_HOLLOW_HOLD_AND_ROLL \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)7)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_KICKS \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)8)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_KICKS \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)9)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_KNEE_RAISES \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)10)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_KNEE_RAISES \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)11)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_KNEELING_SUPERMAN \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)12)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_KNEELING_SUPERMAN \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)13)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_LAT_PULL_DOWN_WITH_ROW \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)14)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_MEDICINE_BALL_DEADLIFT_TO_REACH \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)15)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_ONE_ARM_ONE_LEG_ROW \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)16)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_ONE_ARM_ROW_WITH_BAND \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)17)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_OVERHEAD_LUNGE_WITH_MEDICINE_BALL \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)18)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_PLANK_KNEE_TUCKS \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)19)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_PLANK_KNEE_TUCKS \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)20)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SIDE_STEP \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)21)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_SIDE_STEP \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)22)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SINGLE_LEG_BACK_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)23)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_BACK_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)24)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SPINE_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)25)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_SPINE_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)26)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_STATIC_BACK_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)27)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_STATIC_BACK_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)28)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SUPERMAN_FROM_FLOOR \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)29)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_SUPERMAN_FROM_FLOOR \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)30)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SWISS_BALL_BACK_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)31)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_SWISS_BALL_BACK_EXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)32)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SWISS_BALL_HYPEREXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)33)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_SWISS_BALL_HYPEREXTENSION \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)34)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SWISS_BALL_OPPOSITE_ARM_AND_LEG_LIFT \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)35)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_WEIGHTED_SWISS_BALL_OPPOSITE_ARM_AND_LEG_LIFT \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)36)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_SUPERMAN_ON_SWISS_BALL \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)37)
#define FIT_HYPEREXTENSION_EXERCISE_NAME_COBRA \
  ((FIT_HYPEREXTENSION_EXERCISE_NAME)38)
...
#define FIT_LATERAL_RAISE_EXERCISE_NAME_45_DEGREE_CABLE_EXTERNAL_ROTATION \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)0)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_ALTERNATING_LATERAL_RAISE_WITH_STATIC_HOLD \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)1)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_BAR_MUSCLE_UP \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)2)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_BENT_OVER_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)3)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_CABLE_DIAGONAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)4)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_CABLE_FRONT_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)5)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_CALORIE_ROW \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)6)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_COMBO_SHOULDER_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)7)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_DUMBBELL_DIAGONAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)8)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_DUMBBELL_V_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)9)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_FRONT_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)10)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_LEANING_DUMBBELL_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)11)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_LYING_DUMBBELL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)12)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_MUSCLE_UP \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)13)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_ONE_ARM_CABLE_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)14)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_OVERHAND_GRIP_REAR_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)15)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_PLATE_RAISES \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)16)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_RING_DIP \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)17)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_WEIGHTED_RING_DIP \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)18)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_RING_MUSCLE_UP \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)19)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_WEIGHTED_RING_MUSCLE_UP \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)20)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_ROPE_CLIMB \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)21)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_WEIGHTED_ROPE_CLIMB \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)22)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_SCAPTION \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)23)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_SEATED_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)24)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_SEATED_REAR_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)25)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_SIDE_LYING_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)26)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_STANDING_LIFT \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)27)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_SUSPENDED_ROW \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)28)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_UNDERHAND_GRIP_REAR_LATERAL_RAISE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)29)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_WALL_SLIDE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)30)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_WEIGHTED_WALL_SLIDE \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)31)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_ARM_CIRCLES \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)32)
#define FIT_LATERAL_RAISE_EXERCISE_NAME_SHAVING_THE_HEAD \
  ((FIT_LATERAL_RAISE_EXERCISE_NAME)33)
...
#define FIT_LEG_CURL_EXERCISE_NAME_WEIGHTED_LEG_CURL \
  ((FIT_LEG_CURL_EXERCISE_NAME)1)
#define FIT_LEG_CURL_EXERCISE_NAME_GOOD_MORNING ((FIT_LEG_CURL_EXERCISE_NAME)2)
#define FIT_LEG_CURL_EXERCISE_NAME_SEATED_BARBELL_GOOD_MORNING \
  ((FIT_LEG_CURL_EXERCISE_NAME)3)
#define FIT_LEG_CURL_EXERCISE_NAME_SINGLE_LEG_BARBELL_GOOD_MORNING \
  ((FIT_LEG_CURL_EXERCISE_NAME)4)
#define FIT_LEG_CURL_EXERCISE_NAME_SINGLE_LEG_SLIDING_LEG_CURL \
  ((FIT_LEG_CURL_EXERCISE_NAME)5)
#define FIT_LEG_CURL_EXERCISE_NAME_SLIDING_LEG_CURL \
  ((FIT_LEG_CURL_EXERCISE_NAME)6)
#define FIT_LEG_CURL_EXERCISE_NAME_SPLIT_BARBELL_GOOD_MORNING \
  ((FIT_LEG_CURL_EXERCISE_NAME)7)
#define FIT_LEG_CURL_EXERCISE_NAME_SPLIT_STANCE_EXTENSION \
  ((FIT_LEG_CURL_EXERCISE_NAME)8)
#define FIT_LEG_CURL_EXERCISE_NAME_STAGGERED_STANCE_GOOD_MORNING \
  ((FIT_LEG_CURL_EXERCISE_NAME)9)
#define FIT_LEG_CURL_EXERCISE_NAME_SWISS_BALL_HIP_RAISE_AND_LEG_CURL \
  ((FIT_LEG_CURL_EXERCISE_NAME)10)
#define FIT_LEG_CURL_EXERCISE_NAME_ZERCHER_GOOD_MORNING \
  ((FIT_LEG_CURL_EXERCISE_NAME)11)
...
#define FIT_LEG_RAISE_EXERCISE_NAME_HANGING_KNEE_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)0)
#define FIT_LEG_RAISE_EXERCISE_NAME_HANGING_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)1)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_HANGING_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)2)
#define FIT_LEG_RAISE_EXERCISE_NAME_HANGING_SINGLE_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)3)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_HANGING_SINGLE_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)4)
#define FIT_LEG_RAISE_EXERCISE_NAME_KETTLEBELL_LEG_RAISES \
  ((FIT_LEG_RAISE_EXERCISE_NAME)5)
#define FIT_LEG_RAISE_EXERCISE_NAME_LEG_LOWERING_DRILL \
  ((FIT_LEG_RAISE_EXERCISE_NAME)6)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_LEG_LOWERING_DRILL \
  ((FIT_LEG_RAISE_EXERCISE_NAME)7)
#define FIT_LEG_RAISE_EXERCISE_NAME_LYING_STRAIGHT_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)8)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_LYING_STRAIGHT_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)9)
#define FIT_LEG_RAISE_EXERCISE_NAME_MEDICINE_BALL_LEG_DROPS \
  ((FIT_LEG_RAISE_EXERCISE_NAME)10)
#define FIT_LEG_RAISE_EXERCISE_NAME_QUADRUPED_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)11)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_QUADRUPED_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)12)
#define FIT_LEG_RAISE_EXERCISE_NAME_REVERSE_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)13)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_REVERSE_LEG_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)14)
#define FIT_LEG_RAISE_EXERCISE_NAME_REVERSE_LEG_RAISE_ON_SWISS_BALL \
  ((FIT_LEG_RAISE_EXERCISE_NAME)15)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_REVERSE_LEG_RAISE_ON_SWISS_BALL \
  ((FIT_LEG_RAISE_EXERCISE_NAME)16)
#define FIT_LEG_RAISE_EXERCISE_NAME_SINGLE_LEG_LOWERING_DRILL \
  ((FIT_LEG_RAISE_EXERCISE_NAME)17)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_LOWERING_DRILL \
  ((FIT_LEG_RAISE_EXERCISE_NAME)18)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_HANGING_KNEE_RAISE \
  ((FIT_LEG_RAISE_EXERCISE_NAME)19)
#define FIT_LEG_RAISE_EXERCISE_NAME_LATERAL_STEPOVER \
  ((FIT_LEG_RAISE_EXERCISE_NAME)20)
#define FIT_LEG_RAISE_EXERCISE_NAME_WEIGHTED_LATERAL_STEPOVER \
  ((FIT_LEG_RAISE_EXERCISE_NAME)21)
...
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_LUNGE_MATRIX \
  ((FIT_LUNGE_EXERCISE_NAME)2)
#define FIT_LUNGE_EXERCISE_NAME_ALTERNATING_BARBELL_FORWARD_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)3)
#define FIT_LUNGE_EXERCISE_NAME_ALTERNATING_DUMBBELL_LUNGE_WITH_REACH \
  ((FIT_LUNGE_EXERCISE_NAME)4)
#define FIT_LUNGE_EXERCISE_NAME_BACK_FOOT_ELEVATED_DUMBBELL_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)5)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_BOX_LUNGE ((FIT_LUNGE_EXERCISE_NAME)6)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_BULGARIAN_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)7)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_CROSSOVER_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)8)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_FRONT_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)9)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_LUNGE ((FIT_LUNGE_EXERCISE_NAME)10)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_REVERSE_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)11)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_SIDE_LUNGE ((FIT_LUNGE_EXERCISE_NAME)12)
#define FIT_LUNGE_EXERCISE_NAME_BARBELL_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)13)
#define FIT_LUNGE_EXERCISE_NAME_CORE_CONTROL_REAR_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)14)
...
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_BULGARIAN_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)18)
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_CROSSOVER_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)19)
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_DIAGONAL_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)20)
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_LUNGE ((FIT_LUNGE_EXERCISE_NAME)21)
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_LUNGE_AND_ROTATION \
  ((FIT_LUNGE_EXERCISE_NAME)22)
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_OVERHEAD_BULGARIAN_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)23)
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_REVERSE_LUNGE_TO_HIGH_KNEE_AND_PRESS \
  ((FIT_LUNGE_EXERCISE_NAME)24)
#define FIT_LUNGE_EXERCISE_NAME_DUMBBELL_SIDE_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)25)
#define FIT_LUNGE_EXERCISE_NAME_ELEVATED_FRONT_FOOT_BARBELL_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)26)
#define FIT_LUNGE_EXERCISE_NAME_FRONT_FOOT_ELEVATED_DUMBBELL_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)27)
...
#define FIT_LUNGE_EXERCISE_NAME_LOW_LUNGE_WITH_ISOMETRIC_ADDUCTION \
  ((FIT_LUNGE_EXERCISE_NAME)30)
#define FIT_LUNGE_EXERCISE_NAME_LOW_SIDE_TO_SIDE_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)31)
...
#define FIT_LUNGE_EXERCISE_NAME_LUNGE_WITH_ARM_REACH \
  ((FIT_LUNGE_EXERCISE_NAME)34)
#define FIT_LUNGE_EXERCISE_NAME_LUNGE_WITH_DIAGONAL_REACH \
  ((FIT_LUNGE_EXERCISE_NAME)35)
#define FIT_LUNGE_EXERCISE_NAME_LUNGE_WITH_SIDE_BEND \
  ((FIT_LUNGE_EXERCISE_NAME)36)
#define FIT_LUNGE_EXERCISE_NAME_OFFSET_DUMBBELL_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)37)
#define FIT_LUNGE_EXERCISE_NAME_OFFSET_DUMBBELL_REVERSE_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)38)
#define FIT_LUNGE_EXERCISE_NAME_OVERHEAD_BULGARIAN_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)39)
#define FIT_LUNGE_EXERCISE_NAME_OVERHEAD_DUMBBELL_REVERSE_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)40)
#define FIT_LUNGE_EXERCISE_NAME_OVERHEAD_DUMBBELL_SPLIT_SQUAT \
  ((FIT_LUNGE_EXERCISE_NAME)41)
#define FIT_LUNGE_EXERCISE_NAME_OVERHEAD_LUNGE_WITH_ROTATION \
  ((FIT_LUNGE_EXERCISE_NAME)42)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_BARBELL_BOX_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)43)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_BOX_LUNGE ((FIT_LUNGE_EXERCISE_NAME)44)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_DUMBBELL_BOX_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)45)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_DUMBBELL_CROSSOVER_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)46)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_DUMBBELL_DIAGONAL_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)47)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_LUNGE_WITH_REACH_BACK \
  ((FIT_LUNGE_EXERCISE_NAME)48)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_REVERSE_LUNGE_WITH_REACH_BACK \
  ((FIT_LUNGE_EXERCISE_NAME)49)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_LUNGE_WITH_TWIST_AND_OVERHEAD_REACH \
  ((FIT_LUNGE_EXERCISE_NAME)50)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_REVERSE_LUNGE_WITH_TWIST_AND_OVERHEAD_REACH \
  ((FIT_LUNGE_EXERCISE_NAME)51)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_SLIDING_BOX_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)52)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_REVERSE_SLIDING_BOX_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)53)
#define FIT_LUNGE_EXERCISE_NAME_REVERSE_SLIDING_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)54)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_REVERSE_SLIDING_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)55)
#define FIT_LUNGE_EXERCISE_NAME_RUNNERS_LUNGE_TO_BALANCE \
  ((FIT_LUNGE_EXERCISE_NAME)56)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_RUNNERS_LUNGE_TO_BALANCE \
  ((FIT_LUNGE_EXERCISE_NAME)57)
#define FIT_LUNGE_EXERCISE_NAME_SHIFTING_SIDE_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)58)
#define FIT_LUNGE_EXERCISE_NAME_SIDE_AND_CROSSOVER_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)59)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_SIDE_AND_CROSSOVER_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)60)
#define FIT_LUNGE_EXERCISE_NAME_SIDE_LUNGE ((FIT_LUNGE_EXERCISE_NAME)61)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_SIDE_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)62)
#define FIT_LUNGE_EXERCISE_NAME_SIDE_LUNGE_AND_PRESS \
  ((FIT_LUNGE_EXERCISE_NAME)63)
#define FIT_LUNGE_EXERCISE_NAME_SIDE_LUNGE_JUMP_OFF \
  ((FIT_LUNGE_EXERCISE_NAME)64)
#define FIT_LUNGE_EXERCISE_NAME_SIDE_LUNGE_SWEEP ((FIT_LUNGE_EXERCISE_NAME)65)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_SIDE_LUNGE_SWEEP \
  ((FIT_LUNGE_EXERCISE_NAME)66)
#define FIT_LUNGE_EXERCISE_NAME_SIDE_LUNGE_TO_CROSSOVER_TAP \
  ((FIT_LUNGE_EXERCISE_NAME)67)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_SIDE_LUNGE_TO_CROSSOVER_TAP \
  ((FIT_LUNGE_EXERCISE_NAME)68)
#define FIT_LUNGE_EXERCISE_NAME_SIDE_TO_SIDE_LUNGE_CHOPS \
  ((FIT_LUNGE_EXERCISE_NAME)69)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_SIDE_TO_SIDE_LUNGE_CHOPS \
  ((FIT_LUNGE_EXERCISE_NAME)70)
#define FIT_LUNGE_EXERCISE_NAME_SIFF_JUMP_LUNGE ((FIT_LUNGE_EXERCISE_NAME)71)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_SIFF_JUMP_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)72)
#define FIT_LUNGE_EXERCISE_NAME_SINGLE_ARM_REVERSE_LUNGE_AND_PRESS \
  ((FIT_LUNGE_EXERCISE_NAME)73)
#define FIT_LUNGE_EXERCISE_NAME_SLIDING_LATERAL_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)74)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_SLIDING_LATERAL_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)75)
#define FIT_LUNGE_EXERCISE_NAME_WALKING_BARBELL_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)76)
#define FIT_LUNGE_EXERCISE_NAME_WALKING_DUMBBELL_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)77)
#define FIT_LUNGE_EXERCISE_NAME_WALKING_LUNGE ((FIT_LUNGE_EXERCISE_NAME)78)
#define FIT_LUNGE_EXERCISE_NAME_WEIGHTED_WALKING_LUNGE \
  ((FIT_LUNGE_EXERCISE_NAME)79)
#define FIT_LUNGE_EXERCISE_NAME_WIDE_GRIP_OVERHEAD_BARBELL_SPLIT_SQUAT \
...
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_HANG_POWER_CLEAN \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)0)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_HANG_SQUAT_CLEAN \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)1)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_POWER_CLEAN \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)2)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_POWER_SNATCH \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)3)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_SQUAT_CLEAN \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)4)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_CLEAN_AND_JERK \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)5)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_HANG_POWER_SNATCH \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)6)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_HANG_PULL \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)7)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_HIGH_PULL \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)8)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_SNATCH \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)9)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_BARBELL_SPLIT_JERK \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)10)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_CLEAN \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)11)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_DUMBBELL_CLEAN \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)12)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_DUMBBELL_HANG_PULL \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)13)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_ONE_HAND_DUMBBELL_SPLIT_SNATCH \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)14)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_PUSH_JERK \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)15)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_SINGLE_ARM_DUMBBELL_SNATCH \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)16)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_SINGLE_ARM_HANG_SNATCH \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)17)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_SINGLE_ARM_KETTLEBELL_SNATCH \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)18)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_SPLIT_JERK \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)19)
#define FIT_OLYMPIC_LIFT_EXERCISE_NAME_SQUAT_CLEAN_AND_JERK \
  ((FIT_OLYMPIC_LIFT_EXERCISE_NAME)20)
...
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_45_DEGREE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)1)
#define FIT_PLANK_EXERCISE_NAME_90_DEGREE_STATIC_HOLD \
  ((FIT_PLANK_EXERCISE_NAME)2)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_90_DEGREE_STATIC_HOLD \
  ((FIT_PLANK_EXERCISE_NAME)3)
...
#define FIT_PLANK_EXERCISE_NAME_CROSS_BODY_MOUNTAIN_CLIMBER \
  ((FIT_PLANK_EXERCISE_NAME)6)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_CROSS_BODY_MOUNTAIN_CLIMBER \
  ((FIT_PLANK_EXERCISE_NAME)7)
#define FIT_PLANK_EXERCISE_NAME_ELBOW_PLANK_PIKE_JACKS \
  ((FIT_PLANK_EXERCISE_NAME)8)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_ELBOW_PLANK_PIKE_JACKS \
  ((FIT_PLANK_EXERCISE_NAME)9)
#define FIT_PLANK_EXERCISE_NAME_ELEVATED_FEET_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)10)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_ELEVATED_FEET_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)11)
#define FIT_PLANK_EXERCISE_NAME_ELEVATOR_ABS ((FIT_PLANK_EXERCISE_NAME)12)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_ELEVATOR_ABS \
  ((FIT_PLANK_EXERCISE_NAME)13)
#define FIT_PLANK_EXERCISE_NAME_EXTENDED_PLANK ((FIT_PLANK_EXERCISE_NAME)14)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_EXTENDED_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)15)
#define FIT_PLANK_EXERCISE_NAME_FULL_PLANK_PASSE_TWIST \
  ((FIT_PLANK_EXERCISE_NAME)16)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_FULL_PLANK_PASSE_TWIST \
  ((FIT_PLANK_EXERCISE_NAME)17)
#define FIT_PLANK_EXERCISE_NAME_INCHING_ELBOW_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)18)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_INCHING_ELBOW_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)19)
#define FIT_PLANK_EXERCISE_NAME_INCHWORM_TO_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)20)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_INCHWORM_TO_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)21)
#define FIT_PLANK_EXERCISE_NAME_KNEELING_PLANK ((FIT_PLANK_EXERCISE_NAME)22)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_KNEELING_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)23)
#define FIT_PLANK_EXERCISE_NAME_KNEELING_SIDE_PLANK_WITH_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)24)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_KNEELING_SIDE_PLANK_WITH_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)25)
#define FIT_PLANK_EXERCISE_NAME_LATERAL_ROLL ((FIT_PLANK_EXERCISE_NAME)26)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_LATERAL_ROLL \
  ((FIT_PLANK_EXERCISE_NAME)27)
#define FIT_PLANK_EXERCISE_NAME_LYING_REVERSE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)28)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_LYING_REVERSE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)29)
#define FIT_PLANK_EXERCISE_NAME_MEDICINE_BALL_MOUNTAIN_CLIMBER \
  ((FIT_PLANK_EXERCISE_NAME)30)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_MEDICINE_BALL_MOUNTAIN_CLIMBER \
  ((FIT_PLANK_EXERCISE_NAME)31)
#define FIT_PLANK_EXERCISE_NAME_MODIFIED_MOUNTAIN_CLIMBER_AND_EXTENSION \
  ((FIT_PLANK_EXERCISE_NAME)32)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_MODIFIED_MOUNTAIN_CLIMBER_AND_EXTENSION \
  ((FIT_PLANK_EXERCISE_NAME)33)
#define FIT_PLANK_EXERCISE_NAME_MOUNTAIN_CLIMBER ((FIT_PLANK_EXERCISE_NAME)34)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_MOUNTAIN_CLIMBER \
  ((FIT_PLANK_EXERCISE_NAME)35)
#define FIT_PLANK_EXERCISE_NAME_MOUNTAIN_CLIMBER_ON_SLIDING_DISCS \
  ((FIT_PLANK_EXERCISE_NAME)36)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_MOUNTAIN_CLIMBER_ON_SLIDING_DISCS \
  ((FIT_PLANK_EXERCISE_NAME)37)
#define FIT_PLANK_EXERCISE_NAME_MOUNTAIN_CLIMBER_WITH_FEET_ON_BOSU_BALL \
  ((FIT_PLANK_EXERCISE_NAME)38)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_MOUNTAIN_CLIMBER_WITH_FEET_ON_BOSU_BALL \
  ((FIT_PLANK_EXERCISE_NAME)39)
#define FIT_PLANK_EXERCISE_NAME_MOUNTAIN_CLIMBER_WITH_HANDS_ON_BENCH \
  ((FIT_PLANK_EXERCISE_NAME)40)
#define FIT_PLANK_EXERCISE_NAME_MOUNTAIN_CLIMBER_WITH_HANDS_ON_SWISS_BALL \
  ((FIT_PLANK_EXERCISE_NAME)41)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_MOUNTAIN_CLIMBER_WITH_HANDS_ON_SWISS_BALL \
  ((FIT_PLANK_EXERCISE_NAME)42)
#define FIT_PLANK_EXERCISE_NAME_PLANK ((FIT_PLANK_EXERCISE_NAME)43)
#define FIT_PLANK_EXERCISE_NAME_PLANK_JACKS_WITH_FEET_ON_SLIDING_DISCS \
  ((FIT_PLANK_EXERCISE_NAME)44)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_JACKS_WITH_FEET_ON_SLIDING_DISCS \
  ((FIT_PLANK_EXERCISE_NAME)45)
#define FIT_PLANK_EXERCISE_NAME_PLANK_KNEE_TWIST ((FIT_PLANK_EXERCISE_NAME)46)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_KNEE_TWIST \
  ((FIT_PLANK_EXERCISE_NAME)47)
#define FIT_PLANK_EXERCISE_NAME_PLANK_PIKE_JUMPS ((FIT_PLANK_EXERCISE_NAME)48)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_PIKE_JUMPS \
  ((FIT_PLANK_EXERCISE_NAME)49)
#define FIT_PLANK_EXERCISE_NAME_PLANK_PIKES ((FIT_PLANK_EXERCISE_NAME)50)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_PIKES \
  ((FIT_PLANK_EXERCISE_NAME)51)
#define FIT_PLANK_EXERCISE_NAME_PLANK_TO_STAND_UP ((FIT_PLANK_EXERCISE_NAME)52)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_TO_STAND_UP \
  ((FIT_PLANK_EXERCISE_NAME)53)
#define FIT_PLANK_EXERCISE_NAME_PLANK_WITH_ARM_RAISE \
  ((FIT_PLANK_EXERCISE_NAME)54)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_WITH_ARM_RAISE \
  ((FIT_PLANK_EXERCISE_NAME)55)
#define FIT_PLANK_EXERCISE_NAME_PLANK_WITH_KNEE_TO_ELBOW \
  ((FIT_PLANK_EXERCISE_NAME)56)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_WITH_KNEE_TO_ELBOW \
  ((FIT_PLANK_EXERCISE_NAME)57)
#define FIT_PLANK_EXERCISE_NAME_PLANK_WITH_OBLIQUE_CRUNCH \
  ((FIT_PLANK_EXERCISE_NAME)58)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK_WITH_OBLIQUE_CRUNCH \
  ((FIT_PLANK_EXERCISE_NAME)59)
#define FIT_PLANK_EXERCISE_NAME_PLYOMETRIC_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)60)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLYOMETRIC_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)61)
#define FIT_PLANK_EXERCISE_NAME_ROLLING_SIDE_PLANK ((FIT_PLANK_EXERCISE_NAME)62)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_ROLLING_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)63)
#define FIT_PLANK_EXERCISE_NAME_SIDE_KICK_PLANK ((FIT_PLANK_EXERCISE_NAME)64)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_KICK_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)65)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK ((FIT_PLANK_EXERCISE_NAME)66)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)67)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_AND_ROW ((FIT_PLANK_EXERCISE_NAME)68)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_AND_ROW \
  ((FIT_PLANK_EXERCISE_NAME)69)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_LIFT ((FIT_PLANK_EXERCISE_NAME)70)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)71)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_WITH_ELBOW_ON_BOSU_BALL \
  ((FIT_PLANK_EXERCISE_NAME)72)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_WITH_ELBOW_ON_BOSU_BALL \
  ((FIT_PLANK_EXERCISE_NAME)73)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_WITH_FEET_ON_BENCH \
  ((FIT_PLANK_EXERCISE_NAME)74)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_WITH_FEET_ON_BENCH \
  ((FIT_PLANK_EXERCISE_NAME)75)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_WITH_KNEE_CIRCLE \
  ((FIT_PLANK_EXERCISE_NAME)76)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_WITH_KNEE_CIRCLE \
  ((FIT_PLANK_EXERCISE_NAME)77)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_WITH_KNEE_TUCK \
  ((FIT_PLANK_EXERCISE_NAME)78)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_WITH_KNEE_TUCK \
  ((FIT_PLANK_EXERCISE_NAME)79)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_WITH_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)80)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_WITH_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)81)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_WITH_REACH_UNDER \
  ((FIT_PLANK_EXERCISE_NAME)82)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SIDE_PLANK_WITH_REACH_UNDER \
  ((FIT_PLANK_EXERCISE_NAME)83)
#define FIT_PLANK_EXERCISE_NAME_SINGLE_LEG_ELEVATED_FEET_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)84)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_ELEVATED_FEET_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)85)
#define FIT_PLANK_EXERCISE_NAME_SINGLE_LEG_FLEX_AND_EXTEND \
  ((FIT_PLANK_EXERCISE_NAME)86)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_FLEX_AND_EXTEND \
  ((FIT_PLANK_EXERCISE_NAME)87)
#define FIT_PLANK_EXERCISE_NAME_SINGLE_LEG_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)88)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)89)
#define FIT_PLANK_EXERCISE_NAME_SPIDERMAN_PLANK ((FIT_PLANK_EXERCISE_NAME)90)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SPIDERMAN_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)91)
#define FIT_PLANK_EXERCISE_NAME_STRAIGHT_ARM_PLANK ((FIT_PLANK_EXERCISE_NAME)92)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_STRAIGHT_ARM_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)93)
#define FIT_PLANK_EXERCISE_NAME_STRAIGHT_ARM_PLANK_WITH_SHOULDER_TOUCH \
  ((FIT_PLANK_EXERCISE_NAME)94)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_STRAIGHT_ARM_PLANK_WITH_SHOULDER_TOUCH \
  ((FIT_PLANK_EXERCISE_NAME)95)
#define FIT_PLANK_EXERCISE_NAME_SWISS_BALL_PLANK ((FIT_PLANK_EXERCISE_NAME)96)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)97)
#define FIT_PLANK_EXERCISE_NAME_SWISS_BALL_PLANK_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)98)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PLANK_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)99)
#define FIT_PLANK_EXERCISE_NAME_SWISS_BALL_PLANK_LEG_LIFT_AND_HOLD \
  ((FIT_PLANK_EXERCISE_NAME)100)
#define FIT_PLANK_EXERCISE_NAME_SWISS_BALL_PLANK_WITH_FEET_ON_BENCH \
  ((FIT_PLANK_EXERCISE_NAME)101)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PLANK_WITH_FEET_ON_BENCH \
  ((FIT_PLANK_EXERCISE_NAME)102)
#define FIT_PLANK_EXERCISE_NAME_SWISS_BALL_PRONE_JACKKNIFE \
  ((FIT_PLANK_EXERCISE_NAME)103)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PRONE_JACKKNIFE \
  ((FIT_PLANK_EXERCISE_NAME)104)
#define FIT_PLANK_EXERCISE_NAME_SWISS_BALL_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)105)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SWISS_BALL_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)106)
#define FIT_PLANK_EXERCISE_NAME_THREE_WAY_PLANK ((FIT_PLANK_EXERCISE_NAME)107)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_THREE_WAY_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)108)
#define FIT_PLANK_EXERCISE_NAME_TOWEL_PLANK_AND_KNEE_IN \
  ((FIT_PLANK_EXERCISE_NAME)109)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_TOWEL_PLANK_AND_KNEE_IN \
  ((FIT_PLANK_EXERCISE_NAME)110)
#define FIT_PLANK_EXERCISE_NAME_T_STABILIZATION ((FIT_PLANK_EXERCISE_NAME)111)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_T_STABILIZATION \
  ((FIT_PLANK_EXERCISE_NAME)112)
#define FIT_PLANK_EXERCISE_NAME_TURKISH_GET_UP_TO_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)113)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_TURKISH_GET_UP_TO_SIDE_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)114)
#define FIT_PLANK_EXERCISE_NAME_TWO_POINT_PLANK ((FIT_PLANK_EXERCISE_NAME)115)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_TWO_POINT_PLANK \
  ((FIT_PLANK_EXERCISE_NAME)116)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_PLANK ((FIT_PLANK_EXERCISE_NAME)117)
#define FIT_PLANK_EXERCISE_NAME_WIDE_STANCE_PLANK_WITH_DIAGONAL_ARM_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)118)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_WIDE_STANCE_PLANK_WITH_DIAGONAL_ARM_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)119)
#define FIT_PLANK_EXERCISE_NAME_WIDE_STANCE_PLANK_WITH_DIAGONAL_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)120)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_WIDE_STANCE_PLANK_WITH_DIAGONAL_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)121)
#define FIT_PLANK_EXERCISE_NAME_WIDE_STANCE_PLANK_WITH_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)122)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_WIDE_STANCE_PLANK_WITH_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)123)
#define FIT_PLANK_EXERCISE_NAME_WIDE_STANCE_PLANK_WITH_OPPOSITE_ARM_AND_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)124)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_MOUNTAIN_CLIMBER_WITH_HANDS_ON_BENCH \
  ((FIT_PLANK_EXERCISE_NAME)125)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PLANK_LEG_LIFT_AND_HOLD \
  ((FIT_PLANK_EXERCISE_NAME)126)
#define FIT_PLANK_EXERCISE_NAME_WEIGHTED_WIDE_STANCE_PLANK_WITH_OPPOSITE_ARM_AND_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)127)
#define FIT_PLANK_EXERCISE_NAME_PLANK_WITH_FEET_ON_SWISS_BALL \
  ((FIT_PLANK_EXERCISE_NAME)128)
#define FIT_PLANK_EXERCISE_NAME_SIDE_PLANK_TO_PLANK_WITH_REACH_UNDER \
  ((FIT_PLANK_EXERCISE_NAME)129)
#define FIT_PLANK_EXERCISE_NAME_BRIDGE_WITH_GLUTE_LOWER_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)130)
#define FIT_PLANK_EXERCISE_NAME_BRIDGE_ONE_LEG_BRIDGE \
  ((FIT_PLANK_EXERCISE_NAME)131)
#define FIT_PLANK_EXERCISE_NAME_PLANK_WITH_ARM_VARIATIONS \
  ((FIT_PLANK_EXERCISE_NAME)132)
#define FIT_PLANK_EXERCISE_NAME_PLANK_WITH_LEG_LIFT \
  ((FIT_PLANK_EXERCISE_NAME)133)
#define FIT_PLANK_EXERCISE_NAME_REVERSE_PLANK_WITH_LEG_PULL \
  ((FIT_PLANK_EXERCISE_NAME)134)
...
#define FIT_PLYO_EXERCISE_NAME_ALTERNATING_JUMP_LUNGE \
  ((FIT_PLYO_EXERCISE_NAME)0)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_ALTERNATING_JUMP_LUNGE \
  ((FIT_PLYO_EXERCISE_NAME)1)
#define FIT_PLYO_EXERCISE_NAME_BARBELL_JUMP_SQUAT ((FIT_PLYO_EXERCISE_NAME)2)
#define FIT_PLYO_EXERCISE_NAME_BODY_WEIGHT_JUMP_SQUAT \
  ((FIT_PLYO_EXERCISE_NAME)3)
...
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_CROSS_KNEE_STRIKE \
  ((FIT_PLYO_EXERCISE_NAME)6)
...
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_FRONT_KNEE_STRIKE \
  ((FIT_PLYO_EXERCISE_NAME)12)
#define FIT_PLYO_EXERCISE_NAME_HIGH_BOX_JUMP ((FIT_PLYO_EXERCISE_NAME)13)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_HIGH_BOX_JUMP \
  ((FIT_PLYO_EXERCISE_NAME)14)
#define FIT_PLYO_EXERCISE_NAME_ISOMETRIC_EXPLOSIVE_BODY_WEIGHT_JUMP_SQUAT \
  ((FIT_PLYO_EXERCISE_NAME)15)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_ISOMETRIC_EXPLOSIVE_JUMP_SQUAT \
  ((FIT_PLYO_EXERCISE_NAME)16)
#define FIT_PLYO_EXERCISE_NAME_LATERAL_LEAP_AND_HOP ((FIT_PLYO_EXERCISE_NAME)17)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_LATERAL_LEAP_AND_HOP \
  ((FIT_PLYO_EXERCISE_NAME)18)
#define FIT_PLYO_EXERCISE_NAME_LATERAL_PLYO_SQUATS ((FIT_PLYO_EXERCISE_NAME)19)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_LATERAL_PLYO_SQUATS \
  ((FIT_PLYO_EXERCISE_NAME)20)
#define FIT_PLYO_EXERCISE_NAME_LATERAL_SLIDE ((FIT_PLYO_EXERCISE_NAME)21)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_LATERAL_SLIDE \
  ((FIT_PLYO_EXERCISE_NAME)22)
#define FIT_PLYO_EXERCISE_NAME_MEDICINE_BALL_OVERHEAD_THROWS \
  ((FIT_PLYO_EXERCISE_NAME)23)
#define FIT_PLYO_EXERCISE_NAME_MEDICINE_BALL_SIDE_THROW \
  ((FIT_PLYO_EXERCISE_NAME)24)
#define FIT_PLYO_EXERCISE_NAME_MEDICINE_BALL_SLAM ((FIT_PLYO_EXERCISE_NAME)25)
#define FIT_PLYO_EXERCISE_NAME_SIDE_TO_SIDE_MEDICINE_BALL_THROWS \
  ((FIT_PLYO_EXERCISE_NAME)26)
#define FIT_PLYO_EXERCISE_NAME_SIDE_TO_SIDE_SHUFFLE_JUMP \
  ((FIT_PLYO_EXERCISE_NAME)27)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_SIDE_TO_SIDE_SHUFFLE_JUMP \
  ((FIT_PLYO_EXERCISE_NAME)28)
#define FIT_PLYO_EXERCISE_NAME_SQUAT_JUMP_ONTO_BOX ((FIT_PLYO_EXERCISE_NAME)29)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_SQUAT_JUMP_ONTO_BOX \
  ((FIT_PLYO_EXERCISE_NAME)30)
#define FIT_PLYO_EXERCISE_NAME_SQUAT_JUMPS_IN_AND_OUT \
  ((FIT_PLYO_EXERCISE_NAME)31)
#define FIT_PLYO_EXERCISE_NAME_WEIGHTED_SQUAT_JUMPS_IN_AND_OUT \
  ((FIT_PLYO_EXERCISE_NAME)32)
...
#define FIT_PULL_UP_EXERCISE_NAME_30_DEGREE_LAT_PULLDOWN \
  ((FIT_PULL_UP_EXERCISE_NAME)1)
#define FIT_PULL_UP_EXERCISE_NAME_BAND_ASSISTED_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)2)
#define FIT_PULL_UP_EXERCISE_NAME_CLOSE_GRIP_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)3)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_CLOSE_GRIP_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)4)
#define FIT_PULL_UP_EXERCISE_NAME_CLOSE_GRIP_LAT_PULLDOWN \
  ((FIT_PULL_UP_EXERCISE_NAME)5)
#define FIT_PULL_UP_EXERCISE_NAME_CROSSOVER_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)6)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_CROSSOVER_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)7)
...
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_HANGING_HURDLE \
  ((FIT_PULL_UP_EXERCISE_NAME)10)
#define FIT_PULL_UP_EXERCISE_NAME_KNEELING_LAT_PULLDOWN \
  ((FIT_PULL_UP_EXERCISE_NAME)11)
#define FIT_PULL_UP_EXERCISE_NAME_KNEELING_UNDERHAND_GRIP_LAT_PULLDOWN \
  ((FIT_PULL_UP_EXERCISE_NAME)12)
#define FIT_PULL_UP_EXERCISE_NAME_LAT_PULLDOWN ((FIT_PULL_UP_EXERCISE_NAME)13)
#define FIT_PULL_UP_EXERCISE_NAME_MIXED_GRIP_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)14)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_MIXED_GRIP_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)15)
#define FIT_PULL_UP_EXERCISE_NAME_MIXED_GRIP_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)16)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_MIXED_GRIP_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)17)
#define FIT_PULL_UP_EXERCISE_NAME_REVERSE_GRIP_PULLDOWN \
  ((FIT_PULL_UP_EXERCISE_NAME)18)
#define FIT_PULL_UP_EXERCISE_NAME_STANDING_CABLE_PULLOVER \
  ((FIT_PULL_UP_EXERCISE_NAME)19)
#define FIT_PULL_UP_EXERCISE_NAME_STRAIGHT_ARM_PULLDOWN \
  ((FIT_PULL_UP_EXERCISE_NAME)20)
#define FIT_PULL_UP_EXERCISE_NAME_SWISS_BALL_EZ_BAR_PULLOVER \
  ((FIT_PULL_UP_EXERCISE_NAME)21)
#define FIT_PULL_UP_EXERCISE_NAME_TOWEL_PULL_UP ((FIT_PULL_UP_EXERCISE_NAME)22)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_TOWEL_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)23)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)24)
#define FIT_PULL_UP_EXERCISE_NAME_WIDE_GRIP_LAT_PULLDOWN \
  ((FIT_PULL_UP_EXERCISE_NAME)25)
#define FIT_PULL_UP_EXERCISE_NAME_WIDE_GRIP_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)26)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_WIDE_GRIP_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)27)
#define FIT_PULL_UP_EXERCISE_NAME_BURPEE_PULL_UP ((FIT_PULL_UP_EXERCISE_NAME)28)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_BURPEE_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)29)
#define FIT_PULL_UP_EXERCISE_NAME_JUMPING_PULL_UPS \
  ((FIT_PULL_UP_EXERCISE_NAME)30)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_JUMPING_PULL_UPS \
  ((FIT_PULL_UP_EXERCISE_NAME)31)
#define FIT_PULL_UP_EXERCISE_NAME_KIPPING_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)32)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_KIPPING_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)33)
#define FIT_PULL_UP_EXERCISE_NAME_L_PULL_UP ((FIT_PULL_UP_EXERCISE_NAME)34)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_L_PULL_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)35)
#define FIT_PULL_UP_EXERCISE_NAME_SUSPENDED_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)36)
#define FIT_PULL_UP_EXERCISE_NAME_WEIGHTED_SUSPENDED_CHIN_UP \
  ((FIT_PULL_UP_EXERCISE_NAME)37)
...
#define FIT_PUSH_UP_EXERCISE_NAME_CHEST_PRESS_WITH_BAND \
  ((FIT_PUSH_UP_EXERCISE_NAME)0)
#define FIT_PUSH_UP_EXERCISE_NAME_ALTERNATING_STAGGERED_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)1)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_ALTERNATING_STAGGERED_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)2)
#define FIT_PUSH_UP_EXERCISE_NAME_ALTERNATING_HANDS_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)3)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_ALTERNATING_HANDS_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)4)
#define FIT_PUSH_UP_EXERCISE_NAME_BOSU_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)5)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_BOSU_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)6)
#define FIT_PUSH_UP_EXERCISE_NAME_CLAPPING_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)7)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_CLAPPING_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)8)
#define FIT_PUSH_UP_EXERCISE_NAME_CLOSE_GRIP_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)9)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_CLOSE_GRIP_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)10)
#define FIT_PUSH_UP_EXERCISE_NAME_CLOSE_HANDS_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)11)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_CLOSE_HANDS_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)12)
#define FIT_PUSH_UP_EXERCISE_NAME_DECLINE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)13)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_DECLINE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)14)
#define FIT_PUSH_UP_EXERCISE_NAME_DIAMOND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)15)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_DIAMOND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)16)
#define FIT_PUSH_UP_EXERCISE_NAME_EXPLOSIVE_CROSSOVER_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)17)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_EXPLOSIVE_CROSSOVER_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)18)
#define FIT_PUSH_UP_EXERCISE_NAME_EXPLOSIVE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)19)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_EXPLOSIVE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)20)
#define FIT_PUSH_UP_EXERCISE_NAME_FEET_ELEVATED_SIDE_TO_SIDE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)21)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_FEET_ELEVATED_SIDE_TO_SIDE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)22)
#define FIT_PUSH_UP_EXERCISE_NAME_HAND_RELEASE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)23)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_HAND_RELEASE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)24)
#define FIT_PUSH_UP_EXERCISE_NAME_HANDSTAND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)25)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_HANDSTAND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)26)
#define FIT_PUSH_UP_EXERCISE_NAME_INCLINE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)27)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_INCLINE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)28)
#define FIT_PUSH_UP_EXERCISE_NAME_ISOMETRIC_EXPLOSIVE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)29)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_ISOMETRIC_EXPLOSIVE_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)30)
#define FIT_PUSH_UP_EXERCISE_NAME_JUDO_PUSH_UP ((FIT_PUSH_UP_EXERCISE_NAME)31)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_JUDO_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)32)
#define FIT_PUSH_UP_EXERCISE_NAME_KNEELING_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)33)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_KNEELING_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)34)
#define FIT_PUSH_UP_EXERCISE_NAME_MEDICINE_BALL_CHEST_PASS \
  ((FIT_PUSH_UP_EXERCISE_NAME)35)
#define FIT_PUSH_UP_EXERCISE_NAME_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)36)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)37)
#define FIT_PUSH_UP_EXERCISE_NAME_ONE_ARM_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)38)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_ONE_ARM_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)39)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)40)
#define FIT_PUSH_UP_EXERCISE_NAME_PUSH_UP_AND_ROW \
  ((FIT_PUSH_UP_EXERCISE_NAME)41)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_PUSH_UP_AND_ROW \
  ((FIT_PUSH_UP_EXERCISE_NAME)42)
#define FIT_PUSH_UP_EXERCISE_NAME_PUSH_UP_PLUS ((FIT_PUSH_UP_EXERCISE_NAME)43)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_PUSH_UP_PLUS \
  ((FIT_PUSH_UP_EXERCISE_NAME)44)
#define FIT_PUSH_UP_EXERCISE_NAME_PUSH_UP_WITH_FEET_ON_SWISS_BALL \
  ((FIT_PUSH_UP_EXERCISE_NAME)45)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_PUSH_UP_WITH_FEET_ON_SWISS_BALL \
  ((FIT_PUSH_UP_EXERCISE_NAME)46)
#define FIT_PUSH_UP_EXERCISE_NAME_PUSH_UP_WITH_ONE_HAND_ON_MEDICINE_BALL \
  ((FIT_PUSH_UP_EXERCISE_NAME)47)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_PUSH_UP_WITH_ONE_HAND_ON_MEDICINE_BALL \
  ((FIT_PUSH_UP_EXERCISE_NAME)48)
#define FIT_PUSH_UP_EXERCISE_NAME_SHOULDER_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)49)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_SHOULDER_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)50)
#define FIT_PUSH_UP_EXERCISE_NAME_SINGLE_ARM_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)51)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_SINGLE_ARM_MEDICINE_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)52)
#define FIT_PUSH_UP_EXERCISE_NAME_SPIDERMAN_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)53)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_SPIDERMAN_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)54)
#define FIT_PUSH_UP_EXERCISE_NAME_STACKED_FEET_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)55)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_STACKED_FEET_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)56)
#define FIT_PUSH_UP_EXERCISE_NAME_STAGGERED_HANDS_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)57)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_STAGGERED_HANDS_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)58)
#define FIT_PUSH_UP_EXERCISE_NAME_SUSPENDED_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)59)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_SUSPENDED_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)60)
#define FIT_PUSH_UP_EXERCISE_NAME_SWISS_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)61)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)62)
#define FIT_PUSH_UP_EXERCISE_NAME_SWISS_BALL_PUSH_UP_PLUS \
  ((FIT_PUSH_UP_EXERCISE_NAME)63)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_SWISS_BALL_PUSH_UP_PLUS \
  ((FIT_PUSH_UP_EXERCISE_NAME)64)
#define FIT_PUSH_UP_EXERCISE_NAME_T_PUSH_UP ((FIT_PUSH_UP_EXERCISE_NAME)65)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_T_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)66)
#define FIT_PUSH_UP_EXERCISE_NAME_TRIPLE_STOP_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)67)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_TRIPLE_STOP_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)68)
#define FIT_PUSH_UP_EXERCISE_NAME_WIDE_HANDS_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)69)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_WIDE_HANDS_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)70)
#define FIT_PUSH_UP_EXERCISE_NAME_PARALLETTE_HANDSTAND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)71)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_PARALLETTE_HANDSTAND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)72)
#define FIT_PUSH_UP_EXERCISE_NAME_RING_HANDSTAND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)73)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_RING_HANDSTAND_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)74)
#define FIT_PUSH_UP_EXERCISE_NAME_RING_PUSH_UP ((FIT_PUSH_UP_EXERCISE_NAME)75)
#define FIT_PUSH_UP_EXERCISE_NAME_WEIGHTED_RING_PUSH_UP \
  ((FIT_PUSH_UP_EXERCISE_NAME)76)
...
#define FIT_ROW_EXERCISE_NAME_BARBELL_STRAIGHT_LEG_DEADLIFT_TO_ROW \
  ((FIT_ROW_EXERCISE_NAME)0)
...
#define FIT_ROW_EXERCISE_NAME_ELEVATED_FEET_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)3)
#define FIT_ROW_EXERCISE_NAME_WEIGHTED_ELEVATED_FEET_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)4)
#define FIT_ROW_EXERCISE_NAME_FACE_PULL ((FIT_ROW_EXERCISE_NAME)5)
#define FIT_ROW_EXERCISE_NAME_FACE_PULL_WITH_EXTERNAL_ROTATION \
  ((FIT_ROW_EXERCISE_NAME)6)
#define FIT_ROW_EXERCISE_NAME_INVERTED_ROW_WITH_FEET_ON_SWISS_BALL \
  ((FIT_ROW_EXERCISE_NAME)7)
#define FIT_ROW_EXERCISE_NAME_WEIGHTED_INVERTED_ROW_WITH_FEET_ON_SWISS_BALL \
  ((FIT_ROW_EXERCISE_NAME)8)
...
#define FIT_ROW_EXERCISE_NAME_WEIGHTED_MODIFIED_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)11)
#define FIT_ROW_EXERCISE_NAME_NEUTRAL_GRIP_ALTERNATING_DUMBBELL_ROW \
  ((FIT_ROW_EXERCISE_NAME)12)
#define FIT_ROW_EXERCISE_NAME_ONE_ARM_BENT_OVER_ROW ((FIT_ROW_EXERCISE_NAME)13)
#define FIT_ROW_EXERCISE_NAME_ONE_LEGGED_DUMBBELL_ROW \
  ((FIT_ROW_EXERCISE_NAME)14)
#define FIT_ROW_EXERCISE_NAME_RENEGADE_ROW ((FIT_ROW_EXERCISE_NAME)15)
#define FIT_ROW_EXERCISE_NAME_REVERSE_GRIP_BARBELL_ROW \
  ((FIT_ROW_EXERCISE_NAME)16)
...
#define FIT_ROW_EXERCISE_NAME_SINGLE_ARM_CABLE_ROW_AND_ROTATION \
  ((FIT_ROW_EXERCISE_NAME)21)
#define FIT_ROW_EXERCISE_NAME_SINGLE_ARM_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)22)
#define FIT_ROW_EXERCISE_NAME_WEIGHTED_SINGLE_ARM_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)23)
#define FIT_ROW_EXERCISE_NAME_SINGLE_ARM_NEUTRAL_GRIP_DUMBBELL_ROW \
  ((FIT_ROW_EXERCISE_NAME)24)
#define FIT_ROW_EXERCISE_NAME_SINGLE_ARM_NEUTRAL_GRIP_DUMBBELL_ROW_AND_ROTATION \
  ((FIT_ROW_EXERCISE_NAME)25)
#define FIT_ROW_EXERCISE_NAME_SUSPENDED_INVERTED_ROW ((FIT_ROW_EXERCISE_NAME)26)
#define FIT_ROW_EXERCISE_NAME_WEIGHTED_SUSPENDED_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)27)
#define FIT_ROW_EXERCISE_NAME_T_BAR_ROW ((FIT_ROW_EXERCISE_NAME)28)
#define FIT_ROW_EXERCISE_NAME_TOWEL_GRIP_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)29)
#define FIT_ROW_EXERCISE_NAME_WEIGHTED_TOWEL_GRIP_INVERTED_ROW \
  ((FIT_ROW_EXERCISE_NAME)30)
#define FIT_ROW_EXERCISE_NAME_UNDERHAND_GRIP_CABLE_ROW \
  ((FIT_ROW_EXERCISE_NAME)31)
#define FIT_ROW_EXERCISE_NAME_V_GRIP_CABLE_ROW ((FIT_ROW_EXERCISE_NAME)32)
#define FIT_ROW_EXERCISE_NAME_WIDE_GRIP_SEATED_CABLE_ROW \
  ((FIT_ROW_EXERCISE_NAME)33)
...
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_ALTERNATING_DUMBBELL_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)0)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_ARNOLD_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)1)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_BARBELL_FRONT_SQUAT_TO_PUSH_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)2)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_BARBELL_PUSH_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)3)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_BARBELL_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)4)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_DEAD_CURL_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)5)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_DUMBBELL_ALTERNATING_SHOULDER_PRESS_AND_TWIST \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)6)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_DUMBBELL_HAMMER_CURL_TO_LUNGE_TO_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)7)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_DUMBBELL_PUSH_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)8)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_FLOOR_INVERTED_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)9)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_WEIGHTED_FLOOR_INVERTED_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)10)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_INVERTED_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)11)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_WEIGHTED_INVERTED_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)12)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_ONE_ARM_PUSH_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)13)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_OVERHEAD_BARBELL_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)14)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_OVERHEAD_DUMBBELL_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)15)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_SEATED_BARBELL_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)16)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_SEATED_DUMBBELL_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)17)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_SINGLE_ARM_DUMBBELL_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)18)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_SINGLE_ARM_STEP_UP_AND_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)19)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_SMITH_MACHINE_OVERHEAD_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)20)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_SPLIT_STANCE_HAMMER_CURL_TO_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)21)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_SWISS_BALL_DUMBBELL_SHOULDER_PRESS \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)22)
#define FIT_SHOULDER_PRESS_EXERCISE_NAME_WEIGHT_PLATE_FRONT_RAISE \
  ((FIT_SHOULDER_PRESS_EXERCISE_NAME)23)
...
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_90_DEGREE_CABLE_EXTERNAL_ROTATION \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)0)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_BAND_EXTERNAL_ROTATION \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)1)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_BAND_INTERNAL_ROTATION \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)2)
...
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_CABLE_EXTERNAL_ROTATION \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)4)
...
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_FLOOR_I_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)6)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_FLOOR_I_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)7)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_FLOOR_T_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)8)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_FLOOR_T_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)9)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_FLOOR_Y_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)10)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_FLOOR_Y_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)11)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_INCLINE_I_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)12)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_INCLINE_I_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)13)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_INCLINE_L_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)14)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_INCLINE_L_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)15)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_INCLINE_T_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)16)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_INCLINE_T_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)17)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_INCLINE_W_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)18)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_INCLINE_W_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)19)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_INCLINE_Y_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)20)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_INCLINE_Y_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)21)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_LYING_EXTERNAL_ROTATION \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)22)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_SEATED_DUMBBELL_EXTERNAL_ROTATION \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)23)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_STANDING_L_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)24)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_SWISS_BALL_I_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)25)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_SWISS_BALL_I_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)26)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_SWISS_BALL_T_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)27)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_SWISS_BALL_T_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)28)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_SWISS_BALL_W_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)29)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_SWISS_BALL_W_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)30)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_SWISS_BALL_Y_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)31)
#define FIT_SHOULDER_STABILITY_EXERCISE_NAME_WEIGHTED_SWISS_BALL_Y_RAISE \
  ((FIT_SHOULDER_STABILITY_EXERCISE_NAME)32)
...
#define FIT_SHRUG_EXERCISE_NAME_BEHIND_THE_BACK_SMITH_MACHINE_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)3)
...
#define FIT_SHRUG_EXERCISE_NAME_DUMBBELL_UPRIGHT_ROW \
  ((FIT_SHRUG_EXERCISE_NAME)6)
#define FIT_SHRUG_EXERCISE_NAME_INCLINE_DUMBBELL_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)7)
#define FIT_SHRUG_EXERCISE_NAME_OVERHEAD_BARBELL_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)8)
#define FIT_SHRUG_EXERCISE_NAME_OVERHEAD_DUMBBELL_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)9)
#define FIT_SHRUG_EXERCISE_NAME_SCAPTION_AND_SHRUG ((FIT_SHRUG_EXERCISE_NAME)10)
#define FIT_SHRUG_EXERCISE_NAME_SCAPULAR_RETRACTION \
  ((FIT_SHRUG_EXERCISE_NAME)11)
#define FIT_SHRUG_EXERCISE_NAME_SERRATUS_CHAIR_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)12)
#define FIT_SHRUG_EXERCISE_NAME_WEIGHTED_SERRATUS_CHAIR_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)13)
#define FIT_SHRUG_EXERCISE_NAME_SERRATUS_SHRUG ((FIT_SHRUG_EXERCISE_NAME)14)
#define FIT_SHRUG_EXERCISE_NAME_WEIGHTED_SERRATUS_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)15)
#define FIT_SHRUG_EXERCISE_NAME_WIDE_GRIP_JUMP_SHRUG \
  ((FIT_SHRUG_EXERCISE_NAME)16)
...
#define FIT_SIT_UP_EXERCISE_NAME_ALTERNATING_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)0)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_ALTERNATING_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)1)
#define FIT_SIT_UP_EXERCISE_NAME_BENT_KNEE_V_UP ((FIT_SIT_UP_EXERCISE_NAME)2)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_BENT_KNEE_V_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)3)
#define FIT_SIT_UP_EXERCISE_NAME_BUTTERFLY_SIT_UP ((FIT_SIT_UP_EXERCISE_NAME)4)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_BUTTERFLY_SITUP \
  ((FIT_SIT_UP_EXERCISE_NAME)5)
#define FIT_SIT_UP_EXERCISE_NAME_CROSS_PUNCH_ROLL_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)6)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_CROSS_PUNCH_ROLL_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)7)
#define FIT_SIT_UP_EXERCISE_NAME_CROSSED_ARMS_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)8)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_CROSSED_ARMS_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)9)
#define FIT_SIT_UP_EXERCISE_NAME_GET_UP_SIT_UP ((FIT_SIT_UP_EXERCISE_NAME)10)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_GET_UP_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)11)
#define FIT_SIT_UP_EXERCISE_NAME_HOVERING_SIT_UP ((FIT_SIT_UP_EXERCISE_NAME)12)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_HOVERING_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)13)
#define FIT_SIT_UP_EXERCISE_NAME_KETTLEBELL_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)14)
#define FIT_SIT_UP_EXERCISE_NAME_MEDICINE_BALL_ALTERNATING_V_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)15)
#define FIT_SIT_UP_EXERCISE_NAME_MEDICINE_BALL_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)16)
#define FIT_SIT_UP_EXERCISE_NAME_MEDICINE_BALL_V_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)17)
...
#define FIT_SIT_UP_EXERCISE_NAME_ONE_ARM_FULL_SIT_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)20)
#define FIT_SIT_UP_EXERCISE_NAME_RECLINING_CIRCLE ((FIT_SIT_UP_EXERCISE_NAME)21)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_RECLINING_CIRCLE \
  ((FIT_SIT_UP_EXERCISE_NAME)22)
#define FIT_SIT_UP_EXERCISE_NAME_REVERSE_CURL_UP ((FIT_SIT_UP_EXERCISE_NAME)23)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_REVERSE_CURL_UP \
  ((FIT_SIT_UP_EXERCISE_NAME)24)
#define FIT_SIT_UP_EXERCISE_NAME_SINGLE_LEG_SWISS_BALL_JACKKNIFE \
  ((FIT_SIT_UP_EXERCISE_NAME)25)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_SWISS_BALL_JACKKNIFE \
  ((FIT_SIT_UP_EXERCISE_NAME)26)
#define FIT_SIT_UP_EXERCISE_NAME_THE_TEASER ((FIT_SIT_UP_EXERCISE_NAME)27)
#define FIT_SIT_UP_EXERCISE_NAME_THE_TEASER_WEIGHTED \
  ((FIT_SIT_UP_EXERCISE_NAME)28)
#define FIT_SIT_UP_EXERCISE_NAME_THREE_PART_ROLL_DOWN \
  ((FIT_SIT_UP_EXERCISE_NAME)29)
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_THREE_PART_ROLL_DOWN \
  ((FIT_SIT_UP_EXERCISE_NAME)30)
...
#define FIT_SIT_UP_EXERCISE_NAME_WEIGHTED_RUSSIAN_TWIST_ON_SWISS_BALL \
  ((FIT_SIT_UP_EXERCISE_NAME)33)
...
#define FIT_SQUAT_EXERCISE_NAME_BACK_SQUAT_WITH_BODY_BAR \
  ((FIT_SQUAT_EXERCISE_NAME)1)
#define FIT_SQUAT_EXERCISE_NAME_BACK_SQUATS ((FIT_SQUAT_EXERCISE_NAME)2)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_BACK_SQUATS \
  ((FIT_SQUAT_EXERCISE_NAME)3)
#define FIT_SQUAT_EXERCISE_NAME_BALANCING_SQUAT ((FIT_SQUAT_EXERCISE_NAME)4)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_BALANCING_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)5)
...
#define FIT_SQUAT_EXERCISE_NAME_BARBELL_HANG_SQUAT_SNATCH \
  ((FIT_SQUAT_EXERCISE_NAME)10)
#define FIT_SQUAT_EXERCISE_NAME_BARBELL_LATERAL_STEP_UP \
  ((FIT_SQUAT_EXERCISE_NAME)11)
#define FIT_SQUAT_EXERCISE_NAME_BARBELL_QUARTER_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)12)
#define FIT_SQUAT_EXERCISE_NAME_BARBELL_SIFF_SQUAT ((FIT_SQUAT_EXERCISE_NAME)13)
#define FIT_SQUAT_EXERCISE_NAME_BARBELL_SQUAT_SNATCH \
  ((FIT_SQUAT_EXERCISE_NAME)14)
#define FIT_SQUAT_EXERCISE_NAME_BARBELL_SQUAT_WITH_HEELS_RAISED \
  ((FIT_SQUAT_EXERCISE_NAME)15)
...
#define FIT_SQUAT_EXERCISE_NAME_BENCH_SQUAT_WITH_ROTATIONAL_CHOP \
  ((FIT_SQUAT_EXERCISE_NAME)18)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_BENCH_SQUAT_WITH_ROTATIONAL_CHOP \
  ((FIT_SQUAT_EXERCISE_NAME)19)
#define FIT_SQUAT_EXERCISE_NAME_BODY_WEIGHT_WALL_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)20)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_WALL_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)21)
#define FIT_SQUAT_EXERCISE_NAME_BOX_STEP_SQUAT ((FIT_SQUAT_EXERCISE_NAME)22)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_BOX_STEP_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)23)
#define FIT_SQUAT_EXERCISE_NAME_BRACED_SQUAT ((FIT_SQUAT_EXERCISE_NAME)24)
#define FIT_SQUAT_EXERCISE_NAME_CROSSED_ARM_BARBELL_FRONT_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)25)
#define FIT_SQUAT_EXERCISE_NAME_CROSSOVER_DUMBBELL_STEP_UP \
  ((FIT_SQUAT_EXERCISE_NAME)26)
#define FIT_SQUAT_EXERCISE_NAME_DUMBBELL_FRONT_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)27)
#define FIT_SQUAT_EXERCISE_NAME_DUMBBELL_SPLIT_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)28)
#define FIT_SQUAT_EXERCISE_NAME_DUMBBELL_SQUAT ((FIT_SQUAT_EXERCISE_NAME)29)
#define FIT_SQUAT_EXERCISE_NAME_DUMBBELL_SQUAT_CLEAN \
  ((FIT_SQUAT_EXERCISE_NAME)30)
...
#define FIT_SQUAT_EXERCISE_NAME_ELEVATED_SINGLE_LEG_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)33)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_ELEVATED_SINGLE_LEG_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)34)
#define FIT_SQUAT_EXERCISE_NAME_FIGURE_FOUR_SQUATS ((FIT_SQUAT_EXERCISE_NAME)35)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_FIGURE_FOUR_SQUATS \
  ((FIT_SQUAT_EXERCISE_NAME)36)
...
#define FIT_SQUAT_EXERCISE_NAME_KETTLEBELL_SWING_OVERHEAD \
  ((FIT_SQUAT_EXERCISE_NAME)39)
#define FIT_SQUAT_EXERCISE_NAME_KETTLEBELL_SWING_WITH_FLIP_TO_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)40)
#define FIT_SQUAT_EXERCISE_NAME_LATERAL_DUMBBELL_STEP_UP \
  ((FIT_SQUAT_EXERCISE_NAME)41)
#define FIT_SQUAT_EXERCISE_NAME_ONE_LEGGED_SQUAT ((FIT_SQUAT_EXERCISE_NAME)42)
#define FIT_SQUAT_EXERCISE_NAME_OVERHEAD_DUMBBELL_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)43)
#define FIT_SQUAT_EXERCISE_NAME_OVERHEAD_SQUAT ((FIT_SQUAT_EXERCISE_NAME)44)
#define FIT_SQUAT_EXERCISE_NAME_PARTIAL_SINGLE_LEG_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)45)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_PARTIAL_SINGLE_LEG_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)46)
#define FIT_SQUAT_EXERCISE_NAME_PISTOL_SQUAT ((FIT_SQUAT_EXERCISE_NAME)47)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_PISTOL_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)48)
#define FIT_SQUAT_EXERCISE_NAME_PLIE_SLIDES ((FIT_SQUAT_EXERCISE_NAME)49)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_PLIE_SLIDES \
  ((FIT_SQUAT_EXERCISE_NAME)50)
#define FIT_SQUAT_EXERCISE_NAME_PLIE_SQUAT ((FIT_SQUAT_EXERCISE_NAME)51)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_PLIE_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)52)
#define FIT_SQUAT_EXERCISE_NAME_PRISONER_SQUAT ((FIT_SQUAT_EXERCISE_NAME)53)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_PRISONER_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)54)
#define FIT_SQUAT_EXERCISE_NAME_SINGLE_LEG_BENCH_GET_UP \
  ((FIT_SQUAT_EXERCISE_NAME)55)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_BENCH_GET_UP \
  ((FIT_SQUAT_EXERCISE_NAME)56)
#define FIT_SQUAT_EXERCISE_NAME_SINGLE_LEG_BENCH_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)57)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_BENCH_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)58)
#define FIT_SQUAT_EXERCISE_NAME_SINGLE_LEG_SQUAT_ON_SWISS_BALL \
  ((FIT_SQUAT_EXERCISE_NAME)59)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_SQUAT_ON_SWISS_BALL \
  ((FIT_SQUAT_EXERCISE_NAME)60)
...
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_STAGGERED_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)65)
...
#define FIT_SQUAT_EXERCISE_NAME_SUMO_SQUAT_SLIDE_IN \
  ((FIT_SQUAT_EXERCISE_NAME)70)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_SUMO_SQUAT_SLIDE_IN \
  ((FIT_SQUAT_EXERCISE_NAME)71)
#define FIT_SQUAT_EXERCISE_NAME_SUMO_SQUAT_TO_HIGH_PULL \
  ((FIT_SQUAT_EXERCISE_NAME)72)
#define FIT_SQUAT_EXERCISE_NAME_SUMO_SQUAT_TO_STAND \
  ((FIT_SQUAT_EXERCISE_NAME)73)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_SUMO_SQUAT_TO_STAND \
  ((FIT_SQUAT_EXERCISE_NAME)74)
#define FIT_SQUAT_EXERCISE_NAME_SUMO_SQUAT_WITH_ROTATION \
  ((FIT_SQUAT_EXERCISE_NAME)75)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_SUMO_SQUAT_WITH_ROTATION \
  ((FIT_SQUAT_EXERCISE_NAME)76)
#define FIT_SQUAT_EXERCISE_NAME_SWISS_BALL_BODY_WEIGHT_WALL_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)77)
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_SWISS_BALL_WALL_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)78)
...
#define FIT_SQUAT_EXERCISE_NAME_WEIGHTED_UNEVEN_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)81)
#define FIT_SQUAT_EXERCISE_NAME_WAIST_SLIMMING_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)82)
#define FIT_SQUAT_EXERCISE_NAME_WALL_BALL ((FIT_SQUAT_EXERCISE_NAME)83)
#define FIT_SQUAT_EXERCISE_NAME_WIDE_STANCE_BARBELL_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)84)
#define FIT_SQUAT_EXERCISE_NAME_WIDE_STANCE_GOBLET_SQUAT \
  ((FIT_SQUAT_EXERCISE_NAME)85)
#define FIT_SQUAT_EXERCISE_NAME_ZERCHER_SQUAT ((FIT_SQUAT_EXERCISE_NAME)86)
#define FIT_SQUAT_EXERCISE_NAME_KBS_OVERHEAD \
  ((FIT_SQUAT_EXERCISE_NAME)87)  // Deprecated do not use
#define FIT_SQUAT_EXERCISE_NAME_SQUAT_AND_SIDE_KICK \
  ((FIT_SQUAT_EXERCISE_NAME)88)
#define FIT_SQUAT_EXERCISE_NAME_SQUAT_JUMPS_IN_N_OUT \
  ((FIT_SQUAT_EXERCISE_NAME)89)
#define FIT_SQUAT_EXERCISE_NAME_PILATES_PLIE_SQUATS_PARALLEL_TURNED_OUT_FLAT_AND_HEELS \
  ((FIT_SQUAT_EXERCISE_NAME)90)
#define FIT_SQUAT_EXERCISE_NAME_RELEVE_STRAIGHT_LEG_AND_KNEE_BENT_WITH_ONE_LEG_VARIATION \
  ((FIT_SQUAT_EXERCISE_NAME)91)
...
#define FIT_TOTAL_BODY_EXERCISE_NAME_WEIGHTED_BURPEE \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)1)
#define FIT_TOTAL_BODY_EXERCISE_NAME_BURPEE_BOX_JUMP \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)2)
#define FIT_TOTAL_BODY_EXERCISE_NAME_WEIGHTED_BURPEE_BOX_JUMP \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)3)
#define FIT_TOTAL_BODY_EXERCISE_NAME_HIGH_PULL_BURPEE \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)4)
#define FIT_TOTAL_BODY_EXERCISE_NAME_MAN_MAKERS \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)5)
#define FIT_TOTAL_BODY_EXERCISE_NAME_ONE_ARM_BURPEE \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)6)
#define FIT_TOTAL_BODY_EXERCISE_NAME_SQUAT_THRUSTS \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)7)
#define FIT_TOTAL_BODY_EXERCISE_NAME_WEIGHTED_SQUAT_THRUSTS \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)8)
#define FIT_TOTAL_BODY_EXERCISE_NAME_SQUAT_PLANK_PUSH_UP \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)9)
#define FIT_TOTAL_BODY_EXERCISE_NAME_WEIGHTED_SQUAT_PLANK_PUSH_UP \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)10)
#define FIT_TOTAL_BODY_EXERCISE_NAME_STANDING_T_ROTATION_BALANCE \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)11)
#define FIT_TOTAL_BODY_EXERCISE_NAME_WEIGHTED_STANDING_T_ROTATION_BALANCE \
  ((FIT_TOTAL_BODY_EXERCISE_NAME)12)
...
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_BENCH_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)0)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_WEIGHTED_BENCH_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)1)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_BODY_WEIGHT_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)2)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_CABLE_KICKBACK \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)3)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_CABLE_LYING_TRICEPS_EXTENSION \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)4)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_CABLE_OVERHEAD_TRICEPS_EXTENSION \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)5)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_DUMBBELL_KICKBACK \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)6)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_DUMBBELL_LYING_TRICEPS_EXTENSION \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)7)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_EZ_BAR_OVERHEAD_TRICEPS_EXTENSION \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)8)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_INCLINE_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)9)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_WEIGHTED_INCLINE_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)10)
...
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_LYING_EZ_BAR_TRICEPS_EXTENSION \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)13)
...
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_RECLINING_TRICEPS_PRESS \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)16)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_REVERSE_GRIP_PRESSDOWN \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)17)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_REVERSE_GRIP_TRICEPS_PRESSDOWN \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)18)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_ROPE_PRESSDOWN \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)19)
...
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_SINGLE_LEG_BENCH_DIP_AND_KICK \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)26)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_BENCH_DIP_AND_KICK \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)27)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_SINGLE_LEG_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)28)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_WEIGHTED_SINGLE_LEG_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)29)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_STATIC_LYING_TRICEPS_EXTENSION \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)30)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_SUSPENDED_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)31)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_WEIGHTED_SUSPENDED_DIP \
...
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_TABLETOP_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)36)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_WEIGHTED_TABLETOP_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)37)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_TRICEPS_EXTENSION_ON_FLOOR \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)38)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_TRICEPS_PRESSDOWN \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)39)
#define FIT_TRICEPS_EXTENSION_EXERCISE_NAME_WEIGHTED_DIP \
  ((FIT_TRICEPS_EXTENSION_EXERCISE_NAME)40)
...
#define FIT_WARM_UP_EXERCISE_NAME_QUADRUPED_ROCKING \
  ((FIT_WARM_UP_EXERCISE_NAME)0)
...
#define FIT_WARM_UP_EXERCISE_NAME_ANKLE_DORSIFLEXION_WITH_BAND \
  ((FIT_WARM_UP_EXERCISE_NAME)3)
#define FIT_WARM_UP_EXERCISE_NAME_ANKLE_INTERNAL_ROTATION \
  ((FIT_WARM_UP_EXERCISE_NAME)4)
#define FIT_WARM_UP_EXERCISE_NAME_ARM_CIRCLES ((FIT_WARM_UP_EXERCISE_NAME)5)
#define FIT_WARM_UP_EXERCISE_NAME_BENT_OVER_REACH_TO_SKY \
  ((FIT_WARM_UP_EXERCISE_NAME)6)
#define FIT_WARM_UP_EXERCISE_NAME_CAT_CAMEL ((FIT_WARM_UP_EXERCISE_NAME)7)
#define FIT_WARM_UP_EXERCISE_NAME_ELBOW_TO_FOOT_LUNGE \
  ((FIT_WARM_UP_EXERCISE_NAME)8)
#define FIT_WARM_UP_EXERCISE_NAME_FORWARD_AND_BACKWARD_LEG_SWINGS \
  ((FIT_WARM_UP_EXERCISE_NAME)9)
#define FIT_WARM_UP_EXERCISE_NAME_GROINERS ((FIT_WARM_UP_EXERCISE_NAME)10)
#define FIT_WARM_UP_EXERCISE_NAME_INVERTED_HAMSTRING_STRETCH \
  ((FIT_WARM_UP_EXERCISE_NAME)11)
#define FIT_WARM_UP_EXERCISE_NAME_LATERAL_DUCK_UNDER \
  ((FIT_WARM_UP_EXERCISE_NAME)12)
#define FIT_WARM_UP_EXERCISE_NAME_NECK_ROTATIONS ((FIT_WARM_UP_EXERCISE_NAME)13)
#define FIT_WARM_UP_EXERCISE_NAME_OPPOSITE_ARM_AND_LEG_BALANCE \
  ((FIT_WARM_UP_EXERCISE_NAME)14)
#define FIT_WARM_UP_EXERCISE_NAME_REACH_ROLL_AND_LIFT \
  ((FIT_WARM_UP_EXERCISE_NAME)15)
#define FIT_WARM_UP_EXERCISE_NAME_SCORPION \
  ((FIT_WARM_UP_EXERCISE_NAME)16)  // Deprecated do not use
#define FIT_WARM_UP_EXERCISE_NAME_SHOULDER_CIRCLES \
  ((FIT_WARM_UP_EXERCISE_NAME)17)
#define FIT_WARM_UP_EXERCISE_NAME_SIDE_TO_SIDE_LEG_SWINGS \
  ((FIT_WARM_UP_EXERCISE_NAME)18)
#define FIT_WARM_UP_EXERCISE_NAME_SLEEPER_STRETCH \
  ((FIT_WARM_UP_EXERCISE_NAME)19)
#define FIT_WARM_UP_EXERCISE_NAME_SLIDE_OUT ((FIT_WARM_UP_EXERCISE_NAME)20)
#define FIT_WARM_UP_EXERCISE_NAME_SWISS_BALL_HIP_CROSSOVER \
  ((FIT_WARM_UP_EXERCISE_NAME)21)
#define FIT_WARM_UP_EXERCISE_NAME_SWISS_BALL_REACH_ROLL_AND_LIFT \
  ((FIT_WARM_UP_EXERCISE_NAME)22)
#define FIT_WARM_UP_EXERCISE_NAME_SWISS_BALL_WINDSHIELD_WIPERS \
  ((FIT_WARM_UP_EXERCISE_NAME)23)
#define FIT_WARM_UP_EXERCISE_NAME_THORACIC_ROTATION \
  ((FIT_WARM_UP_EXERCISE_NAME)24)
#define FIT_WARM_UP_EXERCISE_NAME_WALKING_HIGH_KICKS \
  ((FIT_WARM_UP_EXERCISE_NAME)25)
#define FIT_WARM_UP_EXERCISE_NAME_WALKING_HIGH_KNEES \
  ((FIT_WARM_UP_EXERCISE_NAME)26)
#define FIT_WARM_UP_EXERCISE_NAME_WALKING_KNEE_HUGS \
  ((FIT_WARM_UP_EXERCISE_NAME)27)
#define FIT_WARM_UP_EXERCISE_NAME_WALKING_LEG_CRADLES \
  ((FIT_WARM_UP_EXERCISE_NAME)28)
#define FIT_WARM_UP_EXERCISE_NAME_WALKOUT ((FIT_WARM_UP_EXERCISE_NAME)29)
#define FIT_WARM_UP_EXERCISE_NAME_WALKOUT_FROM_PUSH_UP_POSITION \
  ((FIT_WARM_UP_EXERCISE_NAME)30)
...
#define FIT_TISSUE_MODEL_TYPE_ZHL_16C \
  ((FIT_TISSUE_MODEL_TYPE)0)  // Buhlmann's decompression algorithm, version C
...
#define FIT_DIVE_ALERT_PO2_CCR_DIL_LOW \
  ((FIT_DIVE_ALERT)33)  // ccr diluent has low po2
#define FIT_DIVE_ALERT_DECO_STOP_CLEARED \
  ((FIT_DIVE_ALERT)34)  // a deco stop has been cleared
#define FIT_DIVE_ALERT_APNEA_NEUTRAL_BUOYANCY \
  ((FIT_DIVE_ALERT)35)  // Target Depth Apnea Alarm triggered
#define FIT_DIVE_ALERT_APNEA_TARGET_DEPTH \
  ((FIT_DIVE_ALERT)36)  // Neutral Buoyance Apnea Alarm triggered
#define FIT_DIVE_ALERT_APNEA_SURFACE \
  ((FIT_DIVE_ALERT)37)  // Surface Apnea Alarm triggered
#define FIT_DIVE_ALERT_APNEA_HIGH_SPEED \
  ((FIT_DIVE_ALERT)38)  // High Speed Apnea Alarm triggered
#define FIT_DIVE_ALERT_APNEA_LOW_SPEED \
  ((FIT_DIVE_ALERT)39)  // Low Speed Apnea Alarm triggered
#define FIT_DIVE_ALERT_COUNT 39

typedef FIT_ENUM FIT_DIVE_ALARM_TYPE;
#define FIT_DIVE_ALARM_TYPE_INVALID FIT_ENUM_INVALID
#define FIT_DIVE_ALARM_TYPE_DEPTH \
  ((FIT_DIVE_ALARM_TYPE)0)  // Alarm when a certain depth is crossed
#define FIT_DIVE_ALARM_TYPE_TIME \
  ((FIT_DIVE_ALARM_TYPE)1)  // Alarm when a certain time has transpired
#define FIT_DIVE_ALARM_TYPE_SPEED \
  ((FIT_DIVE_ALARM_TYPE)2)  // Alarm when a certain ascent or descent rate is
                            // exceeded
...
#define FIT_SPO2_MEASUREMENT_TYPE_CONTINUOUS_CHECK \
  ((FIT_SPO2_MEASUREMENT_TYPE)2)
...
#define FIT_CCR_SETPOINT_SWITCH_MODE_MANUAL \
  ((FIT_CCR_SETPOINT_SWITCH_MODE)0)  // User switches setpoints manually
#define FIT_CCR_SETPOINT_SWITCH_MODE_AUTOMATIC \
  ((FIT_CCR_SETPOINT_SWITCH_MODE)1)  // Switch automatically based on depth
...
#define FIT_SPLIT_TYPE_RWD_RUN \
  ((FIT_SPLIT_TYPE)17)  // run/walk detection running
#define FIT_SPLIT_TYPE_RWD_WALK \
  ((FIT_SPLIT_TYPE)18)  // run/walk detection walking
#define FIT_SPLIT_TYPE_WINDSURF_ACTIVE ((FIT_SPLIT_TYPE)21)
#define FIT_SPLIT_TYPE_RWD_STAND \
  ((FIT_SPLIT_TYPE)22)  // run/walk detection standing
#define FIT_SPLIT_TYPE_TRANSITION \
  ((FIT_SPLIT_TYPE)23)  // Marks the time going from ascent_split to
...
#define FIT_GAS_CONSUMPTION_RATE_TYPE_RMV \
  ((FIT_GAS_CONSUMPTION_RATE_TYPE)2)  // Respiratory Minute Volume
...
#define FIT_RADAR_THREAT_LEVEL_TYPE_THREAT_UNKNOWN \
  ((FIT_RADAR_THREAT_LEVEL_TYPE)0)
#define FIT_RADAR_THREAT_LEVEL_TYPE_THREAT_NONE ((FIT_RADAR_THREAT_LEVEL_TYPE)1)
#define FIT_RADAR_THREAT_LEVEL_TYPE_THREAT_APPROACHING \
  ((FIT_RADAR_THREAT_LEVEL_TYPE)2)
#define FIT_RADAR_THREAT_LEVEL_TYPE_THREAT_APPROACHING_FAST \
  ((FIT_RADAR_THREAT_LEVEL_TYPE)3)
...
#define FIT_MAX_MET_HEART_RATE_SOURCE_WHR \
  ((FIT_MAX_MET_HEART_RATE_SOURCE)0)  // Wrist Heart Rate Monitor
#define FIT_MAX_MET_HEART_RATE_SOURCE_HRM \
  ((FIT_MAX_MET_HEART_RATE_SOURCE)1)  // Chest Strap Heart Rate Monitor
...
#define FIT_NO_FLY_TIME_MODE_STANDARD \
  ((FIT_NO_FLY_TIME_MODE)0)  // Standard Diver Alert Network no-fly guidance
#define FIT_NO_FLY_TIME_MODE_FLAT_24_HOURS \
  ((FIT_NO_FLY_TIME_MODE)1)  // Flat 24 hour no-fly guidance
...
class Profile {
 public:
  ...
  static const FIT_UINT16 GetFieldIndex(const FIT_UINT16 mesgNum,
                                        const FIT_UINT8 fieldNum);
  static const FIT_UINT16 GetFieldIndex(const std::string& mesgName,
                                        const std::string& fieldName);
  static const Profile::FIELD* GetField(const FIT_UINT16 mesgNum,
                                        const FIT_UINT8 fieldNum);
  static const Profile::FIELD* GetField(const std::string& mesgName,
                                        const std::string& fieldName);
  static const FIT_UINT16 GetSubFieldIndex(const std::string& mesgName,
                                           const std::string& fieldName,
                                           const std::string& subFieldName);
  static const Profile::SUBFIELD* GetSubField(const FIT_UINT16 mesgNum,
                                              const FIT_UINT8 fieldNum,
                                              const FIT_UINT16 subFieldIndex);
  static const Profile::SUBFIELD* GetSubField(const std::string& mesgName,
                                              const std::string& fieldName,
                                              const std::string& subFieldName);
};
