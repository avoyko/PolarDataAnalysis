...
DeveloperField::DeveloperField(const DeveloperField& other)
    : FieldBase(other), mDefinition(nullptr) {
...
DeveloperField::DeveloperField(const FieldDescriptionMesg& definition,
...
FIT_BOOL DeveloperField::GetIsAccumulated() const { return FIT_FALSE; }

FIT_BOOL DeveloperField::IsValid() const { return nullptr != mDefinition; }

FIT_UINT8 DeveloperField::GetNum() const { return mDefinition->GetNum(); }

std::string DeveloperField::GetName() const {
  if (mDefinition->IsDefined()) {
    return Unicode::Encode_BaseToUTF8(
        mDefinition->GetDescription().GetFieldName(0));
  }
...
std::string DeveloperField::GetUnits() const {
  if (mDefinition->IsDefined()) {
    return Unicode::Encode_BaseToUTF8(
        mDefinition->GetDescription().GetUnits(0));
  }
...
const Profile::SUBFIELD* DeveloperField::GetSubField(
...
FIT_UINT16 DeveloperField::GetNumSubFields() const { return 0; }

const Profile::FIELD_COMPONENT* DeveloperField::GetComponent(
    const FIT_UINT16 component) const {
  // Unused
  (void)component;
  return nullptr;
}

FIT_UINT16 DeveloperField::GetNumComponents() const { return 0; }
