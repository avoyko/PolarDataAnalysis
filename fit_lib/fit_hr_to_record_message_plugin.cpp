 ...
 * data contained in the HR mesg. <p> Requirements for correct operation:
 *    - HR data must be in the order of increasing timestamp
 *    - Record data must be in the order of increasing timestamp
 *    - The order of incoming HR and record mesgs may be independent of each
 * other
 *    - The first HR mesg must contain a timestamp (with optional
...
HRToRecordMesgBroadcastPlugin::~HRToRecordMesgBroadcastPlugin() {
  if (record_range_start_time != NULL) delete record_range_start_time;
}
/**
...
void HRToRecordMesgBroadcastPlugin::OnIncomingMesg(const Mesg& mesg) {
  switch (mesg.GetNum()) {
    case FIT_MESG_NUM_FILE_ID: {
      ...
      if (fileIdMesg.GetType() == FIT_FILE_ACTIVITY) isActivityFile = true;
    } break;
 ...
 * @param mesgs the message list that is about to be broadcast to all
 MesgListeners.  \
                                Note: The List is 'final' but the references
 within the list are not, \
                                therefore editing Mesg objects within mesgs will
 alter the messages   \ that are broadcast to listeners.
 *
...
void HRToRecordMesgBroadcastPlugin::OnBroadcast(std::vector<Mesg>& mesgs) {
  ...
  if (isActivityFile && (hr_start_index != INVALID_INDEX)) {
    ...
    for (FIT_UINT32 mesgCounter = 0; mesgCounter < mesgs.size();
         ++mesgCounter) {
      ...
      if (mesg.GetNum() == FIT_MESG_NUM_RECORD) {
        ...
        FIT_DATE_TIME timestamp =
            mesg.GetFieldUINT32Value(253, 0, FIT_SUBFIELD_INDEX_MAIN_FIELD);
        ...
        while (findingInRangeHrMesgs && (hr_mesg_counter < mesgs.size())) {
          ...
          if (mesgs.at(hr_mesg_counter).GetNum() == FIT_MESG_NUM_HR) {
            ...
            if (hrMesg.GetTimestamp() != FIT_UINT32_INVALID) {
              ...
              auto fit_float32_invalid = FIT_FLOAT32_INVALID;
              if (memcmp(&fractionalTimestamp, &fit_float32_invalid,
                         sizeof(FIT_FLOAT32)) != 0)
              ...
              if (hrMesg.GetNumEventTimestamp() == 1) {
                ...
                throw RuntimeException(
                    "FIT HrToRecordMesgBroadcastPlugin Error: Anchor HR mesg "
                    "must have 1 event_timestamp");
              }
            }

            if (hr_anchor_set == FIT_FALSE) {
              // We cannot process any HR messages if we have not received a
              // timestamp anchor
              throw RuntimeException(
                  "FIT HrToRecordMesgBroadcastPlugin Error: No anchor "
                  "timestamp received in a HR mesg before diff HR mesgs");
            } else if (hrMesg.GetNumEventTimestamp() !=
                       hrMesg.GetNumFilteredBpm()) {
              throw RuntimeException(
                  "FIT HrToRecordMesgBroadcastPlugin Error: HR mesg with "
                  "mismatching event timestamp and filtered bpm");
            }

            for (FIT_UINT8 j = (FIT_UINT8)hr_sub_mesg_counter;
                 j < hrMesg.GetNumEventTimestamp(); j++) {
              // Build up timestamp for each message using the anchor and
              // event_timestamp
              ...
              if (event_timestamp < hr_anchor_event_timestamp) {
                if ((hr_anchor_event_timestamp - event_timestamp) > (1 << 21)) {
                  ...
                  throw RuntimeException(
                      "FIT HrToRecordMesgBroadcastPlugin Error: Anchor "
                      "event_timestamp is greater than subsequent "
                      "event_timestamp. This does not allow for correct delta "
                      "calculation.");
                }
              }
              ...
              // check if hrMesgTime exceeds the record time
                ...
                if (hrSumCount > 0) {
                  ...
                  mesg.SetFieldUINT8Value(3, last_valid_hr, 0,
                                          FIT_SUBFIELD_INDEX_MAIN_FIELD);
                }
                // If no stored HR is available, fill in record messages with
                // the last valid filtered hr for a maximum of 5 seconds
                else if ((record_range_start_time->CompareTo(
                              last_valid_hr_time) > 0) &&
