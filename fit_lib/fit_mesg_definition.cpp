...
MesgDefinition::MesgDefinition(void)
    : num(FIT_MESG_NUM_INVALID), localNum(0), fields(), devFields() {}
...
FIT_UINT16 MesgDefinition::GetNum() const { return num; }

FIT_UINT8 MesgDefinition::GetLocalNum() const { return localNum; }

void MesgDefinition::SetNum(const FIT_UINT16 newNum) { num = newNum; }
...
int MesgDefinition::GetNumFields() const { return ((int)fields.size()); }

int MesgDefinition::GetNumDevFields() const { return ((int)devFields.size()); }

std::vector<FieldDefinition>& MesgDefinition::GetFields() { return fields; }
...
FieldDefinition* MesgDefinition::GetField(const FIT_UINT8 fieldNum) {
  for (int i = 0; i < (int)fields.size(); i++) {
    if (fieldNum == fields[i].GetNum()) return &(fields[i]);
  }
...
DeveloperFieldDefinition* MesgDefinition::GetDevFieldByIndex(
    const FIT_UINT16 index) {
  if (index < devFields.size()) return &devFields[index];
...
const std::vector<DeveloperFieldDefinition>& MesgDefinition::GetDevFields()
    const {
  return devFields;
}

const FieldDefinition* MesgDefinition::GetField(
...
const FieldDefinition* MesgDefinition::GetFieldByIndex(
    const FIT_UINT16 index) const {
  if (index < fields.size()) return &(fields[index]);

  return FIT_NULL;
}

const DeveloperFieldDefinition* MesgDefinition::GetDevFieldByIndex(
    const FIT_UINT16 index) const {
  if (index < devFields.size()) return &devFields[index];
...
FIT_BOOL MesgDefinition::operator==(const MesgDefinition& mesgDef) const {
  if (num != mesgDef.num) return FIT_FALSE;

  if (localNum != mesgDef.localNum) return FIT_FALSE;

  if (fields.size() != mesgDef.fields.size()) return FIT_FALSE;

  for (int i = 0; i < (int)fields.size(); i++) {
    if (fields[i] != mesgDef.fields[i]) return FIT_FALSE;
  }
...
FIT_BOOL MesgDefinition::Supports(const MesgDefinition& mesgDef) const {
  ...
  for (int i = 0; i < (int)mesgDef.fields.size(); i++) {
    const FieldDefinition* supportedFieldDef =
        GetField(mesgDef.fields[i].GetNum());

    if (supportedFieldDef ==
        FIT_NULL)  // Could not find field with matching number.
      return FIT_FALSE;

    if (mesgDef.fields[i].GetSize() >
        supportedFieldDef->GetSize())  // Other field definition is larger than
  ...
  for (int i = 0; i < (int)mesgDef.devFields.size(); i++) {
    ...
    const DeveloperFieldDefinition* supportedDef =
        GetDevField(fieldDef.GetDeveloperDataIndex(), fieldDef.GetNum());

    // There is a Field Definition that we don't have a description for
    if (supportedDef == FIT_NULL) return FIT_FALSE;

    // The definition is a larger size that we dont support
    if (fieldDef.GetSize() > supportedDef->GetSize()) return FIT_FALSE;
  }

  return FIT_TRUE;
}

const DeveloperFieldDefinition* MesgDefinition::GetDevField(
    const FIT_UINT8 developerIndex, const FIT_UINT8 num) const {
  for (int i = 0; i < (int)devFields.size(); i++) {
    if ((devFields[i].GetNum() == num) &&
        (devFields[i].GetDeveloperDataIndex() == developerIndex))
      return &(devFields[i]);
  }
  return FIT_NULL;
}
...
int MesgDefinition::Write(std::ostream& file) const {
  ...
  if (devFields.size() > 0) {
    file.put(
        (FIT_HDR_TYPE_DEF_BIT | FIT_HDR_DEV_FIELD_BIT) |
        (localNum & FIT_HDR_TYPE_MASK));  // Message definition record header
                                          // with local message number.
  } else {
    file.put(
        (FIT_HDR_TYPE_DEF_BIT) |
        (localNum & FIT_HDR_TYPE_MASK));  // Message definition record header
                                          // with local message number.
  }
  file.put(0);  // Reserved
