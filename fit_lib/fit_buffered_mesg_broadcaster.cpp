...
void BufferedMesgBroadcaster::RegisterMesgBroadcastPlugin(
    MesgBroadcastPlugin* plugin) {
...
void BufferedMesgBroadcaster::OnMesg(Mesg& mesg) {
  ...
  for (size_t i = 0; i < plugins.size(); i++) {
    if (plugins[i]) plugins[i]->OnIncomingMesg(mesg);
  }
}

void BufferedMesgBroadcaster::Broadcast(void) {
  for (size_t i = 0; i < plugins.size(); i++) {
    if (plugins[i]) plugins[i]->OnBroadcast(mesgs);
  }
