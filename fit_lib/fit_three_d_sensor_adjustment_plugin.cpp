      ...
      case FIT_MESG_NUM_ACCELEROMETER_DATA:
        if (haveAccelCal) {
          ...
          for (FIT_UINT8 j = 0; j < accelData.GetNumAccelX(); j++) {
            ...
            FIT_UINT8 fieldNums[3] = {X_AXIS_FIELD_NUM, Y_AXIS_FIELD_NUM,
                                      Z_AXIS_FIELD_NUM};
            ProcessCalibrationFactor(mesgs[i], (const FIT_UINT8*)fieldNums,
                                     (const FIT_FLOAT32*)&XYZvalues);
          }
        }
        break;
...
void ThreeDSensorAdjustmentPlugin::ProcessCalibrationFactor(
    fit::Mesg& mesg, const FIT_UINT8* fieldsXYZ,
  ...
  mesg.GetField(fieldsXYZ[X_AXIS_OFFSET])
      ->SetFLOAT32Value(calibratedXYZ[X_AXIS_OFFSET], index);
  mesg.GetField(fieldsXYZ[Y_AXIS_OFFSET])
      ->SetFLOAT32Value(calibratedXYZ[Y_AXIS_OFFSET], index);
  mesg.GetField(fieldsXYZ[Z_AXIS_OFFSET])
      ->SetFLOAT32Value(calibratedXYZ[Z_AXIS_OFFSET], index);
}

// load the calibration parameters from the incoming calibration message
void ThreeDSensorAdjustmentPlugin::CalibrationParameters::LoadParams(
    const fit::ThreeDSensorCalibrationMesg& calMesg) {
  ...
  for (FIT_UINT8 i = 0; i < numRows; i++)
    for (FIT_UINT8 j = 0; j < numColumns; j++)
      this->rotationMatrix[i][j] =
          calMesg.GetOrientationMatrix(i * numColumns + j);
}

void ThreeDSensorAdjustmentPlugin::adjustSensorData(
    FIT_FLOAT32 (&rawData)[3], CalibrationParameters calParams) {
  ...
  // [Rotation] * [XYZ]
  rawData[0] = calParams.rotationMatrix[0][0] * tempData[0] +
               calParams.rotationMatrix[0][1] * tempData[1] +
               calParams.rotationMatrix[0][2] * tempData[2];
  rawData[1] = calParams.rotationMatrix[1][0] * tempData[0] +
               calParams.rotationMatrix[1][1] * tempData[1] +
               calParams.rotationMatrix[1][2] * tempData[2];
  rawData[2] = calParams.rotationMatrix[2][0] * tempData[0] +
               calParams.rotationMatrix[2][1] * tempData[1] +
