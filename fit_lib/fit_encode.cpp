...
Encode::Encode(ProtocolVersion version)
    : dataSize(0), file(nullptr), version(version), validator(version) {}
...
void Encode::Write(const MesgDefinition& mesgDef) {
  ...
  if (!validator.ValidateMesgDefn(mesgDef)) return;
...
void Encode::Write(const Mesg& mesg) {
  ...
  if (!validator.ValidateMesg(mesg)) return;
...
FIT_BOOL Encode::Close(void) {
  ...
  while (fileBytesLeft > 0) {
    ...
    if (file->eof()) return FIT_FALSE;  // File smaller than expected.
...
void Encode::OnMesg(Mesg& mesg) { Write(mesg); }

void Encode::OnMesgDefinition(MesgDefinition& mesgDef) { Write(mesgDef); }
...
void Encode::WriteFileHeader() {
  ...
  file_header.crc =
      CRC::Calc16(&file_header, FIT_STRUCT_OFFSET(crc, FIT_FILE_HDR));
