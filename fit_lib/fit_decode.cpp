...
void Decode::IncompleteStream() {
  ...
  if (file != NULL) {
    throw RuntimeException(
        "Can't set incompleteStream option after Decode started!");
  }
...
void Decode::Pause(void) { pause = FIT_TRUE; }
...
FIT_BOOL Decode::Resume(void) {
    ...
    for (; currentByteIndex < bytesRead; currentByteIndex++) {
      ...
      switch (decodeReturn) {
        ...
        case RETURN_MESG:
          if (mesg.GetNum() == FIT_MESG_NUM_DEVELOPER_DATA_ID) {
            ...
            if (!devIdMesg.IsDeveloperDataIndexValid()) {
              throw fit::RuntimeException(
                  "Invalid developer data index in DeveloperDataIdMesg");
            }

            FIT_UINT8 index = devIdMesg.GetDeveloperDataIndex();
            developers[index] = devIdMesg;
            descriptions[index] =
                std::unordered_map<FIT_UINT8, FieldDescriptionMesg>();
            ...
            if (!descMesg.IsDeveloperDataIndexValid()) {
              throw fit::RuntimeException(
                  "Invalid developer data index in FieldDescriptionMesg");
            }

            if (!descMesg.IsFieldDefinitionNumberValid()) {
              throw fit::RuntimeException(
                  "Invalid developer field definition number in "
          ...
          if (mesgListener) mesgListener->OnMesg(mesg);
          break;
        ...
        case RETURN_END_OF_FILE:
          // Increment so we do not read the same byte twice in the case of a
          // chained file
          currentByteIndex++;
          currentByteOffset++;
          return FIT_TRUE;
  ...
  if ((streamIsComplete == FIT_TRUE) && (skipHeader == FIT_FALSE)) {
    // When decoding a complete file we should exit via RETURN_END_OF_FILE state
    // only.
    std::ostringstream message;
    message << "FIT decode error: Unexpected end of input stream at byte: "
            << currentByteOffset;
...
FIT_BOOL Decode::getInvalidDataSize(void) { return invalidDataSize; }

void Decode::setInvalidDataSize(FIT_BOOL value) { invalidDataSize = value; }

void Decode::InitRead(std::istream& file) { InitRead(file, FIT_TRUE); }
...
void Decode::InitRead(std::istream& file, FIT_BOOL startOfFile) {
  ...
  // to skip the header.
  if (skipHeader == FIT_FALSE) state = STATE_FILE_HDR;
  lastTimeOffset = 0;
...
void Decode::UpdateEndianness(FIT_UINT8 type, FIT_UINT8 size) {
  ...
  if (((type & FIT_BASE_TYPE_ENDIAN_FLAG) != 0) &&
      ((archs[localMesgIndex] & FIT_ARCH_ENDIAN_MASK) !=
       FIT_ARCH_ENDIAN_LITTLE)) {
...
Decode::RETURN Decode::ReadByte(FIT_UINT8 data) {
  if ((fileBytesLeft > 0) && (skipHeader == FIT_FALSE)) {
    ...
    if (fileBytesLeft == 1)  // CRC low byte.
    {
      if (state != STATE_RECORD) {
        std::ostringstream message;
        message
            << "FIT decode error: Decoder not in correct state after last data "
               "byte in file. Check message definitions. Error at byte: "
            << currentByteOffset;
        throw(RuntimeException(message.str()));
      }
    ...
    if (fileBytesLeft == 0)  // CRC high byte.
    {
      if (crc != 0) {
        std::ostringstream message;
        message << "FIT decode error: File CRC failed. Error at byte: "
                << currentByteOffset;
        throw(RuntimeException(message.str()));
      }
  ...
  switch (state) {
      ...
      switch (fileHdrOffset++) {
        case 0:
          if (data < FIT_HEADER_SIZE_NO_CRC) {
            std::ostringstream message;
            message << "FIT decode error: File header size invalid.  File is "
                       "not FIT. Error at byte : "
                    << currentByteOffset;
        ...
        case 1:
          if ((data & FIT_PROTOCOL_VERSION_MAJOR_MASK) >
              (FIT_PROTOCOL_VERSION_MAJOR
               << FIT_PROTOCOL_VERSION_MAJOR_SHIFT)) {
            std::ostringstream message;
            message << "FIT decode error: Protocol version "
                    << (data & FIT_PROTOCOL_VERSION_MAJOR_MASK)
                    << FIT_PROTOCOL_VERSION_MAJOR_SHIFT << "."
                    << (data & FIT_PROTOCOL_VERSION_MINOR_MASK)
                    << " not supported.  Must be " << FIT_PROTOCOL_VERSION_MAJOR
                    << ".15 or earlier.";
            headerException = message.str();
          }
          break;
        ...
        case 7:
          ...
          if ((fileDataSize == 0) && (invalidDataSize == FIT_FALSE)) {
            ...
            std::ostringstream message;
            message << "FIT decode error: File Size is 0. Error at byte : "
                    << currentByteOffset;
            headerException = message.str();
          }
          break;
        case 8:
          if (data != '.') {
            std::ostringstream message;
            message << "FIT decode error: File header signature mismatch.  "
                       "File is not FIT. Error at byte : "
                    << currentByteOffset;
            headerException = message.str();
          }
          break;
      ...
      if (fileBytesLeft > 1) {
        if ((data & FIT_HDR_TIME_REC_BIT) != 0) {
          Field timestampField =
              Field(Profile::MESG_RECORD, Profile::RECORD_MESG_TIMESTAMP);
          ...
          localMesgIndex =
              (data & FIT_HDR_TIME_TYPE_MASK) >> FIT_HDR_TIME_TYPE_SHIFT;

          if (localMesgDefs[localMesgIndex].GetNum() == FIT_MESG_NUM_INVALID) {
            std::ostringstream message;
            message << "FIT decode error: Missing FIT message definition for "
                       "local message number "
                    << ((int)localMesgIndex)
                    << ". Error at byte: " << currentByteOffset;
            throw(RuntimeException(message.str()));
          }
          ...
          if ((data & FIT_HDR_TYPE_DEF_BIT) != 0) {
            ...
            if (localMesgDefs[localMesgIndex].GetNum() ==
                FIT_MESG_NUM_INVALID) {
              std::ostringstream message;
              message << "FIT decode error: Missing FIT message definition for "
                         "local message number "
                      << ((int)localMesgIndex)
            ...
            if (localMesgDefs[localMesgIndex].GetFields().size() != 0) {
              state = STATE_FIELD_DATA;
            } else if (localMesgDefs[localMesgIndex].GetDevFields().size() !=
                       0) {
    ...
    case STATE_MESG_NUM_0:
      // Read the global message number bytes in as if they are in little endian
      // format.
    ...
    case STATE_MESG_NUM_1:
      localMesgDefs[localMesgIndex].SetNum(
          localMesgDefs[localMesgIndex].GetNum() | ((FIT_UINT16)data << 8));

      // We have to check for endianness.
      if (archs[localMesgIndex] == FIT_ARCH_ENDIAN_BIG) {
        localMesgDefs[localMesgIndex].SetNum(
            (localMesgDefs[localMesgIndex].GetNum() >> 8) |
    ...
    case STATE_FIELD_SIZE:
      if (data == 0) {
        // Bad Size
        std::ostringstream message;
        message << "FIT decode error: Invalid Field Size " << data
                << ". Error at byte: " << currentByteOffset;
        throw(RuntimeException(message.str()));
      }
    ...
    case STATE_DEV_FIELD_SIZE:
      if (data == 0) {
        // Bad Size
        std::ostringstream message;
        message << "FIT decode error: Invalid Developer Field Size " << data
                << ". Error at byte: " << currentByteOffset;
        throw(RuntimeException(message.str()));
      }
    ...
    case STATE_DEV_FIELD_INDEX:
      ...
      try {
        const FieldDescriptionMesg& desc =
            descriptions.at(fieldData[DevFieldIndexOffset])
                .at(fieldData[DevFieldNumOffset]);
        const DeveloperDataIdMesg& developer =
            developers.at(fieldData[DevFieldIndexOffset]);

        localMesgDefs[localMesgIndex].AddDevField(DeveloperFieldDefinition(
            desc, developer, fieldData[DevFieldSizeOffset]));
      } catch (std::out_of_range) {
        // No Matching Description Message add a Generic Definition
        localMesgDefs[localMesgIndex].AddDevField(DeveloperFieldDefinition(
            fieldData[DevFieldNumOffset], fieldData[DevFieldSizeOffset],
    ...
    case STATE_FIELD_DATA:
      if (fieldBytesLeft == 0) {
        fieldDataIndex = 0;
        fieldBytesLeft = localMesgDefs[localMesgIndex]
      ...
      if (fieldBytesLeft == 0) {
        ...
        if (baseType <
            FIT_BASE_TYPES)  // Ignore field if base type not supported.
        {
          ...
          if (field.IsValid())  // If known field type.
          {
            if (field.GetType() != fldDefn->GetType()) {
              FIT_UINT8 profileSize = fit::baseTypeSizes[(
                  field.GetType() & FIT_BASE_TYPE_NUM_MASK)];
            ...
            if (read) {
              field.Read(&fieldData,
                         defn.GetFieldByIndex(fieldIndex)->GetSize());
            }

            // The special case time record.
            if (defn.GetFieldByIndex(fieldIndex)->GetNum() ==
                FIT_FIELD_NUM_TIMESTAMP) {
              timestamp = field.GetUINT32Value();
              lastTimeOffset =
                  (FIT_UINT8)(timestamp & FIT_HDR_TIME_OFFSET_MASK);
            }

            // Allows messages containing the accumulated field to set the
            // accumulated value
              ...
              for (i = 0; i < field.GetNumValues(); i++) {
                ...
                for (j = 0; j < mesg.GetNumFields(); j++) {
                  ...
                  FIT_UINT16 numComponents =
                      containingField->GetNumComponents();

                  for (k = 0; k < numComponents; k++) {
                    const Profile::FIELD_COMPONENT* fc =
                        containingField->GetComponent(k);
                    if ((fc->num == field.GetNum()) && (fc->accumulate)) {
                      value =
                          ((((value / field.GetScale()) - field.GetOffset()) +
                            fc->offset) *
                           fc->scale);
                    }
                  }
                }
                accumulator.Set(mesg.GetNum(), field.GetNum(),
                                (FIT_UINT32)value);
              }
            }
      ...
      if (fieldIndex >= localMesgDefs[localMesgIndex].GetFields().size()) {
        // Now that the entire message is decoded we may evaluate subfields and
        // expand components
        for (FIT_UINT16 i = 0; i < mesg.GetNumFields(); i++) {
          FIT_UINT16 activeSubField =
              mesg.GetActiveSubFieldIndexByFieldIndex(i);
            ...
            if (activeSubField == FIT_SUBFIELD_INDEX_MAIN_FIELD) {
              if (mesg.GetFieldByIndex(i)->GetNumComponents() > 0) {
                ExpandComponents(mesg.GetFieldByIndex(i),
                                 mesg.GetFieldByIndex(i)->GetComponent(0),
                                 mesg.GetFieldByIndex(i)->GetNumComponents());
              }
            } else {
              if (mesg.GetFieldByIndex(i)
                      ->GetSubField(activeSubField)
                      ->numComponents > 0) {
                ExpandComponents(mesg.GetFieldByIndex(i),
                                 mesg.GetFieldByIndex(i)
                                     ->GetSubField(activeSubField)
                                     ->components,
                                 mesg.GetFieldByIndex(i)
                                     ->GetSubField(activeSubField)
    ...
    case STATE_DEV_FIELD_DATA: {
      ...
      DeveloperFieldDefinition* fieldDef =
          localMesgDef.GetDevFieldByIndex(fieldIndex);
      ...
      if (fieldBytesLeft == 0) {
        ...
        if (fieldBytesLeft == 0) {
          fieldBytesLeft =
              localMesgDefs->GetDevFieldByIndex(++fieldIndex)->GetSize();
        }
      }
...
void Decode::ExpandComponents(Field* containingField,
                              const Profile::FIELD_COMPONENT* components,
                              FIT_UINT16 numComponents) {
  FIT_UINT16 offset = 0;
  FIT_UINT16 i;
  ...
  for (i = 0; i < numComponents; i++) {
    ...
    if (component->num != FIT_FIELD_NUM_INVALID) {
      ...
      FIT_UINT16 subfieldIndex =
          mesg.GetActiveSubFieldIndex(componentField.GetNum());
      FIT_FLOAT64 value;
      ...
      if (componentField.IsSignedInteger()) {
        signedBitsValue =
            containingField->GetBitsSignedValue(offset, component->bits);

        if (signedBitsValue == FIT_SINT32_INVALID)
          break;  // No more data for components.

        if (component->accumulate)
          bitsValue = accumulator.Accumulate(mesg.GetNum(), component->num,
                                             signedBitsValue, component->bits);
      } else {
        ...
        if (component->accumulate)
          bitsValue = accumulator.Accumulate(mesg.GetNum(), component->num,
                                             bitsValue, component->bits);
      }

      // If the component field itself has *one* component apply the scale and
      // offset of the componentField's (nested) component
      if (componentField.GetNumComponents() == 1) {
        if (componentField.IsSignedInteger())
          value = (((signedBitsValue / (FIT_FLOAT64)component->scale) -
                    component->offset) +
                   componentField.GetComponent(0)->offset) *
                  componentField.GetComponent(0)->scale;
        else
          value = (((bitsValue / (FIT_FLOAT64)component->scale) -
                    component->offset) +
                   componentField.GetComponent(0)->offset) *
                  componentField.GetComponent(0)->scale;
      ...
      // The component field is itself a composite field (more than one
      // component).  Don't use scale/offset, containing field data must already
      // be encoded.  Add elements to it until we have added bitsvalue
      else if (componentField.GetNumComponents() > 1) {
        int bitsAdded = 0;
        long mask;

        while (bitsAdded < component->bits) {
          mask = ((long)1 << baseTypeSizes[componentField.GetType() &
                                           FIT_BASE_TYPE_NUM_MASK]) -
                 1;
          if (mesg.HasField(componentField.GetNum())) {
            ...
            componentField.AddValue(bitsValue & mask,
                                    componentField.GetNumValues());
            mesg.AddField(componentField);
          }
          bitsValue >>=
              baseTypeSizes[componentField.GetType() & FIT_BASE_TYPE_NUM_MASK];
          bitsAdded +=
              baseTypeSizes[componentField.GetType() & FIT_BASE_TYPE_NUM_MASK];
        }
      }
