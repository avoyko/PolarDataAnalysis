...
Mesg::Mesg(const Profile::MESG_INDEX index)
    : profile(&Profile::mesgs[index]), localNum(0), fields(), devFields() {}

Mesg::Mesg(const std::string &name)
    : profile(Profile::GetMesg(name)), localNum(0), fields(), devFields() {}

Mesg::Mesg(const FIT_UINT16 num)
    : profile(Profile::GetMesg(num)), localNum(0), fields(), devFields() {}

FIT_BOOL Mesg::IsValid(void) const { return (profile != FIT_NULL); }
...
const DeveloperField *Mesg::GetDeveloperField(FIT_UINT8 developerDataIndex,
                                              FIT_UINT8 num) const {
...
int Mesg::Write(std::ostream &file, const MesgDefinition *mesgDef) const {
  ...
  file.put(
      (localNum &
       FIT_HDR_TYPE_MASK));  // Message record header with local message number.
  ...
  for (FIT_UINT16 fieldDefIndex = 0;
       fieldDefIndex < (mesgDef->GetFields().size()); fieldDefIndex++) {
    const Field *field =
        GetField(mesgDef->GetFieldByIndex(fieldDefIndex)->GetNum());
    const FieldDefinition *definition = mesgDef->GetFieldByIndex(fieldDefIndex);

    fieldSize =
        WriteField(file, field, definition->GetSize(), definition->GetType());
    if (fieldSize == 0) {
  ...
  for (DeveloperFieldDefinition def : mesgDef->GetDevFields()) {
    const DeveloperField *field =
        GetDeveloperField(def.GetDeveloperDataIndex(), def.GetNum());
...
std::string Mesg::GetName() const {
  if (profile == FIT_NULL) return "unknown";

  return profile->name;
}

FIT_UINT16 Mesg::GetNum() const {
  if (profile == FIT_NULL) return FIT_MESG_NUM_INVALID;

  return profile->num;
}

FIT_UINT8 Mesg::GetLocalNum() const { return localNum; }

void Mesg::SetLocalNum(const FIT_UINT8 newLocalNum) {
  if (newLocalNum < FIT_MAX_LOCAL_MESGS) localNum = newLocalNum;
}

FIT_BOOL Mesg::HasField(const int fieldNum) const {
  for (int i = 0; i < (int)fields.size(); i++) {
    if (fields[i].GetNum() == fieldNum) return FIT_TRUE;
  }
...
void Mesg::AddField(const Field &field) {
  ...
  if (existingField == FIT_NULL) fields.push_back(field);
}
...
void Mesg::SetFields(const Mesg &mesg) {
  if (mesg.GetNum() != GetNum()) return;

  for (int i = 0; i < (int)mesg.fields.size(); i++) SetField(mesg.fields[i]);
}

int Mesg::GetNumFields() const { return (int)fields.size(); }

int Mesg::GetNumDevFields() const { return (int)devFields.size(); }
...
Field *Mesg::GetField(const FIT_UINT8 fieldNum) {
  for (int i = 0; i < (int)fields.size(); i++) {
    if (fields[i].GetNum() == fieldNum) return &fields[i];
  }
...
Field *Mesg::GetField(const std::string &name) {
  for (FIT_UINT16 i = 0; i < (FIT_UINT16)fields.size(); i++) {
    if (fields[i].GetName().compare(name) == 0) return &fields[i];

    for (FIT_UINT16 j = 0; j < fields[i].GetNumSubFields(); j++) {
      if ((fields[i].GetName(j).compare(name) == 0) &&
          CanSupportSubField(&(fields[i]), j))
        return &fields[i];
    }
  }
...
FIT_BOOL Mesg::CanSupportSubField(const FIT_UINT8 fieldNum,
                                  const FIT_UINT16 subFieldIndex) const {
  if ((subFieldIndex == FIT_SUBFIELD_INDEX_MAIN_FIELD) ||
      (subFieldIndex == FIT_SUBFIELD_INDEX_ACTIVE_SUBFIELD))
    return FIT_TRUE;

  const Profile::SUBFIELD *subField =
      Profile::GetSubField(GetNum(), fieldNum, subFieldIndex);

  if (subField == FIT_NULL) return FIT_FALSE;

  for (int i = 0; i < (int)subField->numMaps; i++) {
    const Field *refField = GetField(subField->maps[i].refFieldNum);

    if (refField != FIT_NULL) {
      FIT_FLOAT64 refValue =
          refField->GetFLOAT64Value(0, FIT_SUBFIELD_INDEX_MAIN_FIELD);
...
FIT_BOOL Mesg::CanSupportSubField(const Field *field,
                                  const FIT_UINT16 subFieldIndex) const {
  if ((subFieldIndex == FIT_SUBFIELD_INDEX_MAIN_FIELD) ||
      (subFieldIndex == FIT_SUBFIELD_INDEX_ACTIVE_SUBFIELD))
    return FIT_TRUE;

  if (field == FIT_NULL) return FIT_FALSE;
...
FIT_UINT16 Mesg::GetActiveSubFieldIndexByFieldIndex(
    const FIT_UINT16 fieldIndex) const {
  if ((int)fieldIndex >= GetNumFields()) return FIT_SUBFIELD_INDEX_MAIN_FIELD;
...
FIT_UINT16 Mesg::GetActiveSubFieldIndex(const FIT_UINT8 fieldNum) const {
  ...
  if (field == FIT_NULL) return FIT_SUBFIELD_INDEX_MAIN_FIELD;
...
FIT_UINT8 Mesg::GetFieldNumValues(const FIT_UINT8 fieldNum,
                                  const FIT_UINT16 subFieldIndex) const {
...
FIT_ENUM Mesg::GetFieldENUMValue(const FIT_UINT8 fieldNum,
...
FIT_BYTE Mesg::GetFieldBYTEValue(const FIT_UINT8 fieldNum,
                                 const FIT_UINT8 fieldArrayIndex,
                                 const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_SINT8 Mesg::GetFieldSINT8Value(const FIT_UINT8 fieldNum,
                                   const FIT_UINT8 fieldArrayIndex,
                                   const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT8 Mesg::GetFieldUINT8Value(const FIT_UINT8 fieldNum,
                                   const FIT_UINT8 fieldArrayIndex,
                                   const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT8Z Mesg::GetFieldUINT8ZValue(const FIT_UINT8 fieldNum,
                                     const FIT_UINT8 fieldArrayIndex,
                                     const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_SINT16 Mesg::GetFieldSINT16Value(const FIT_UINT8 fieldNum,
                                     const FIT_UINT8 fieldArrayIndex,
                                     const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT16 Mesg::GetFieldUINT16Value(const FIT_UINT8 fieldNum,
                                     const FIT_UINT8 fieldArrayIndex,
                                     const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT16Z Mesg::GetFieldUINT16ZValue(const FIT_UINT8 fieldNum,
                                       const FIT_UINT8 fieldArrayIndex,
                                       const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_SINT32 Mesg::GetFieldSINT32Value(const FIT_UINT8 fieldNum,
                                     const FIT_UINT8 fieldArrayIndex,
                                     const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT32 Mesg::GetFieldUINT32Value(const FIT_UINT8 fieldNum,
                                     const FIT_UINT8 fieldArrayIndex,
                                     const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT32Z Mesg::GetFieldUINT32ZValue(const FIT_UINT8 fieldNum,
                                       const FIT_UINT8 fieldArrayIndex,
                                       const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_SINT64 Mesg::GetFieldSINT64Value(const FIT_UINT8 fieldNum,
                                     const FIT_UINT8 fieldArrayIndex,
                                     const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT64 Mesg::GetFieldUINT64Value(const FIT_UINT8 fieldNum,
                                     const FIT_UINT8 fieldArrayIndex,
                                     const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_UINT64Z Mesg::GetFieldUINT64ZValue(const FIT_UINT8 fieldNum,
                                       const FIT_UINT8 fieldArrayIndex,
                                       const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
...
FIT_FLOAT32 Mesg::GetFieldFLOAT32Value(const FIT_UINT8 fieldNum,
                                       const FIT_UINT8 fieldArrayIndex,
                                       const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
  ...
  if (subFieldIndex == FIT_SUBFIELD_INDEX_ACTIVE_SUBFIELD)
    return field->GetFLOAT32Value(fieldArrayIndex,
                                  GetActiveSubFieldIndex(fieldNum));
  else
    return field->GetFLOAT32Value(fieldArrayIndex, subFieldIndex);
}

FIT_FLOAT64 Mesg::GetFieldFLOAT64Value(const FIT_UINT8 fieldNum,
                                       const FIT_UINT8 fieldArrayIndex,
                                       const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
  ...
  if (subFieldIndex == FIT_SUBFIELD_INDEX_ACTIVE_SUBFIELD)
    return field->GetFLOAT64Value(fieldArrayIndex,
                                  GetActiveSubFieldIndex(fieldNum));
  else
    return field->GetFLOAT64Value(fieldArrayIndex, subFieldIndex);
}

FIT_WSTRING Mesg::GetFieldSTRINGValue(const FIT_UINT8 fieldNum,
                                      const FIT_UINT8 fieldArrayIndex,
                                      const FIT_UINT16 subFieldIndex) const {
  const Field *field = GetField(fieldNum);
  ...
  if (subFieldIndex == FIT_SUBFIELD_INDEX_ACTIVE_SUBFIELD)
    return field->GetSTRINGValue(fieldArrayIndex,
                                 GetActiveSubFieldIndex(fieldNum));
  else
    return field->GetSTRINGValue(fieldArrayIndex, subFieldIndex);
}

void Mesg::SetFieldENUMValue(const FIT_UINT8 fieldNum, const FIT_ENUM value,
                             const FIT_UINT8 fieldArrayIndex,
                             const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;

  Field *field = AddField(fieldNum);

  if (field == FIT_NULL) return;

  return field->SetENUMValue(value, fieldArrayIndex);
}

void Mesg::SetFieldBYTEValue(const FIT_UINT8 fieldNum, const FIT_BYTE value,
                             const FIT_UINT8 fieldArrayIndex,
                             const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldSINT8Value(const FIT_UINT8 fieldNum, const FIT_SINT8 value,
                              const FIT_UINT8 fieldArrayIndex,
                              const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldUINT8Value(const FIT_UINT8 fieldNum, const FIT_UINT8 value,
                              const FIT_UINT8 fieldArrayIndex,
                              const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldUINT8ZValue(const FIT_UINT8 fieldNum, const FIT_UINT8 value,
                               const FIT_UINT8 fieldArrayIndex,
                               const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldSINT16Value(const FIT_UINT8 fieldNum, const FIT_SINT16 value,
                               const FIT_UINT8 fieldArrayIndex,
                               const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldUINT16Value(const FIT_UINT8 fieldNum, const FIT_UINT16 value,
                               const FIT_UINT8 fieldArrayIndex,
                               const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldUINT16ZValue(const FIT_UINT8 fieldNum,
                                const FIT_UINT16Z value,
                                const FIT_UINT8 fieldArrayIndex,
                                const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldSINT32Value(const FIT_UINT8 fieldNum, const FIT_SINT32 value,
                               const FIT_UINT8 fieldArrayIndex,
                               const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldUINT32Value(const FIT_UINT8 fieldNum, const FIT_UINT32 value,
                               const FIT_UINT8 fieldArrayIndex,
                               const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldUINT32ZValue(const FIT_UINT8 fieldNum,
                                const FIT_UINT32Z value,
                                const FIT_UINT8 fieldArrayIndex,
                                const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldSINT64Value(const FIT_UINT8 fieldNum, const FIT_SINT64 value,
                               const FIT_UINT8 fieldArrayIndex,
                               const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldUINT64ZValue(const FIT_UINT8 fieldNum,
                                const FIT_UINT64Z value,
                                const FIT_UINT8 fieldArrayIndex,
                                const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
...
void Mesg::SetFieldFLOAT32Value(const FIT_UINT8 fieldNum,
                                const FIT_FLOAT32 value,
                                const FIT_UINT8 fieldArrayIndex,
                                const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
  ...
  if (subFieldIndex == FIT_SUBFIELD_INDEX_ACTIVE_SUBFIELD)
    return field->SetFLOAT32Value(value, fieldArrayIndex,
                                  GetActiveSubFieldIndex(fieldNum));
  else
    return field->SetFLOAT32Value(value, fieldArrayIndex, subFieldIndex);
}

void Mesg::SetFieldFLOAT64Value(const FIT_UINT8 fieldNum,
                                const FIT_FLOAT64 value,
                                const FIT_UINT8 fieldArrayIndex,
                                const FIT_UINT16 subFieldIndex) {
  if (!CanSupportSubField(fieldNum, subFieldIndex)) return;
  ...
  if (subFieldIndex == FIT_SUBFIELD_INDEX_ACTIVE_SUBFIELD)
    return field->SetFLOAT64Value(value, fieldArrayIndex,
                                  GetActiveSubFieldIndex(fieldNum));
  else
    return field->SetFLOAT64Value(value, fieldArrayIndex, subFieldIndex);
}

void Mesg::SetFieldSTRINGValue(const FIT_UINT8 fieldNum,
                               const FIT_WSTRING &value,
                               const FIT_UINT8 fieldArrayIndex) {
  Field *field = AddField(fieldNum);
...
std::vector<const FieldBase *> Mesg::GetOverrideFields(
    const FIT_UINT8 fieldNum) const {
...
int Mesg::WriteField(std::ostream &file, const FieldBase *field,
                     FIT_UINT8 defSize, FIT_UINT8 defType) {
