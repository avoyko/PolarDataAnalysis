...
void MesgWithEventBroadcaster::AddListener(
    MesgWithEventListener& mesgObserver) {
  listeners.push_back(&mesgObserver);
}

void MesgWithEventBroadcaster::RemoveListener(
    MesgWithEventListener& mesgObserver) {
  std::vector<MesgWithEventListener*>::iterator it =
      std::find(listeners.begin(), listeners.end(), &mesgObserver);
  if (listeners.end() != it) listeners.erase(it);
}
...
void MesgWithEventBroadcaster::OnMesg(MesgWithEvent& mesg) {
  ...
  switch (broadcastMesg.GetEventType())
  {
     case EVENT_TYPE_START:
        for (list<int>::iterator it = startedEvents.begin(); it <
  startedEvents[group].end(); it++)
        {
     ...
     case EVENT_TYPE_STOP_DISABLE:
        for (int i = 0; i < startedEvents.get(group).size(); i++) {
           if (startedEvents.get(group).get(i).getEvent() ==
  broadcastMesg.getEvent()) { startedEvents.get(group).remove(i);
           }
        }
        break;
     ...
     case EVENT_TYPE_STOP_ALL:
        for (int i = 0; i < (int)startedEvents[group].size(); i++) {
           if (startedEvents.get(group).get(i).getEvent() !=
  broadcastMesg.getEvent()) { MesgWithEvent stopEvent =
